...
from fastapi import Depends, Request, FastAPI, HTTPException, UploadFile, File
...
_PARSE_SEMAPHORE = asyncio.Semaphore(3)  # throttle sync_folder parse jobs
...
async def auth_delete_key(key_value: str, _admin=Depends(require_admin)):
    return _delete_auth_key(key_value)


@app.post("/api/auth/keys/delete")
async def auth_delete_key_body(req: AuthKeyDelete, _admin=Depends(require_admin)):
    return _delete_auth_key(req.key_value)


def _delete_auth_key(key_value: str):
    conn = _auth_db()
    try:
...
async def list_datasets(_user=Depends(require_user)): return await rag_backend.list_datasets()
...
async def sync_folder(folder: str, _admin=Depends(require_admin)):
    ...
    job_id = job["id"]
    job_tracker[job_id] = {"dataset_id": ds.id, "dataset_name": ds_name, "status": "SCANNING", "total": 0, "processed": 0, "started_at": job["started_at"], "message": "Сканирование..."}
    ...
    job_service.update(job_id, total=len(files), status="running")
    ...
    for i, f in enumerate(files):
        ...
        if is_new or is_changed:
            await rag_backend.upload_file(ds.id, f, relative_path=rel.as_posix())
        ...
        job_service.update(job_id, processed=i + 1, message=job_tracker[job_id]["message"])
    ...
    if force_reindex:
        ...
        job_service.update(job_id, status="running", message=job_tracker[job_id]["message"])
        ...
        job_service.update(
            job_id,
            status="running" if (new_count + changed_count) > 0 else "completed",
            message=job_tracker[job_id]["message"],
    ...
    if (new_count + changed_count) > 0 or force_reindex:
        async def _run():
            try:
                ...
                job_service.update(job_id, status="failed", errors=1, message=job_tracker[job_id]["message"])
...
async def upload_file(dataset_id: str, file: UploadFile = File(...), _admin=Depends(require_admin)):
    ...
    temp_path = await save_upload_tmp(file)
    doc_id = await rag_backend.upload_file(dataset_id, temp_path, relative_path=original_name)
    async def _parse():
        try:
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            for attempt in range(1, 3):
                ...
                if attempt == 2:
                    ...
                    ctx_chunks = chunks
                    context  = _build_ctx(ctx_chunks, 12000)
                    sys_msg  = _SYS_NORMAL
            ...
            # ── Если ответ не подтверждён — безопасный fallback ────────────────
            answer, crag_status = final_answer_for_status(answer, crag_status)
            if answer == _SAFE_FALLBACK:
                logger.error(f"[SAFERAG] Ответ не подтверждён ({crag_status}) — блокируем")
...
async def run_diagnostics(_internal=Depends(require_internal)):
    ...
    # ── LLM lightweight latency (read-only ping) ──
    async def _chk_chat():
        t = time.time()
        llm_url = os.getenv("MLX_URL", "http://host.docker.internal:8080")
        ...
        return status, f"{ms:.0f} ms", "<5000 ms", "MLX health OK" if ok else f"HTTP {r.status_code}"
...
def _ejik_job(job_type: str, source: str) -> tuple:
    ...
    durable = job_service.create(job_type, source=source, status="running")
    jid = durable["id"]
    job = {
        "id": jid,
        "type": job_type,
...
async def upload_pst(dataset_id: str, file: UploadFile = File(...), _admin=Depends(require_admin)):
    """
    ...
    safe_name = safe_upload_name(file.filename or "", {".pst"})
    if not safe_name.lower().endswith(".pst"):
    ...
    pst_path = pst_dir / f"{uuid.uuid4().hex}_{safe_name}"
    ...
    logger.info(f"[EJIK] PST загружен: {safe_name} ({size_mb:.1f} MB), job={jid}")
    ...
    async def _parse_pst():
        try:
            ...
            job_service.update(jid, status="completed", processed=job["processed"], total=job["total"], errors=job["errors"], message=job["message"], result={"chunks_added": job["chunks_added"]})
            logger.info(f"[EJIK] PST done: {safe_name} | {job['processed']} писем")

        except Exception as e:
            job["status"] = "error"
            job["message"] = str(e)
            job["finished_at"] = datetime.utcnow().isoformat()
            job_service.update(jid, status="failed", processed=job["processed"], total=job["total"], errors=job["errors"] + 1, message=job["message"])
            logger.error(f"[EJIK] PST error: {e}")
        finally:
            pst_path.unlink(missing_ok=True)

    asyncio.create_task(_parse_pst())
    return {"job_id": jid, "status": "running", "source": safe_name, "size_mb": round(size_mb, 1)}


@app.post("/api/mail/sync-imap")
async def sync_imap(req: IMAPRequest, dataset_id: str, _admin=Depends(require_admin)):
    """
    ...
    async def _do_imap():
        try:
            ...
            job_service.update(jid, total=job["total"], message=f"{job['total']} новых писем")
            ...
            job_service.update(jid, status="completed", processed=job["processed"], total=job["total"], errors=job["errors"], message=job["message"], result={"chunks_added": added})
...
async def mail_status(_admin=Depends(require_admin)):
...
async def index_table(dataset_id: str, file: UploadFile = File(...), _admin=Depends(require_admin)):
    """
    ...
    safe_name = safe_upload_name(file.filename or "", {".xlsx", ".xls", ".csv"})
    ext = Path(safe_name).suffix.lower()
    if ext not in (".xlsx", ".xls", ".csv"):
        raise HTTPException(400, "Ожидается XLSX или CSV файл")

    content = await file.read()
    tmp_path = Path(f"/tmp/{uuid.uuid4().hex}_{safe_name}")
    await asyncio.to_thread(tmp_path.write_bytes, content)

    jid, job = _ejik_job("parquet", safe_name)
    ...
    async def _do_parquet():
        try:
            ...
            job_service.update(jid, total=job["total"], processed=job["processed"])
            ...
            job_service.update(jid, status="completed", processed=job["processed"], total=job["total"], errors=job["errors"], message=job["message"], result={"chunks_added": added, "parquet_path": result.get("parquet_path"), "doc_type": result.get("doc_type")})
            logger.info(f"[PARQUET] done: {safe_name} | {result['doc_type']} | {result['rows']} строк")
            ...
            job["finished_at"] = datetime.utcnow().isoformat()
            job_service.update(jid, status="failed", processed=job["processed"], total=job["total"], errors=job["errors"] + 1, message=job["message"])
            logger.error(f"[PARQUET] error {safe_name}: {e}")
        finally:
            tmp_path.unlink(missing_ok=True)

    asyncio.create_task(_do_parquet())
    return {"job_id": jid, "status": "running", "source": safe_name}
...
async def get_jobs(_user=Depends(require_user)):
    durable = job_service.list()
    merged = dict(durable)
    merged.update(job_tracker)
    return merged
