...
def _vision_extract_rows(image) -> list[dict]:
    ...
    glossary = os.getenv(
        ...
        "чек-листы приёмки электрики/слаботочки/ОВ). Типичные графы шапки: Поз., "
        "Помещение, Наименование оборудования, Тип/марка, Артикул, Ед.изм., "
        "Количество, Отметка о приёмке (АОРПИ, монтаж, наладка — часто графы ГП/ФН "
        "с подписью и датой), Замечание/Примечание, Корпус. Марки щитов/шкафов: "
        "ШР, ШО, ШС, ШРП, ШРМ, ШПО, РШТ, ЩРН, ЩО. ",
    )
    prompt = (
        "Ты распознаёшь СКАН строительной ведомости/таблицы по помещениям или позициям. "
        + glossary +
