...
def review_summary(items: list[ReviewItem]) -> dict:
    ...
    for it in items:
        ...
        decision = it.human_decision or "unset"
    ...
    return {"total": len(items), "by_status": by_status,
            ...
            "by_human_decision": by_decision,
            "human_confirmed": by_decision.get("confirmed", 0),
            "human_rejected": by_decision.get("rejected", 0),
            "human_needs_more_evidence": by_decision.get("needs_more_evidence", 0)}


HUMAN_DECISIONS = {"unset", "confirmed", "rejected", "needs_more_evidence"}


def apply_human_decisions(items: list[ReviewItem], decisions: dict) -> list[ReviewItem]:
    """Apply persisted engineer decisions to review items.

    ``decisions`` may be ``{"RULE": "confirmed"}`` or
    ``{"RULE": {"decision": "confirmed", "comment": "...", "decided_at": "..."}}``.
    Invalid values are ignored so a damaged sidecar cannot corrupt the review output.
    """
    if not isinstance(decisions, dict):
        return items
    for it in items:
        raw = decisions.get(it.rule_id)
        if isinstance(raw, dict):
            decision = str(raw.get("decision") or "unset").strip()
            comment = str(raw.get("comment") or "").strip()
            decided_at = str(raw.get("decided_at") or "").strip()
        else:
            decision = str(raw or "unset").strip()
            comment = ""
            decided_at = ""
        if decision not in HUMAN_DECISIONS:
            continue
        it.human_decision = decision
        it.human_comment = comment
        it.human_decided_at = decided_at
    return items
...
def review_defense_pack(items: list[ReviewItem], review_map: ReviewMap) -> dict:
    ...
    for it in items:
        ...
        claims.append(DefenseClaim(
            ...
            gaps=gaps,
            actions=([] if it.human_decision in {"confirmed", "rejected"} else actions),
    ...
    return DefensePack(
        ...
        summary={
            ...
            "human_rejected": summary["human_rejected"],
            "human_needs_more_evidence": summary["human_needs_more_evidence"],
...
def review_to_normalized_remarks(items: list[ReviewItem], review_map: ReviewMap) -> list[dict]:
    ...
    for it in items:
        ...
        out.append({
            ...
            "human_comment": it.human_comment,
            "human_decided_at": it.human_decided_at,
...
def review_to_html(items: list[ReviewItem], review_map: ReviewMap) -> str:
    rows = "".join(
        ...
        f"<td>{it.severity}</td><td>{it.human_decision}</td><td>{it.target}</td><td>{it.model_note}</td></tr>"
        for it in items)
    s = review_summary(items)
...
def review_to_xlsx(items: list[ReviewItem], output_path: Path, review_map: ReviewMap) -> int:
    ...
    ws.append([])
    ws.append(["Правило", "Пункт", "Статус", "Severity", "Решение инженера",
               "Комментарий инженера", "Объект", "Требование", "Пояснение"])
    for cell in ws[3]:
        cell.font = Font(bold=True)
    for it in items:
        ws.append([it.rule_id, it.clause, _STATUS_RU.get(it.status, it.status), it.severity,
                   it.human_decision, it.human_comment, it.target, it.requirement.get("source_ref", ""),
                   it.model_note])
    for col, width in {"A": 22, "B": 8, "C": 20, "D": 10, "E": 18, "F": 34, "G": 34, "H": 34, "I": 60}.items():
        ws.column_dimensions[col].width = width
    wr = wb.create_sheet("normalized_remarks")
    wr.append(["id", "standard", "clause", "status", "severity", "target", "source_refs", "message",
               "human_decision", "human_comment", "finality"])
    for cell in wr[1]:
        cell.font = Font(bold=True)
    for remark in review_to_normalized_remarks(items, review_map):
        wr.append([
            ...
            remark["target"], "; ".join(remark["source_refs"]), remark["message"],
            remark["human_decision"], remark["human_comment"], remark["finality"],
        ])
    for col, width in {"A": 22, "B": 24, "C": 10, "D": 20, "E": 10, "F": 34,
                       "G": 60, "H": 60, "I": 18, "J": 34, "K": 14}.items():
