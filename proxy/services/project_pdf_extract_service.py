...
PROJECT_PDF_EXTRACT_ALGO_VERSION = "0.24.0.313"
...
def project_pdf_extract_root(dataset_id: str, *, storage_root: Path = Path("storage/datasets")) -> Path:
    return storage_root / str(dataset_id) / PROJECT_PDF_EXTRACT_DIR
...
def project_pdf_extract_status(
    ...
    *,
    ...
    return {
        ...
        "files_extracted": len(summary.get("files") or []) if isinstance(summary, dict) else 0,
...
def project_pdf_extract_summary(
    ...
    *,
    ...
    return {
        ...
        "source_navigation": [],
        "coverage": {"pdf_documents": 0, "files_extracted": 0},
...
def run_project_pdf_extract(
    ...
    *,
    ...
    result = {
        ...
        "sidecar_root": root.as_posix(),
        "files": files,
        "volume_register": volume_register[:300],
        ...
        "coverage": _coverage(docs, files, electrical_summary, project_table_summary),
...
def _candidate_pdf_paths(raw_path: Path, *, dataset_id: str, storage_root: Path) -> list[Path]:
    if raw_path.is_absolute():
        return [raw_path]
    dataset_root = storage_root / str(dataset_id)
    return [
        raw_path,
...
def _doc_role(file_name: str, fields: dict[str, Any]) -> str:
    text = f"{file_name} {fields.get('document_kind_code') or ''}".casefold()
    if "вор" in text:
        return "ведомость объемов работ"
    if "со" in text or "спецификац" in text:
        return "спецификация оборудования"
    if "пз" in text or "поясн" in text:
        return "пояснительная записка"
    if "содержание" in text or "состав" in text:
        return "состав тома"
    if "расчет нагруз" in text or "расчёт нагруз" in text:
        return "таблица расчета нагрузок"
    return "проектный PDF"


def _discipline(file_name: str, fields: dict[str, Any]) -> str:
    code = str(fields.get("subdiscipline_code") or fields.get("discipline_code") or "").upper()
    if code:
        return code
    text = file_name.upper()
    for token in ("ЭОМ", "ЭС", "ОВ", "ВК", "СС", "АПС", "АР", "КР", "КЖ", "КМ"):
        if token in text:
            return token
    return ""


def _looks_electrical(file_name: str, drawing: dict[str, Any], pd_manifest: dict[str, Any]) -> bool:
    text = file_name.upper()
    if any(token in text for token in ("ЭОМ", "ЭС", "ГРЩ", "ВРУ", "РАСЧЕТ НАГРУЗ", "РАСЧЁТ НАГРУЗ")):
        return True
    fields = _primary_fields(drawing)
    if str(fields.get("subdiscipline_code") or "").upper() == "ЭС":
        return True
    for row in (pd_manifest.get("volume_contents_register") or {}).get("rows", []) if isinstance(pd_manifest, dict) else []:
        joined = f"{row.get('designation') or ''} {row.get('name') or ''}".upper()
        if any(token in joined for token in ("ЭОМ", "ЭС", "ЭЛЕКТРО", "ГРЩ", "ВРУ")):
            return True
    return False
...
def _manifest_source_refs(manifest: dict[str, Any]) -> list[str]:
    ...
    for page in manifest.get("pages") or []:
        if isinstance(page, dict):
            refs.append(f"{manifest.get('file_name')}#page={page.get('page')}")
    for row in manifest.get("volume_contents") or []:
        if isinstance(row, dict) and row.get("source_ref"):
            refs.append(str(row.get("source_ref")))
    return refs
...
def _coverage(
    ...
    return {
        ...
        "volume_rows": sum(1 for item in files if item.get("doc_role") == "состав тома"),
