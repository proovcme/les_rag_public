...
def ks3_rows(assembled: dict[str, Any]) -> list[list[str]]:
    """КС-3 MVP: одна итоговая строка; все три денежные колонки = grand total."""
    grand = _f((assembled.get("summary") or {}).get("total"))
    ...
    *,
    ...
    if isinstance(rim_form, dict):
        ...
        if from_rim["positions"]:
            return from_rim
    from_session = load_last_assembled_from_session(session_id)
    if from_session:
        return from_session
    return load_latest_assembled_from_disk(artifact_dir)
    ...
    *,
    ...
    if fid in KS2_KS3_FORMS:
        ...
        resolved_assembled = resolve_assembled(
            assembled=assembled, rim_form=rim_form, session_id=session_id,
        )
