...
def _h_doc_review(q: str, pid: int):
    # Нормоконтроль требует документов комплекта (датасета) — исполнение в потоке chat.py,
    # где есть скоуп (effective_dataset_ids). Роутер ВЫБИРАЕТ интент, блок ниже исполняет.
    return None
...
_TOOLS: tuple[dict[str, Any], ...] = (
    ...
    {"name": "price_lookup", "handler": _h_smeta,
     "desc": "ЦЕНА / стоимость / расценка ресурса или позиции по КОДУ-ШИФРУ ФГИС ЦС (формат вида "
             "91.05.01-017 или 01.7.15.06-0111). Любой вопрос «цена/сколько стоит/какая расценка» с "
             "таким кодом — это РАСЧЁТ цены инструментом, НЕ справка/RAG.",
     "examples": ["цена ресурса 91.05.01-017", "сколько стоит позиция 01.7.15.06-0111",
                  "какая расценка у кода 91.05.01-017 в ФГИС ЦС"]},
    ...
    {"name": "doc_review", "handler": _h_doc_review,
     "desc": "НОРМОКОНТРОЛЬ / проверка комплекта проектной документации по ГОСТ Р 21.101 (СПДС): "
             "состав комплекта, обозначения/шифры, соответствие ведомости, оформление. Это ПРОВЕРКА "
    ...
    {"name": "help", "handler": _h_help,
     "desc": "СПРАВКА как ПОЛЬЗОВАТЬСЯ системой ЛЕС: «что умеешь / как спрашивать / помощь / "
             "что можно спросить». НЕ для вопросов о содержании, объекте, документах или нормативах "
             "(те → none).",
     "examples": ["что ты умеешь", "как спрашивать ЛЕС", "помощь"]},
    {"name": "none", "handler": None,
     "desc": "ЛЮБОЙ вопрос об информации/содержании/фактах (что известно про X, расскажи/объясни "
             "про X, справка по объекту, какие требования, что в папке, состав/реестр документации "
             "объекта, сводка проекта) — обычный поиск по документам (RAG). Это выбор ПО УМОЛЧАНИЮ.",
...
_FEWSHOT: tuple[tuple[str, str], ...] = (
    ...
    ("сделай нормоконтроль комплекта по ГОСТ Р 21.101", "doc_review"),
    ("проверь оформление документации объекта по СПДС", "doc_review"),
)
