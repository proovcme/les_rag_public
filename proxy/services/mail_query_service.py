...
# Демоут пресета (RAG-first): этот сервис — ТОЛЬКО тонкая НАВИГАЦИЯ по ящику
# (покажи/список/сколько последних писем, письма от X). Любой контентный или
# аналитический вопрос («какие задачи», «о чём договорились по кровле», «что
# висит на Иване», «собери график из писем») здесь НЕ перехватывается — он
# уходит в обычный семантический RAG со scope на MAIL-датасет
# (chat.py → retrieve_chat_chunks), где LLM рассуждает над извлечёнными чанками
# писем и отвечает С ЦИТАТАМИ. Никаких keyword-пресетов на ПОНИМАНИЕ темы.
#
# Навигация = глагол показа/счёта/выборки + почтовый объект. Это «достань мне
# письма», а не «что в письмах написано».
NAV_VERB_TOKENS = (
    "покажи",
    "показать",
    "выведи",
    ...
    "сколько",
    "найди",
    "последн",  # последние / последних
    "открой",
    "дай ",
)

# Чистые навигационные фразы-исключения, где объект «письма» подразумевается
# глаголом-выборкой (например «покажи список писем»). Тут нет анализа смысла.
MAIL_OBJECT_TOKENS = (
    "письм",
    "писем",  # род. мн.: «сколько писем», «список писем»
    "почт",
    "переписк",
    "цепочк",
    "thread",
...
# Сильные сигналы КОНТЕНТНОГО/аналитического вопроса. Если хоть один присутствует
# — это НЕ навигация, отдаём в RAG (даже если рядом стоит «покажи»).
...
def _looks_like_mail_query(question: str) -> bool:
    """Только НАВИГАЦИЯ по ящику (демоут пресета, RAG-first).

    True ⇔ это явная просьба «достань/покажи/посчитай письма» БЕЗ контентного
    интента. Контентные/аналитические вопросы («какие задачи», «о чём
    договорились», «что висит на Иване») → False, чтобы chat.py пустил их в
    обычный семантический RAG над MAIL-датасетом (ответ с цитатами из писем).
    """
    q = question.casefold()
    # Любой признак содержательного вопроса → не навигация, в RAG.
    if any(token in q for token in CONTENT_INTENT_TOKENS):
        return False
    has_nav_verb = any(token in q for token in NAV_VERB_TOKENS)
    has_mail_object = any(token in q for token in MAIL_OBJECT_TOKENS)
    return has_nav_verb and has_mail_object
