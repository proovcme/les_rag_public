...
- **Split UI:** `les.ovc.me/` открывает только лёгкий AI-чат и историю; `les.ovc.me/les` открывает админский контур. Основной чат больше не монтирует обзор, Самовар, Прораб, диагностику и В.О.Л.К.
...
| `les.ovc.me`, если Caddy видит клиента как `10.195.146.x` | не нужен | Caddy ставит `X-LES-Trusted-Network: 1`; proxy/UI доверяют заголовку только от `TRUSTED_PROXY_NETWORKS` |
...
└── С.О.В.У.Ш.К.А. (NiceGUI, порт 8051) — UI v5.0+
    ...
    └── /les         — ОБЗОР, С.А.М.О.В.А.Р., П.Р.О.Р.А.Б., ГРАФ, ДИАГН, В.О.Л.К.
```
