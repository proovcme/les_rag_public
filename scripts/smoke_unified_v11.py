#!/usr/bin/env python
"""Unified Construction Harness v0.11 — real-project acceptance smoke + index health.
...
Per-question try/except (один сбой не валит прогон). Дамп JSON + append failure-ledger.
  ...
  # фикстура
  LES_UNIFIED_CONSTRUCTION_HARNESS_ENABLED=1 uv run python scripts/smoke_unified_v11.py --append-ledger
  # реальный датасет рантайма (read-only)
  LES_UNIFIED_CONSTRUCTION_HARNESS_ENABLED=1 uv run python scripts/smoke_unified_v11.py \
    --dataset-id <ID> --storage-root /Users/ovc/LES/storage/datasets \
    --output artifacts/unified_v11_real_smoke.json --append-ledger
"""
...
def _failure(status, intent, astat, health) -> str | None:
    ...
    asbuilt-без-актов ≠ mail). v0.11-фикс."""
...
async def amain(args) -> None:
    ...
    print(f"# Unified Harness v0.11 smoke | source={source} | datasets={len(dsids)}")
    for d in health["datasets"]:
        print(f"  health[{d['dataset_id'][:8]}] parquet={d['parquet_count']} files={d['file_count']} "
    ...
    out.write_text(json.dumps({"version": "v0.11", "source": source, "datasets": dsids,
    ...
    if args.append_ledger:
        ...
        with led.open("a", encoding="utf-8") as f:
            f.write(f"\n## append (smoke v0.11, source={source}, datasets={len(dsids)})\n")
...
def main() -> None:
    ...
    ap.add_argument("--output", default="artifacts/unified_v11_smoke.json")
