...
- `ARTEL.Revit.FamilyFactory/` — Revit 2025 add-in для extract/validate
  команд и отправки validation report в backend.
- `openapi/agnostis-mvp.yaml` — начальная OpenAPI-схема MVP.
- `skills/agnostis/` — Codex skill для работы с АРТЕЛЬ и LES RAG контуром.
- GitHub Pages workflow живет только в standalone mirror `proovcme/Agnostis`; в LES snapshot он не переносится.
...
## Revit Family Factory

На Windows/Revit host сборка и установка add-in:
...
Подробно: [docs/family-factory.md](docs/family-factory.md) и
