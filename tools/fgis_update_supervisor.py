...
def run_supervised(*, include_gesn: bool, all_periods: bool, attempts: int = 5) -> int:
    ...
    for attempt in range(1, max(1, attempts) + 1):
        code = subprocess.run(command, cwd=str(Path(__file__).resolve().parents[1]), check=False).returncode
        ...
        _write_json(
            DEFAULT_STATUS,
            {
                **status,
                ...
                "retry": {"attempt": attempt + 1, "maximum": max(1, attempts), "delay_seconds": delay},
            },
        )
        time.sleep(delay)
    return 1
