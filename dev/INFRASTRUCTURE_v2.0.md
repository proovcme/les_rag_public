...
Статус: ✅ Активна | Обновлено: 11.05.2026 | Версия: 2.0.1
...
| les-proxy | python:3.11-slim (custom) | 8050 | ~0.5 GB | FastAPI ядро, CRAG, ConverterRouter, SSE, Metrics, JobTracker |
...
Подача 220В → Mac Mini включается (`autorestart 1`).
Загрузка macOS → автологин `ovc`.
Запуск Login Items → Docker Desktop, Ollama.
`docker compose up -d` (если не настроен автозапуск compose).
...
# Логи индексации
docker logs -f les-proxy | grep -E "\[PARSE\]|\[CONVERT\]|\[JOB\]"
...
Или через API: `POST /api/rag/sync/NTD` (возвращает `job_id` для отслеживания прогресса).
...
| 11.05.2026 | Внедрён Job Tracker: /api/jobs, прогресс-бары в UI, статусы SCANNING/PARSING/COMPLETED. |

📅 Документация актуальна на: 11.05.2026

🐳 Сервисы и контейнеры v2.0 (Обновлено 11.05.2026)
...
Job Tracker использует in-memory dict для хранения состояния задач, обновляется в реальном времени при копировании и парсинге файлов.
