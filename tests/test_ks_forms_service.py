...
def test_ks2_xlsx_official_layout(tmp_path):
    ...
    res = K.build_ks_document(
        "ks2", assembled=_assembled(), out_path=out, source="last_lsr",
    )
...
def test_chat_ks2_filename_has_xlsx_extension():
    res = chat.answer_ks_forms_query("собери КС-2", assembled=_assembled())
...
def test_ks3_rows_period_totals():
    ...
    assert len(rows) == 1
    assert rows[0][2] == rows[0][3] == rows[0][4] == "234210.00"
...
def test_chat_detect_and_clarify():
    ...
    assert "ЛСР" in clarified["answer"]
