...
def test_sparse_sidecar_delete_noops_when_disabled(monkeypatch):
    ...
    adapter._file_filter = QdrantLlamaIndexAdapter._file_filter.__get__(adapter)
    adapter._sparse_collection_name = QdrantLlamaIndexAdapter._sparse_collection_name.__get__(adapter)
    adapter._sync_sparse_sidecar_exists = QdrantLlamaIndexAdapter._sync_sparse_sidecar_exists.__get__(adapter)
    adapter._sync_delete_sparse_file_points = QdrantLlamaIndexAdapter._sync_delete_sparse_file_points.__get__(adapter)
    ...
    assert adapter._sync_delete_sparse_file_points(FakeQdrant(), "ds-1", "doc.md") is False
    assert calls == []
...
def test_sparse_sidecar_delete_uses_same_file_filter(monkeypatch):
    ...
    assert adapter._sync_delete_sparse_file_points(FakeQdrant(), "ds-1", "nested/doc.md") is True
    assert calls[0] == ("exists", "les_rag_sparse")
    delete_kwargs = calls[1][1]
    assert delete_kwargs["collection_name"] == "les_rag_sparse"
    assert delete_kwargs["wait"] is True
    conditions = delete_kwargs["points_selector"].filter.must
    assert [(condition.key, condition.match.value) for condition in conditions] == [
        ("file_name", "nested/doc.md"),
        ("dataset_id", "ds-1"),
    ]
...
def test_sparse_sidecar_upsert_uses_dense_point_ids_and_payload(monkeypatch):
    ...
    adapter._sparse_collection_name = QdrantLlamaIndexAdapter._sparse_collection_name.__get__(adapter)
    adapter._sync_sparse_sidecar_exists = QdrantLlamaIndexAdapter._sync_sparse_sidecar_exists.__get__(adapter)
    adapter._sparse_point_from_dense = QdrantLlamaIndexAdapter._sparse_point_from_dense
    adapter._sync_upsert_sparse_points = QdrantLlamaIndexAdapter._sync_upsert_sparse_points.__get__(adapter)
    ...
    dense_point = SimpleNamespace(
        id="point-1",
        payload={"text": "пожарная эвакуация эвакуация", "dataset_id": "ds-1", "file_name": "doc.md"},
    )

    indexed = adapter._sync_upsert_sparse_points(FakeQdrant(), [dense_point])

    assert indexed == 1
    assert upserts[0]["collection_name"] == "les_rag_sparse"
    sparse_point = upserts[0]["points"][0]
    assert sparse_point.id == "point-1"
    assert sparse_point.payload == dense_point.payload
    assert "bm25_sparse" in sparse_point.vector
...
def test_reconcile_reports_and_cleans_sparse_drift(monkeypatch):
    ...
    class FakeDB:
        def __init__(self):
            self.updated = []
            self.chunk_count_updates = 0

        def indexed_files_with_counts(self, dataset_id):
            return [("doc.md", 2)]

        def update_document_status(self, dataset_id, file_name, status, chunk_count):
            self.updated.append((dataset_id, file_name, status, chunk_count))

        def update_dataset_chunk_count(self, dataset_id):
            self.chunk_count_updates += 1
    ...
    class FakeQdrant:
        ...
        def count(self, collection_name, **kwargs):
            return SimpleNamespace(count=1 if collection_name == "les_rag_sparse" else 2)

        def scroll(self, **kwargs):
            return [
                SimpleNamespace(payload={"text": "пожарная эвакуация"}),
                SimpleNamespace(payload={"text": "огнестойкость здания"}),
            ], None

        def delete(self, **kwargs):
            self.deletes.append(kwargs)

    fake_qdrant = FakeQdrant("http://qdrant")
    monkeypatch.setattr(
        "backend.qdrant_adapter.qdrant_client.QdrantClient",
        lambda *args, **kwargs: fake_qdrant,
    )
    db = FakeDB()
    adapter = SimpleNamespace(qdrant_url="http://qdrant", collection_name="les_rag", db=db)
    adapter._file_filter = QdrantLlamaIndexAdapter._file_filter.__get__(adapter)
    adapter._sparse_collection_name = QdrantLlamaIndexAdapter._sparse_collection_name.__get__(adapter)
    adapter._sync_sparse_sidecar_exists = QdrantLlamaIndexAdapter._sync_sparse_sidecar_exists.__get__(adapter)
    adapter._sync_count_file_points = QdrantLlamaIndexAdapter._sync_count_file_points.__get__(adapter)
    adapter._sync_count_sparse_file_points = QdrantLlamaIndexAdapter._sync_count_sparse_file_points.__get__(adapter)
    adapter._sync_count_expected_sparse_file_points = (
        QdrantLlamaIndexAdapter._sync_count_expected_sparse_file_points.__get__(adapter)
    )
    adapter._sync_delete_sparse_file_points = QdrantLlamaIndexAdapter._sync_delete_sparse_file_points.__get__(adapter)

    result = QdrantLlamaIndexAdapter.reconcile_dataset(adapter, "ds-1")

    assert result["mismatched"] == 0
    assert result["sparse_mismatched"] == 1
    assert result["sparse_cleaned"] == 1
    assert result["requeued"] == 1
    assert db.updated == [("ds-1", "doc.md", "PENDING", 0)]
    assert db.chunk_count_updates == 1
...
def test_reconcile_allows_dense_chunks_without_sparse_terms(monkeypatch):
    ...
    class FakeQdrant:
        ...
        def scroll(self, **kwargs):
            return [
                SimpleNamespace(payload={"text": "и в на"}),
                SimpleNamespace(payload={"text": "пожарная эвакуация"}),
            ], None
    ...
    assert result["sparse_mismatched"] == 0
    assert result["sparse_cleaned"] == 0
    assert result["requeued"] == 0
    assert db.updated == []
    assert db.chunk_count_updates == 0
