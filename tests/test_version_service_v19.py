...
def test_version_has_app_and_harness_versions():
    vi = vs.version_info()
    assert vi["product_version"] == vs.PRODUCT_VERSION and vi["build_number"] == vs.BUILD_NUMBER
...
def test_explicit_ozhr_still_available_as_tool_evidence_after_versioning():
...
def test_global_registry_requires_model_final_after_versioning():
    from proxy.services.deterministic_policy_service import can_return_deterministic_final as P
    assert P("registry", "реестр проектов лес", candidate={"operation": "registry"}) == (
        False,
