...
def test_forwarded_headers_are_only_used_from_trusted_proxy(monkeypatch):
    monkeypatch.setattr(trust, "TRUSTED_NETWORKS", ("127.0.0.0/8", "10.195.146.0/24"))
    monkeypatch.setattr(trust, "TRUSTED_PROXY_NETWORKS", ("127.0.0.0/8",))
    monkeypatch.setattr(trust, "TRUSTED_NETWORK_ROLE", "admin")

    assert trust.client_ip_from_request(_request("203.0.113.1", {"x-forwarded-for": "10.195.146.98"})) == "203.0.113.1"
    assert trust.trusted_role_for_request(_request("203.0.113.1", {"x-forwarded-for": "10.195.146.98"})) is None

    assert trust.client_ip_from_request(_request("127.0.0.1", {"x-forwarded-for": "10.195.146.98"})) == "10.195.146.98"
    assert trust.trusted_role_for_request(_request("127.0.0.1", {"x-forwarded-for": "10.195.146.98"})) == "admin"
