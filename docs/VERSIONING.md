...
|---|---:|---|
| `product_version` | `0.27.8` | Единственная версия, которую видит пользователь; обычный SemVer `X.Y.Z` |
| `build_number` | `525` | Монотонный номер Windows-сборки; не является четвёртой частью версии |
| `desktop_version` | `5.1.525` | Внутренняя версия Tauri/NSIS для обновления ранее выпущенных `5.1.x` пакетов |
