...
|---|---:|---|
| `product_version` | `0.30.48` | Единственная версия, которую видит пользователь; обычный SemVer `X.Y.Z` |
| `build_number` | `688` | Монотонный номер Windows-сборки; не является четвёртой частью версии |
| `desktop_version` | `5.1.688` | Внутренняя версия Tauri/NSIS для обновления ранее выпущенных `5.1.x` пакетов |
