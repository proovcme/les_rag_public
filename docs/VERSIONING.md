...
| `build_number` | `473` | Монотонный номер Windows-сборки; не является четвёртой частью версии |
| `desktop_version` | `5.1.473` | Внутренняя версия Tauri/NSIS для обновления ранее выпущенных `5.1.x` пакетов |
