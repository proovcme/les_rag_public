...
4. **Сборка ВОР** (`build_vor`): из площади S и этажности N выводит геометрию — S1=S/N (пятно),
...
5. **Сборка ЛСР** (`lsr_assembly.assemble`): позиции с `{code, qty}` → разворот по ГЭСН →
   цены ФГИС ЦС/КАЦ → ОЗП/ЭМ/М → стеснённость → НР/СП (по виду работ через `nr_sp_service`) →
   Всего по позиции → свод. См. [[ALGO-lsr-assembly]].
6. **Хвост прикидки объекта:** ГЭСН-конструктив (прямые + НР + СП по проверяемым позициям) →
...
7. **Defense-contract:** `estimate()` возвращает `defense.contract` (`defense_contract_v1` из
...
8. **Scope warnings:** явные части задания, которые считаются только укрупнённо, помечаются в
