...
qty_formula, ...}]}`. Сейчас **два** шаблона (по порядку матча):
...
- `monolith_office` (офисное/административное здание, монолит) — ловит «офис/административн/
  ...
  partition_k=0.80; 5 позиций (плитный фундамент 06-02-001-01, монолитные стены 06-16-004-01,
  перекрытия 06-16-005-01, газобетонные перегородки 08-04-003-02, плоская кровля 12-01-021-01).

**Незакрытое:** отделка/окна-двери/инженерка (отдельные сборники ГЭСН/ФЕР); запрос ВНЕ покрытых
шаблонов (напр. каркас/ангар) → честный отказ; расширение — добавление новых шаблонов в
