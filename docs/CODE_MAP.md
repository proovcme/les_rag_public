...
- **routers/** (~14): `chat`, `datasets` (CRUD/upload/reindex), `runtime` (health/metrics/mode), `auth`, `mail`, `speckle` (BIM-мост), `rerank`, `bor` (ВОР из спецификаций, W11), `diagnostics`, `jobs`, `logs`, `settings`, `chat_history`, `status_page`.
- **services/** (~20): `retrieval_service`, `saferag_service` (C-RAG), `semantic_cache`, `runtime_dispatcher`/`runtime_admission` (очередь к MLX), `query_router`, `table_query_service` (Parquet/Excel), `bor_service` (ВОР: Parquet→свод→xlsx, без LLM), `mail_query_service`, `clause_lookup_service`, `clarification_service`, `context_expander_service`, `rerank`, `resource_governor`, `job_service`, `cad_bim_graph` (Speckle→markdown).
