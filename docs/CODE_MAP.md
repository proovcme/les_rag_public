...
- **routers/** (~15): `chat`, `datasets` (CRUD/upload/reindex), `runtime` (health/metrics/mode), `auth`, `mail`, `speckle` (BIM-мост), `rerank`, `bor` (ВОР из спецификаций, W11), `diff` (ревизии моделей/документов, W12), `normcontrol` (формальный нормоконтроль, W13), `filemap` (карта файлового архива, W15), `tasks` (задачник, W16), `diagnostics`, `jobs`, `logs`, `settings`, `chat_history`, `status_page`.
- **services/** (~21): `retrieval_service`, `saferag_service` (C-RAG), `semantic_cache`, `runtime_dispatcher`/`runtime_admission` (очередь к MLX), `query_router`, `table_query_service` (Parquet/Excel), `bor_service` (ВОР: Parquet→свод→xlsx, без LLM), `diff_service` (дифф CAD-графов по source_id + текстов по пунктам, без LLM), `normcontrol_service` (NK-01…NK-04: форматы ГОСТ 2.301, сканы, шифры, ведомость↔состав, без LLM), `file_map_service` (краулер-карта архива: метаданные+шифры, инкрементальный rescan, выборочная индексация из карты, без LLM), `task_service` (задачник: regex-команды чата + SQL, без LLM), `memory_service` (заметки «запомни:…» + лексический recall по заметкам/истории в контекст чата, без LLM), `mail_query_service`, `clause_lookup_service`, `clarification_service`, `context_expander_service`, `rerank`, `resource_governor`, `job_service`, `cad_bim_graph` (Speckle→markdown).
