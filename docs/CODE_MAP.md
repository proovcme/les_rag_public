...
- **[ALGO-gesn.md](ALGO-gesn.md)** — норма ГЭСН → ресурсы (расход на единицу × объём); замыкает сборку ЛСР прямо от `{код, объём}`. Gold: сборка от кода эталона = 11813.04. Сервис `proxy/services/gesn_service.py`, каталог `config/domain/gesn_seed.yaml` (семя; полная база ГЭСН-2022 — отдельный импорт), `GET /api/lsr/gesn[/{code}/expand]`, MCP `les_gesn_expand`.
