...
|---|---:|---|
| `tests/test_tauri_desktop.py` (Windows startup additions) | focused | provider-neutral role catalogue, external-engine warnings that do not block core, setup-button bootstrap lock, offline tokenizer startup, isolated NSIS smoke roots and exact installer deploy stamp |
