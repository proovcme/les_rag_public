...
0.31.6: `test_chat_workspace_ui` проверяет порционное раскрытие всех114 чатов,
60 проектов, создание вне scroll-контейнера, параллельную загрузку и сохранение
DOM ids при неизменных данных. Browser probe дополнительно проверяет пригодную
высоту переписки и реальное попадание клика по полю открытого диалога.
[Проверка с61 проектом, desktop/mobile и вводом](acceptance/chat-workspace-0.31.6.md).
