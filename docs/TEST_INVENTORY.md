...
| `tests/test_activate_qdrant_generation.py` | 5 | stable Qdrant alias activation is gated by a matching green readiness report; alias contract and smeta manifest preserve projection identity, record the physical rollback generation and remain runtime-compatible |
| `tests/test_rag_readiness_service.py` | 4 | GUI readiness reports honest ready/degraded/building/awaiting-activation states from contract, dense, sparse, fingerprint and alias coverage |
