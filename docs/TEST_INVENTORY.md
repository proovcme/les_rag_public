...
|---|---:|---|
| `tests/test_installer_windows.py`, `tests/test_tauri_desktop.py`, `tests/test_install_les.py`, `tests/test_onboard_reranker.py`, `tests/test_software_versions.py`; live `tools/windows_release_smoke.ps1` | focused + Windows live | Windows/Tauri release contract: persistent state, обязательные uv/Ollama/Docker/Qdrant, version/build contract, bootstrap-status и verified BGE. Live gate запускает установленный runtime, доказывает provisioned smeta baseline (≥40 000 норм, ≥1 500 ФСЭМ), настоящий старт resumable ФСНБ-job со слоями/статусом, создание и индексацию нового датасета, точную версию, API/UI и `dense + qdrant_sparse → RRF` в одноразовой коллекции |
...
| `tests/test_qdrant_adapter_parse.py` | focused | parse/Qdrant adapter, explicit immutable model/backend embed contract, universal final token-budget and mixed-base64 sanitation gate across node types |
