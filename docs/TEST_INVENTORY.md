...
| `tests/test_rag_rrf_readiness.py` | focused | fail-closed activation gate: every dataset/destination point needs fingerprint+dense+sparse, source/child/exclusion accounting, complete FTS and identity-checked live RRF endpoint |
| `tests/test_activate_qdrant_generation.py` | focused | stable alias activation requires green structural+FTS+live-RRF report; Qdrant/SQLite alias rollback clears or restores lexical projection |
