...
Гейт: `make verify` (офлайн, синтаксис+импорт-смоук). Полная сюита: `uv run python -m pytest tests/ -q` (2760 тестов на 2026-07-10).
...
**Профильные таблицы v0.16–v0.24: 308 тестов** (+ регрессия v0.3–v0.15 и chat/router при OFF). `make verify` на h0.24 собирает **2760 тестов**.
...
| `tests/test_qdrant_adapter_parse.py` | focused | parse/Qdrant adapter plus universal final token-budget and mixed-base64 sanitation gate across node types |
| `tests/test_build_rag_contract_sibling.py` | 3 | sibling migration safety: exact dataset resolution, unknown-dataset rejection and deterministic child-specific point ids for idempotent Qwen re-embed |
...
| `tests/test_project_pdf_extract_service.py` | 24 | Л.И.С.Т. dataset summary/status: exact role/discipline boundaries, uncorroborated field rejection, attempted vs successful coverage including explicit max-files truncation, honest `ok/partial/failed/empty`, bounded warnings/refs/register metadata, row-ref priority, path-safe dataset IDs and composition/electrical isolation |
| `tests/test_project_pdf_table_service.py` | 48 | shared project table extraction/normalization and semantic navigation across ОВ/ВК/rooms/КР/ООС/ПБ/ИОС, plus noise/text/service guards and negative manual-table cases so generic line/length and quantity columns do not become cable/VOR evidence navigation |
...
| `tests/test_static_assets.py` | 7 | статическая проводка UI: вкладки «Инструменты»/«Документы», Л.И.С.Т. successful/error/missing counts and truncation notice, live dataset filter, topic/section map, CAD inventory panel + target_file deep-link, Qdrant visualizer mount, deploy shell permissions |
