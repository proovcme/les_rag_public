...
|---|---:|---|
| `tests/test_tauri_desktop.py` (Windows startup additions) | focused | provider-neutral role catalogue, external-engine warnings that do not block core, setup-button bootstrap lock, offline tokenizer startup, automatic reuse of the lock-bound release cache, isolated NSIS smoke roots and exact installer deploy stamp |
...
| `tests/test_vps_patch.py`, `tests/test_windows_application_update.py`, `tests/test_windows_update_shell.py`, `tests/test_update_service.py`, `tests/test_manual_update_ui.py` | `make test-updater` | Windows hard + soft updater: полный app-tree rename/install/rollback при отдельном persistent state, exact release identity/SHA, bounded file-backed child processes без WMI/CIM, 120/180-секундная cold-start граница, ранний process-exit против alive-timeout с redacted stderr evidence, живое чтение persistent сметного baseline через ГЭСН expand, Cargo-shell attestation без NSIS/baseline, runtime allowlist, один exact `les-desktop.exe`, soft base/target/archive SHA, атомарная замена/rollback и отдельные UI/API пути для быстрого пакета и переустановки выпуска |
| `tests/test_installer_windows.py`, `tests/test_tauri_desktop.py`, `tests/test_install_les.py`, `tests/test_software_versions.py`, `tests/test_patch_release.py`; `.github/workflows/{verify,release}.yml`; live `tools/windows_{release_smoke,production_deploy}.ps1` | portable + Windows live | проверяет SHA-256 portable Python/uv, exact `uv.lock`, построение/проверку и автоматическое переиспользование offline Windows cache, быструю распаковку bundled Python, уникальные smoke roots, locked/offline bootstrap без system/network fallback, provider-neutral setup и immutable baseline; exact installed smoke подтверждает version, API/UI, process/index contract и не трогает пользовательский RAG |
