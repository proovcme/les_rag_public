...
Гейт: `make verify` (офлайн, синтаксис+импорт-смоук). Полная сюита: `uv run python -m pytest tests/ -q` (2768 тестов на 2026-07-10).
...
**Профильные таблицы v0.16–v0.24: 308 тестов** (+ регрессия v0.3–v0.15 и chat/router при OFF). `make verify` на h0.24 собирает **2768 тестов**.
...
| `tests/test_build_rag_contract_sibling.py` | focused | contract-clean migration safety: selected and all-indexed dataset resolution, unknown-dataset rejection, deterministic child-specific ids and resumable Qwen re-embed contract |
| `tests/test_rag_rrf_readiness.py` | 2 | fail-closed activation gate: every registered dataset and every destination point require compatible fingerprint plus named dense and sparse vectors; missing sparse blocks readiness |
