...
| `tests/test_vps_patch.py`, `tests/test_windows_application_update.py`, `tests/test_windows_update_shell.py`, `tests/test_update_service.py`, `tests/test_manual_update_ui.py` | `make test-updater` | Windows hard + soft updater: полный app-tree rename/install/rollback при отдельном persistent state, exact release identity/SHA, bounded file-backed child processes без WMI/CIM, ранний process-exit против alive-timeout с redacted stderr evidence, Cargo-shell attestation без NSIS/baseline, runtime allowlist, один exact `les-desktop.exe`, soft base/target/archive SHA, атомарная замена/rollback и отдельные UI/API пути для быстрого пакета и переустановки выпуска |
...
| `tests/test_smeta_model_quality_benchmark.py` | 3 | offline-контракт честного Qwen/Gemma A/B: явные model profiles, одинаковый изолированный Ollama env с восстановлением, calculated/partial/missing, tool calls/repeats, route/unit/volume/provenance integrity и qrels-only профессиональная оценка |
