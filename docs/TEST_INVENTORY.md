...
| `tests/test_live_workbook_acceptance_contract.py`, `tests/test_candidate_acceptance_service.py`, `tools/live_workbook_acceptance.py` | offline contract + opt-in live runner | Offline transport exercise is explicitly nonpersistent `contract_test`, so it cannot create `live_runtime` evidence. It proves fail-closed exact typed/redacted receipt validation: two immutable revisions, parent/attachment/checkpoint/provenance/hash/model identity, `/api/version` commit+positive build, monotonic complete SSE progress, readable meaningful XLSX, counts-only missing/blockers and deadline. Fixture paths, unprivileged/non-isolated candidate uploads before persistence, unknown/auth/prompt/path/traceback data are rejected. Only the real entrypoint constructs its HTTP client and can write `live_runtime`; `make live-workbook-acceptance LIVE_WORKBOOK_ACCEPTANCE_ARGS='…'` remains a separate owner-authorized real-input gate, not fixture/model-quality evidence. |
