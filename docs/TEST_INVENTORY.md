...
| `tests/test_live_workbook_acceptance_contract.py`, `tests/test_candidate_acceptance_service.py`, `tools/live_workbook_acceptance.py` | 15 + opt-in live runner | Offline contract proves a fail-closed redacted structured `live_runtime` receipt: two immutable revisions with exact parent/attachment/checkpoint/provenance/hash/model identity, missing/blocker counts and elapsed time; fixture paths and unredacted auth/prompt/path/traceback data are rejected. The runner carries `candidate_acceptance` only to an isolated process-CWD with the read-only Danger bootstrap factor and root-admin boundary; no promotion receipt is synthesized. `make live-workbook-acceptance LIVE_WORKBOOK_ACCEPTANCE_ARGS='…'` is a separate owner-authorized real-input gate, not fixture/model-quality evidence. |
