...
| `tests/test_outlook_mail_poller.py`; Windows live gate | static + Windows live | Classic Outlook sidecar recursively visits stores/folders, excludes Deleted/Drafts/Junk by identifiers, saves Unicode MSG, persists resumable cursor only after intake, opens exact original, and is installed as a three-minute interactive task. Static checks do not replace COM probe, idempotent repeat, INDEXED and original-opening smoke on Legion |
...
| `tests/test_answer_render_v16.py` | 34 | render-хелперы Совушки: strip markdown из ячеек, source-chips, evidence-секции, citation/conflict-блоки, citation drawer payload, PDF page/bbox (fail-soft malformed bbox) и DOCX/XLSX locator deep-links во встроенный viewer, compact trace включая topic-guided retrieval, `answer_copy_text` (Копировать без trace/тела письма) |
...
| `tests/test_list_office_agent_service.py` | 5 | `office_document_ir_v1`: только выбранные server-resolved документы, bounded evidence, schema-known manual fields, invalid evidence→visible assumption, model failure without prose fallback, explicit review gate and preserved IR in immutable manifest |
...
| `tests/test_static_assets.py` | 8 | статическая проводка UI: вкладки «Инструменты»/«Документы», visual CDE contract, Л.И.С.Т. coverage/truncation, Студия офисных документов, live dataset filter, topic/section map, CAD inventory panel + target_file deep-link, Qdrant visualizer mount, deploy shell permissions |
