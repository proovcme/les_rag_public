...
`normalized_remarks` JSON/XLSX**, **0.24.0.1 добавил persist-решения инженера
`confirmed|rejected|needs_more_evidence`** как общий выход для следующих слоёв. **Phase 6** (ПП-87
...
GET  /api/doc-review/{dataset_id}/decisions
POST /api/doc-review/{dataset_id}/decision
...
Начиная с 0.24.0.1 решение инженера — системный sidecar, а не текст в чате. API
`POST /api/doc-review/{dataset_id}/decision` сохраняет `rule_id`, `decision` и комментарий;
`run/download` применяют эти решения к `items`, `normalized_remarks`, `defense.summary`,
HTML и XLSX. Допустимые решения: `confirmed`, `rejected`, `needs_more_evidence`, `unset`.
Интерфейс «Инструменты → Проверка документации» даёт кнопки подтвердить/отклонить/нужны данные.
...
чёрного ящика: блок есть в «Инструментах», а в чате — отдельная кнопка «Служебные источники».
