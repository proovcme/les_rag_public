...
номер сборки:              409     (отдельно от версии продукта)
версия Tauri/NSIS:         5.1.409 (внутренняя совместимость обновления)
...
следующий выпуск:          0.24.1 / build 409 после полного Windows gate
рантайм /api/version:      проверяется отдельно; dev-правка не считается развёртыванием
```

> 0.24.1 / build 409 — единый контракт версии, закреплёный программный контур и автоматический patch-release
>
> Дата: 2026-07-14
