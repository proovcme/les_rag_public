...
```
версия продукта (SemVer):  0.24.22 (dev hotfix, 2026-07-16)
номер сборки:              443     (отдельно от версии продукта)
версия Tauri/NSIS:         5.1.443 (контракт следующей Windows-сборки)
...
следующий выпуск:          0.24.22 / build 443
...
> теперь открывается read-only/immutable, а submit-точный шифр дочитывается без кодового перевыбора.
