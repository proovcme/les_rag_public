...
```
версия (схема 0.N.FEATURE.PATCH): 0.24.0.203  (в КОДЕ: LES_VERSION; в /api/version поле les_version)
ветка:                     feat/les3-p1
dev HEAD:                  HEAD  (см. git log -1)
задеплоено на рантайм:     0.24.0.203 LSR form/export fixes after СКС/БАП 3×3 probe
НЕ задеплоено:             —
рантайм /api/version:      0.24.0.203 (deploy stamp после make ship, 2026-07-03; commit stamp до следующего git commit)
```
