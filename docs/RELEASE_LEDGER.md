...
номер сборки:              433     (отдельно от версии продукта)
версия Tauri/NSIS:         5.1.433 (внутренняя совместимость обновления)
...
следующий выпуск:          0.24.17 / build 433
рантайм /api/version:      проверяется отдельно; dev-правка не считается развёртыванием
```

> 0.24.17 / build 433 — целостность датасетов, нормальный RAG-агент и компактный чат
>
> Дата: 2026-07-15
