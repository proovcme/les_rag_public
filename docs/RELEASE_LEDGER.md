...
```
версия продукта (SemVer):  0.24.17 (release candidate: RAG/dataset/smeta repair, 2026-07-15)
номер сборки:              429     (отдельно от версии продукта)
версия Tauri/NSIS:         5.1.429 (внутренняя совместимость обновления)
ветка выпуска:             main (сведение веток выполняется перед публикацией)
dev HEAD:                  HEAD  (см. git log -1)
задеплоено на рантайм:     0.24.16 / build 427 (live on Legion, 2026-07-15)
Windows-выпуск:            0.24.16 / build 427 (VPS patch baseline)
следующий выпуск:          0.24.17 / build 429
рантайм /api/version:      проверяется отдельно; dev-правка не считается развёртыванием
```

> 0.24.17 / build 429 — целостность датасетов, нормальный RAG-агент и компактный чат
>
> Дата: 2026-07-15
> Статус: release candidate; focused `196 passed`, `make verify` — `3005 collected`; публикация
> VPS-feed разрешена только после полного Gemma→XLSX, NS integrity и UI/API smoke на Legion.
...
> выявил scalar `norm_code` в `read_norms_batch`; build 429 принимает его как одноэлементный список,
> не меняя выбранный моделью код и не добавляя доменную логику.
