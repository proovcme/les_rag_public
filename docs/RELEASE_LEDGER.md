...
номер сборки:              428     (отдельно от версии продукта)
версия Tauri/NSIS:         5.1.428 (внутренняя совместимость обновления)
...
следующий выпуск:          0.24.17 / build 428
рантайм /api/version:      проверяется отдельно; dev-правка не считается развёртыванием
```

> 0.24.17 / build 428 — целостность датасетов, нормальный RAG-агент и компактный чат
>
> Дата: 2026-07-15
...
> ancestry и helper доказывает новый API/UI отдельным процессом перед успехом.
