...
```
версия (схема 0.N.FEATURE.PATCH): 0.24.0.197  (в КОДЕ: LES_VERSION; в /api/version поле les_version)
ветка:                     feat/les3-p1
dev HEAD:                  HEAD  (см. git log -1)
задеплоено на рантайм:     0.24.0.197 smeta norm route navigation + LSR RIM artifact + active estimate preservation
НЕ задеплоено:             —
рантайм /api/version:      0.24.0.197 (deploy stamp e17848c1, 2026-07-03)
```
