...
```
версия (схема 0.N.FEATURE.PATCH): 0.24.0.204  (в КОДЕ: LES_VERSION; в /api/version поле les_version)
ветка:                     feat/les3-p1
dev HEAD:                  HEAD  (см. git log -1)
задеплоено на рантайм:     0.24.0.204 RIM trace-first LSR artifact + smeta RAG map
НЕ задеплоено:             —
рантайм /api/version:      0.24.0.204 (deploy stamp после make ship, 2026-07-03; commit stamp до следующего git commit)
```
