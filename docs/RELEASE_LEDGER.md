...
```
версия продукта (SemVer):  0.24.23 (dev hotfix, 2026-07-16)
номер сборки:              444     (отдельно от версии продукта)
версия Tauri/NSIS:         5.1.444 (контракт следующей Windows-сборки)
...
следующий выпуск:          0.24.23 / build 444
...
> `bind` требует полный technology/overlap evidence, `unbound` — две фактически выполненные
> формулировки, открытые карточки, причины неприменимости и coverage-check. Trace и unit gate
> проверяются до расчёта, ошибочные строки возвращаются той же модели без регенерации уже принятых.
> Локальный transport по умолчанию обрабатывает пакеты по 5 строк с общим контекстом; облачная модель
> сохраняет один разговор. Код не выбирает норму: после доказанного mapping он только считает и
> формирует XLSX.
