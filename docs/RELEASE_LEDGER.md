...
```
версия (схема 0.N.FEATURE.PATCH): 0.24.0.366  (в КОДЕ: LES_VERSION; в /api/version поле les_version)
ветка:                     feat/les3-p1
dev HEAD:                  HEAD  (см. git log -1)
задеплоено на рантайм:     0.24.0.366 UI-only поверх runtime-базы 0.24.0.357
НЕ задеплоено:             RRF contract-v2 0.24.0.360, smeta-изменения 0.24.0.358 и прочий dev divergence
рантайм /api/version:      0.24.0.366, stamp ok, public/local подтверждены
```
