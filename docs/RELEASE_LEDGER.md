...
### Кандидат0.31.6 / build712 — исправления чата

Диалоги принимают мышь; создание проекта над прокруткой; компактный чат,
20 начальных строк вместо полной истории, обновление без пересоздания
неизменных списков. [Приёмка](acceptance/chat-workspace-0.31.6.md).
Пока проверен изолированный UI; установленная публичная версия указана ниже.
...
> и сметную baseline; FGIS operator smoke выявил отсутствующий динамический
> `python -m tools.fgis_update_supervisor` в Windows runtime manifest. Supervisor
> включён явно и удерживается regression. Production Legion и public release
> отказом не менялись.
...
> классификация 68 manifest-файлов занимает около 2,3 с вместо нескольких
> минут. Оркестратор печатает ограниченный живой прогресс. После stage
> `accepted` он сам проверяет, что публичный `main` является предком принятого
...
> Construction Harness ради одного `classify_doc_type`: они используют уже
> существующий самостоятельный `doc_type_classifier`. Новый behavior-test
> воспроизводит отсутствие старого harness и подтверждает file-body retrieval с
> doc-type filter. Старый harness пока не удалён: сначала полезные adapter/extract/
> resource tests отделяются от его поколенческих сценариев. Решение владельца
> также фиксирует удаление ненужных `qwen-agent` и `google-adk` в отдельном полном
> Windows-выпуске; текущий patch-кандидат зависимости не меняет. Сметное ядро,
> RAG-алгоритм, Qdrant, пользовательские данные и установленный Legion не менялись.
> Generated map: 979 tracked Python-файлов / 299480 строк, 350 product-reachable,
> 8 runtime-support, 620 test/tool-only, 1 dormant, 394 API routes, 0 warnings.
...
> `proxy/routers/worklog.py`, изолированный service и его service-only tests.
> Consumer-аудит подтвердил отсутствие UI, model tool, project dossier и
> межсервисных вызовов; исторический план оставлял реальный шаблон, раздел 6 и GUI
> незавершёнными. `les_work_log_meta`, пользовательские БД, field journal,
> сформированные файлы и знание термина ОЖР не удалялись. Активные проекты, сметы,
> RAG, Qdrant и установленный Legion не менялись. Generated map: 983 tracked
> Python-файла, 357 product-reachable, 8 runtime-support, 617 test/tool-only,
> 1 dormant, 399 API routes, 0 warnings.
...
> Единый orchestrator выполняет clean/pushed prepare-gates, автоматическую
> soft/full классификацию, immutable candidate build и локальную/remote Legion
> acceptance без пересборки. Оба publisher теперь fail-closed требуют accepted
...
> Build 637 убрал управление Docker/Qdrant, сделал внешний Qdrant честным
> `available`/`N/A`, отличает version-only `uv.lock` от изменения зависимостей и
> self-hosted обновляет builder/helper. Живой повтор выявил зависший proxy с exact
...
> artifact metadata/download routers с fixture profile snapshot,
> empty retrieval/history ports и нижними model transport, tool shortlist и
> workbook executor.
> **PENDING:** это не model-quality/live-runtime acceptance.

> **Build 624 ASGI application composition:** test-only review-fix отдельно от
> Build 623. Hermetic contract больше не подменяет `_run_chat_with_provider`
> handcrafted final payload: real `_run_chat_with_provider → _run_chat →
> evidence application → harvest` строит SSE final с artifact,
> attachment-retry, checkpoint и source. Fixture задаёт isolated profile
> snapshot, empty retrieval/history ports и нижние model transport, tool
