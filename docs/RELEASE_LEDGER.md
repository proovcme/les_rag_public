...
номер сборки:              431     (отдельно от версии продукта)
версия Tauri/NSIS:         5.1.431 (внутренняя совместимость обновления)
...
следующий выпуск:          0.24.17 / build 431
рантайм /api/version:      проверяется отдельно; dev-правка не считается развёртыванием
```

> 0.24.17 / build 431 — целостность датасетов, нормальный RAG-агент и компактный чат
>
> Дата: 2026-07-15
