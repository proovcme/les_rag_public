...
```
версия (схема 0.N.FEATURE.PATCH): 0.24.0.116  (в КОДЕ: LES_VERSION; в /api/version поле les_version)
ветка:                     feat/les3-p1
dev HEAD:                  HEAD  (см. git log -1)
задеплоено на рантайм:     0.24.0.116 RAG construction engineer prompt pass
НЕ задеплоено:             —
рантайм /api/version:      0.24.0.116 · app 5.1.0 · h0.24 · runtime_alignment=aligned
```
