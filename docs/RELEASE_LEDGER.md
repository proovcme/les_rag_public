...
номер сборки:              437     (отдельно от версии продукта)
версия Tauri/NSIS:         5.1.437 (внутренняя совместимость обновления)
...
следующий выпуск:          0.24.17 / build 437
рантайм /api/version:      проверяется отдельно; dev-правка не считается развёртыванием
```

> 0.24.17 / build 437 — целостность датасетов, нормальный RAG-агент и компактный чат
>
> Дата: 2026-07-15
> Статус: release candidate; focused `196 passed`, `make verify` — `3008 collected`; публикация
> VPS-feed разрешена только после полного model-agent→XLSX, NS integrity и UI/API smoke на Legion.
