...
```
версия продукта (SemVer):  0.24.1  (config/version.json; ещё не опубликована)
номер сборки:              408     (отдельно от версии продукта)
версия Tauri/NSIS:         5.1.408 (внутренняя совместимость обновления)
...
следующий выпуск:          0.24.1 / build 408 после полного Windows gate
рантайм /api/version:      проверяется отдельно; dev-правка не считается развёртыванием
```

> 0.24.1 / build 408 — единый контракт версии, закреплёный программный контур и автоматический patch-release
>
> Дата: 2026-07-14
> Статус: подготовка выпуска, публикация ещё не выполнена. `config/version.json` стал единственным
...
> в общем Qdrant коллекции. Build 408 вместо этого создаёт через API временный датасет, ждёт его
> собственный dense+sparse-индекс, выполняет scoped RRF и удаляет только контрольные данные.
> После regression-тестов этих случаев коллекция содержит `2929` тестов.
