...
## Контракт 0.30.1: модель владеет исследованием и ответом

- `none` — разговор без документального RAG; выбранные датасеты и вложения —
  замкнутый scope; `all` работает только после явного выбора пользователя.
- До первого model call выполняется native RRF и передаётся начальный evidence
  packet. Следующие поисковые формулировки выбирает сама модель, а
  `search_sources` повторно использует тот же production retrieval.
- Semantic answer cache для grounded chat не используется. Нулевой retrieval не
  превращается кодом в `NO_DATA`: модель видит пустой packet и сама сообщает,
  чего не нашла или какой поиск ей нужен.
- Финальный ответ модели не проходит семантический validator/rewrite. Код
  проверяет транспорт и фиксирует trace, но не решает, верен ли инженерный вывод.
- Оператор видит не только названия источников, но точные model-visible
  `EVIDENCE`/`SOURCE_MAP`, их SHA-256 и omissions. Acceptance-вопрос
  `Расскажи про датасет.` сохраняет ответ, sources, packet и model-call traces,
  не проверяя ожидаемые слова и не выставляя модели профессиональную оценку.
...
Размер возвращаемого top-k сам по себе не доказывает no-op: cross-encoder должен переупорядочить
bounded shortlist, который native RRF отобрал из широкого recall-пула. Дорогому CPU-реранкеру нельзя
передавать весь технический пул в 256+ фрагментов: его `candidate_limit` является платформенной
runtime-политикой, а непрошедший shortlist остаётся RRF-хвостом и не исчезает. Контракт проверяет
фактическое изменение головы, `pool_count`, `candidate_limit`, `input_count`, `returned_count`,
исходный и итоговый rank. Недоступный или упавший обязательный reranker блокирует production
RAG-ответ, а не оставляет пользователю непроверенный RRF-порядок.
