...
`MailItem`. Независимый watchdog завершает весь sidecar через 15 секунд даже при зависшем COM-вызове;
...
Loopback intake сначала атомарно сохраняет raw message и отдельный spool-manifest, после чего сразу
отвечает sidecar. Exact registry, извлечение полей `.msg`, загрузка в dataset и parser выполняются
после освобождения Outlook, в отдельной последовательной очереди на ящик. Parser запускается после
опустошения очереди, поэтому не конкурирует с очередной регистрацией за файлы/SQLite. Manifest
удаляется только после успешной загрузки; после перезапуска proxy оставшийся spool автоматически
возобновляется. Ошибка фоновой регистрации видна как `index_status=error` и не откатывает сохранённый
evidence-снимок.
