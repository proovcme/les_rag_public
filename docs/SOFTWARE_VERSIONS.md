...
|---|---:|---|
| Версия продукта | `0.24.3` | Единственный пользовательский номер по схеме `X.Y.Z` |
| Номер сборки | `414` | Монотонный номер Windows-пакета; не является четвёртой частью версии |
| Версия пакета Tauri/NSIS | `5.1.414` | Внутренняя монотонная версия установщика для обновления существующих `5.1.x` |
| Схема строительного контура | `0.24` | Внутренняя версия контракта; в пользовательский номер не входит |

Текущий Windows release candidate получает `0.24.3` и сборку `414`. Новая совместимая возможность
получит `0.25.0`. Готовый стабильный продукт — `1.0.0`.
