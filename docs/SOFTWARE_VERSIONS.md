...
|---|---:|---|
| Версия продукта | `0.28.1` | Единственный пользовательский номер по схеме `X.Y.Z` |
| Номер сборки | `588` | Монотонный номер Windows-пакета; не является четвёртой частью версии |
| Версия пакета Tauri/NSIS | `5.1.588` | Внутренняя монотонная версия установщика для обновления существующих `5.1.x` |
| Схема строительного контура | `0.24` | Внутренняя версия контракта; в пользовательский номер не входит |

`0.28.1` — кандидат публичного Windows-релиза с автономным offline runtime и provider-neutral
setup surface. Статус фактической установки и публикации фиксируется только в
