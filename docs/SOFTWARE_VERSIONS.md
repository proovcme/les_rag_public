...
|---|---:|---|
| Версия продукта | `0.25.10` | Единственный пользовательский номер по схеме `X.Y.Z` |
| Номер сборки | `483` | Монотонный номер Windows-пакета; не является четвёртой частью версии |
| Версия пакета Tauri/NSIS | `5.1.483` | Внутренняя монотонная версия установщика для обновления существующих `5.1.x` |
