...
|---|---:|---|
| Версия продукта | `0.28.3` | Единственный пользовательский номер по схеме `X.Y.Z` |
...
`0.28.3` — candidate поверх публичного Windows-релиза `0.28.1`: файловые
инструменты ЛСР/ВОР в профиле «Сметчик»; живой SSE прогресс ЛСР; tool-loop не падает
на `dataset_ids is None`. Публичный `0.28.2` зарезервирован под hotfix установщика.
Статус фактической установки фиксируется в `docs/RELEASE_LEDGER.md`.
Новая несовместимая возможность получает следующий minor SemVer;
