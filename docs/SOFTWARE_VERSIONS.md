...
|---|---:|---|
| Версия продукта | `0.26.1` | Единственный пользовательский номер по схеме `X.Y.Z` |
| Номер сборки | `509` | Монотонный номер Windows-пакета; не является четвёртой частью версии |
| Версия пакета Tauri/NSIS | `5.1.509` | Внутренняя монотонная версия установщика для обновления существующих `5.1.x` |
