...
|---|---:|---|
| Версия продукта | `0.30.0` | Единственный пользовательский номер по схеме `X.Y.Z` |
| Номер сборки | `634` | Монотонный номер Windows-пакета; не является четвёртой частью версии |
| Версия пакета Tauri/NSIS | `5.1.634` | Внутренняя монотонная версия установщика для обновления существующих `5.1.x` |
