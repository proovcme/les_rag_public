...
|---|---:|---|
| Версия продукта | `0.24.48` | Единственный пользовательский номер по схеме `X.Y.Z` |
| Номер сборки | `469` | Монотонный номер Windows-пакета; не является четвёртой частью версии |
| Версия пакета Tauri/NSIS | `5.1.469` | Внутренняя монотонная версия установщика для обновления существующих `5.1.x` |
...
2026-07-23). `0.24.48` / `469` — текущая dev-сборка (chat persist + smeta
self-check поверх public-review); она не задеплоена и не является
