...
|---|---:|---|
| Версия продукта | `0.27.84` | Единственный пользовательский номер по схеме `X.Y.Z` |
...
| Реранжировщик | `BAAI/bge-reranker-v2-m3` + CUDA `torch` (`cu124` wheel; не `+cpu`) | отдельный проверенный `sentence-transformers` вес на GPU | Общий реранк после RRF; `RERANK_DEVICE=cuda` с CPU-only torch падает на cpu CE |
