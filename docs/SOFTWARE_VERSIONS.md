...
|---|---:|---|
| Версия продукта | `0.24.45` | Единственный пользовательский номер по схеме `X.Y.Z` |
| Номер сборки | `466` | Монотонный номер Windows-пакета; не является четвёртой частью версии |
| Версия пакета Tauri/NSIS | `5.1.466` | Внутренняя монотонная версия установщика для обновления существующих `5.1.x` |
| Схема строительного контура | `0.24` | Внутренняя версия контракта; в пользовательский номер не входит |

Public demo сейчас исполняет `0.24.44` / `465` (проверено через `/api/version`
2026-07-23). `0.24.45` / `466` — версия stacked public-review с полным safe
tracked source и документационным gate; она не задеплоена и не является
Windows-выпуском. Последний подтверждённый Windows/Legion release остаётся
`0.24.26` / `447`. Новая несовместимая возможность получит следующий minor
SemVer. Готовый стабильный продукт — `1.0.0`.
