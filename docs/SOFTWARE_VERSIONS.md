...
|---|---:|---|
| Версия продукта | `0.27.27` | Единственный пользовательский номер по схеме `X.Y.Z` |
| Номер сборки | `544` | Монотонный номер Windows-пакета; не является четвёртой частью версии |
| Версия пакета Tauri/NSIS | `5.1.544` | Внутренняя монотонная версия установщика для обновления существующих `5.1.x` |
