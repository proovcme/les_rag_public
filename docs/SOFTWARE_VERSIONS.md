...
|---|---:|---|
| Версия продукта | `0.24.18` | Единственный пользовательский номер по схеме `X.Y.Z` |
| Номер сборки | `439` | Монотонный номер Windows-пакета; не является четвёртой частью версии |
| Версия пакета Tauri/NSIS | `5.1.439` | Внутренняя монотонная версия установщика для обновления существующих `5.1.x` |
...
установка и живая приёмка на Legion остаются оператору. Новая несовместимая возможность получит
следующий minor SemVer. Готовый стабильный продукт — `1.0.0`.
