...
|---|---:|---|
| Версия продукта | `0.24.16` | Единственный пользовательский номер по схеме `X.Y.Z` |
| Номер сборки | `427` | Монотонный номер Windows-пакета; не является четвёртой частью версии |
| Версия пакета Tauri/NSIS | `5.1.427` | Внутренняя монотонная версия установщика для обновления существующих `5.1.x` |
| Схема строительного контура | `0.24` | Внутренняя версия контракта; в пользовательский номер не входит |

Текущий Windows release candidate получает `0.24.12` и сборку `423`; установленный production до
завершения Legion gate остаётся `0.24.8` / `419`. Новая несовместимая возможность получит следующий
minor SemVer. Готовый стабильный продукт — `1.0.0`.
