...
1. `AGENTS.md`
2. Каноническая цепочка из `AGENTS.md`: `SKILL` → `MODULE_INDEX` → `CODE_MAP` → версии/roadmap/ledger/tests
3. Док конкретного модуля или действующий `ALGO-*`, найденный через `MODULE_INDEX`
4. узкий поиск (`rg` по символу, route или точке входа)
5. и только потом — открывать исходники точечно

`docs/archive/`, старые `PLAN/TODO/AUDIT`, статьи и release notes читаются только для ответа
«почему так сложилось». Они не определяют текущую архитектуру или следующую задачу.
