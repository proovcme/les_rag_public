...
tool payload); непрочитанная карточка, несовместимая единица и неполные доказательные поля сохраняются
в модельной ревизии как построчные расчётные blockers. Python не содержит второго профессионального
prompt: полный `skills/smeta/SKILL.md` загружается как единственный контракт модели.
...
Все три tools доступны модели на каждом ходу. Python не скрывает `submit_lsr_mapping` после поиска,
не назначает следующий tool и не повторяет no-tool ответ с кодовым принуждением. Если модель завершила
ход обычным текстом, workflow честно останавливается с этим модельным текстом вместо скрытого retry.
...
Передаёт завершённую модельную ревизию по исходным строкам; локальная модель может отправлять её
инкрементально, по одной или нескольким строкам:
...
JSON Schema submit-tool повторяет форму этих полей как подсказку нативному tool-call декодеру, но
не является профессиональным validator: пропущенные поля не вызывают retry и не меняют mapping.
