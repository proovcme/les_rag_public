...
> **Статус 2026-07-15: ✅ код и документ синхронизированы.** Канонический PDF→ЛСР путь — один
...
- `proxy.services.smeta_chat_adapter_service` — smeta transport/RAG adapters, prompts и parsers:
  ...
  Любая выбранная модель по умолчанию получает всю ВОР одним модельным разговором; искусственного построчного выбора
  и скрытого сокращения исходника нет. Короткие search/read tool-вызовы имеют отдельный transport-
  бюджет, а полный бюджет сохраняется для итогового `submit_lsr_mapping`; это ограничивает служебную
  JSON-генерацию, не число строк и не профессиональное решение. Операторский
