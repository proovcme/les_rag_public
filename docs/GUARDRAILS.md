...
> `make ship*` ниже относится к внутреннему dev/runtime-деплою и не является
> публичным выпуском. Публичный выпуск имеет одну границу:
> `make release RELEASE_ARGS='run --host legion --publish'`; канон —
...
Нулевой вопрос перед всеми остальными гейтами: **«Не мешаем ли мы модели?»**
На одинаковой модели, задаче, вложении и dataset прямой baseline `модель → её
запросы → RAG → модель` сравнивается со штатным harness. Повтор одинакового
отклонённого вызова, ложный `NO_DATA`, потеря видимого вложения/evidence или
ухудшение результата относительно baseline блокируют выпуск независимо от
числа зелёных mock/unit-тестов.
