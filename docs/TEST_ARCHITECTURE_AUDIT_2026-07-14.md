...
Дата среза: 2026-07-14. Версия дерева: подготовка `0.24.1`, сборка `408`.

## Вердикт

В исходном срезе собирались **2926 тестов из 303 файлов**. После release-bootstrap и
version-contract regression текущая коллекция содержит **2929 тестов**. Число большое, но само по себе не является
показателем качества. По отношению к канонической архитектуре результат такой:

| Класс | Тестов | Что это значит |
|---|---:|---|
| Текущие контуры и действующие модули | 2496 | Проверяют код, который остаётся частью текущей архитектуры: RAG/evidence, ingestion/Л.И.С.Т., smeta, API/UI/runtime/release и действующие доменные модули |
...
| **Всего** | **2929** | Полная регрессионная коллекция, а не 2929 одинаково полезных release-гейтов |
...
**2592 теста** (`2929 - 288 - 49`). Из них не все являются release-критичными: значительная часть
...
| Платформа, API, GUI и выпуск | 56 | 520 | Критично для поставки. Проверяет Windows/Tauri, bootstrap, updater, runtime, безопасность, вложения, idempotency, версии и Совушку |
...
Четыре первые группы дают **1509** проверок основных продуктовых границ. Ещё **1132** проверки
...
229.33 s**; текущий собираемый размер профиля — **2641**. Он пока включает
49 старых повторов из смешанных файлов, потому что их нельзя исключить целым файлом без потери
96 полезных проверок. После переноса полезных тестов ожидаемый чистый размер — 2592.
