...
2. `make test-architecture` — текущая архитектура LES без 11 исторических файлов и без
   `test_artel*`: отдельный продукт ARTEL больше не маскируется под LES release-gate.
...
5. `make test` — вся коллекция, включая историю; полезна во время миграции, но её число нельзя
