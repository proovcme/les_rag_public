...
двух regression-тестов release-bootstrap текущая коллекция содержит **2928 тестов**. Число большое, но само по себе не является
показателем качества. По отношению к канонической архитектуре результат такой:

| Класс | Тестов | Что это значит |
|---|---:|---|
| Текущие контуры и действующие модули | 2495 | Проверяют код, который остаётся частью текущей архитектуры: RAG/evidence, ingestion/Л.И.С.Т., smeta, API/UI/runtime/release и действующие доменные модули |
...
| **Всего** | **2928** | Полная регрессионная коллекция, а не 2928 одинаково полезных release-гейтов |
...
**2591 тест** (`2928 - 288 - 49`). Из них не все являются release-критичными: значительная часть
...
| Платформа, API, GUI и выпуск | 56 | 519 | Критично для поставки. Проверяет Windows/Tauri, bootstrap, updater, runtime, безопасность, вложения, idempotency, версии и Совушку |
...
Четыре первые группы дают **1508** проверок основных продуктовых границ. Ещё **1132** проверки
...
229.33 s**; текущий собираемый размер профиля — **2640**. Он пока включает
49 старых повторов из смешанных файлов, потому что их нельзя исключить целым файлом без потери
96 полезных проверок. После переноса полезных тестов ожидаемый чистый размер — 2591.
