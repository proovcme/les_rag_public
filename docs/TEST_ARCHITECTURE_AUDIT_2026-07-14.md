...
В репозитории собираются **2926 тестов из 303 файлов**. Число большое, но само по себе не является
показателем качества. По отношению к канонической архитектуре результат такой:

| Класс | Тестов | Что это значит |
|---|---:|---|
| Текущие контуры и действующие модули | 2493 | Проверяют код, который остаётся частью текущей архитектуры: RAG/evidence, ingestion/Л.И.С.Т., smeta, API/UI/runtime/release и действующие доменные модули |
...
| **Всего** | **2926** | Полная регрессионная коллекция, а не 2926 одинаково полезных release-гейтов |
...
**2589 тестов** (`2926 - 288 - 49`). Из них не все являются release-критичными: значительная часть
...
| Платформа, API, GUI и выпуск | 56 | 517 | Критично для поставки. Проверяет Windows/Tauri, bootstrap, updater, runtime, безопасность, вложения, idempotency, версии и Совушку |
...
Четыре первые группы дают **1506** проверок основных продуктовых границ. Ещё **1132** проверки
...
Контрольный запуск нового профиля: **2638 passed, 6 warnings in 229.33 s**. Он пока включает
49 старых повторов из смешанных файлов, потому что их нельзя исключить целым файлом без потери
96 полезных проверок. После переноса полезных тестов ожидаемый чистый размер — 2589.
