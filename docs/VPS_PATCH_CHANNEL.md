...
При локальной release-acceptance builder также читает exact установленный runtime
и добавляет его raw SHA только если CRLF→LF-нормализация даёт одно из этих
доверенных commit-состояний. Это покрывает исторические mixed-EOL файлы, не
разрешая произвольный локальный drift.
...
Базовый SHA текстового файла учитывает фактический Windows runtime. Payload
хранится как канонический текст; клиент и helper для разрешённых текстовых
runtime-файлов дополнительно сравнивают SHA после единственной канонизации
CRLF→LF. Бинарные файлы и прочие изменения сравниваются только побайтово. Это
различие обязательно проверяется на установленном runtime.
