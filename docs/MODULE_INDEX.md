...
| smeta/gesn | норма ГЭСН → ресурсы (расход×объём); canonical typed SQLite, raw/unified только source/staging; builder проверяет floor и полное provenance до atomic replace, не повреждая предыдущую базу. Windows clean install получает immutable source/base/FSEM payload с SHA/integrity и не перезаписывает существующий state | `gesn_service`, `gesn_update_service`, `tools/{gesn_update_from_fgis,gesn_unify_base,build_smeta_structured_base,smeta_release_baseline}.py`; `config/domain/smeta_base_active.json`; Make `smeta-base*`; Windows bootstrap/release smoke | [ALGO-gesn.md](ALGO-gesn.md) · [modules/smeta-core.md](modules/smeta-core.md) | ✅ |
...
| rag/answer-render | видимый слой evidence в Совушке: conversation-first чат с progressive disclosure, копирование с источниками и artifact `Источники ответа`; перечень источников закрыт по умолчанию и при раскрытии остаётся в bounded scroll, основной клик открывает поддерживаемый файл во встроенном viewer, отдельная info-кнопка показывает ту же citation drawer со сниппетом, `source_ref`, оригиналом и viewer; композер по умолчанию однострочный, режимы и действия занимают один компактный уровень, а подсказки по активному режиму всплывают компактно над вводом; «Остановить диалог» отменяет только текущий streaming-task с честным статусом, SSE не прижимает читателя вниз после его ручной прокрутки; технические статусы и вторичные действия скрыты из первого слоя; не меняет retrieval/model answer | `sovushka/answer_render.py`, `sovushka/pages/chat.py`, `sovushka/styles.py`; tests `test_answer_render_v16.py`, `test_static_assets.py`, `test_notebook_study_chat_policy.py`, `test_sovushka_chat.py` | [CODE_MAP.md](CODE_MAP.md) · [modules/file-viewer.md](modules/file-viewer.md) | ✅ |
...
| rag/retrieval | exact/FTS/dense/native hybrid/rerank с раздельными score scales; dense требует совместимый `les.rag.index-contract.v1`, retry сохраняет backend, reranker пишет model/score/rank и quality считается после него. После rerank общий format-neutral identifier guard поднимает уже найденный фрагмент с полным буквенно-цифровым обозначением с дефисами, не добавляя кандидатов или доменных весов. Debug endpoint является точной проекцией chunks. Оператор датасетов сверяет старую parse-ошибку с текущим `/api/rag/readiness`: совместимый восстановленный контракт показывает готовность и кнопку «Пуск», а не устаревшее «остановлено» | `retrieval_service`, `retrieval_quality_service`, `lexical_index_service`, `backend/rag_config`, `backend/qdrant_adapter`, `backend/reranker`, `routers/datasets.py`, `sovushka/pages/samovar.py` | [ADR-12-typed-retrieval.md](ADR-12-typed-retrieval.md) · [ALGO-rag-best-practices.md](ALGO-rag-best-practices.md) | ✅ |
| rag/document-explorer | Л.И.С.Т. — no-AI файловый проводник проекта поверх read-only Document Explorer: структура папок, inventory, извлечённые типы/шифры/таблицы и поиск; Qdrant/LES остаётся индексом проверяемых фрагментов, а не генератором ответа. Совушка показывает три панели `выбор датасета → иерархия файлов и папок → контекст датасета или выбранного файла`. Над деревом файлов доступны однозначные фильтры `папка / формат / статус / тип` вместе с поиском по имени. Кнопка `Данные о датасете` открывает паспорт, интерактивную CSS-карту, реестр, извлечённые данные и read-only паспорт `Что попало в RAG`: фактические текстовые фрагменты, символы, headings/table signals и примеры по каждому файлу. Целостность отдельно сверяет исходник, MetaDB, Qdrant, dense/sparse, lexical/FTS и покрытие текстовых страниц PDF. Узлы папок раскрывают дерево, узлы разделов фильтруют по metadata discipline. Клик по PDF дополнительно показывает общий с RAG `list.pdf_page_passport.v1`: тип/качество/OCR, таблицы/графику/штамп, bbox-фрагменты и read-only PNG-превью; upload без абсолютного `source_path` безопасно разрешается только внутри `storage/datasets/{dataset_id}`, абсолютный source path наружу не возвращается. Старые MetaDB без `lexical_chunks[_fts]` получают additive schema migration при первом открытии без reindex | `document_explorer_service`, `pdf_contour_service`, `lexical_index_service`, `backend/qdrant_adapter.py`, `routers/documents.py`, `sovushka/pages/documents.py`, `sovushka/styles.py`; endpoints `GET /api/documents/datasets/{id}/quality`, `GET /api/documents/by-id/{doc_id}/chunks`, `GET /api/documents/by-id/{doc_id}/pdf-contour`, `GET /api/documents/by-id/{doc_id}/pdf-contour/pages/{page}/preview`, `GET /api/rag/datasets/{id}/integrity`, `POST /api/documents/by-id/{doc_id}/open-native`; tests `test_document_explorer_service.py`, `test_pdf_contour_service.py`, `test_dataset_integrity.py`, `test_static_assets.py` | [CODE_MAP.md](CODE_MAP.md) · [modules/pdf-contour.md](modules/pdf-contour.md) | ✅ |
| rag/list-office-studio | GUI-first производство офисных документов внутри Л.И.С.Т.: «Документы → Студия» показывает шаблон, deterministic sources и выбранные основания. Л.Е.С. через typed `office_document_ir_v1` предлагает только ручные поля по bounded exact/FTS-фрагментам с evidence/confidence/assumptions; неизвестное остаётся missing, ошибки модели не маскируются fallback-текстом. GUI отдельно применяет предложения и требует ручную проверку. Только затем Л.И.С.Т. создаёт append-only DOCX/XLSX draft с manifest `list.office_artifact.v1`, IR, SHA-256 и fail-closed скачиванием. OOXML и originals модель не меняет | `list_office_agent_service`, `list_office_service`, `forms_service`, `routers/forms.py`, `sovushka/pages/documents.py`, `config/forms/{technical_letter,meeting_protocol}.yaml`; endpoints `POST /api/forms/agent-draft`, `GET/POST /api/forms/artifacts`, `GET /api/forms/artifacts/{revision_id}/download`; tests `test_list_office_{agent_,}service.py`, `test_forms_service_w113.py`, `test_forms_templates.py`, `test_static_assets.py` | [modules/list-office-studio.md](modules/list-office-studio.md) | ✅ |
...
| intake/mail / Е.Ж.И.К. | read-only полный сбор classic Outlook + multi-account IMAP; один ящик = отдельный P0 dataset; exact registry/cursors/snapshot, `.msg/.eml` message+attachment nodes, Windows interactive sidecar и вкладка «Почта»; legacy `/push` сохранён | `mail_registry_service`, `mail_sync_service`, `mail_profile`; `clients/outlook_mail_poller`; `/api/mail/accounts*`, `/collector/import`, `/messages/{id}/open`; `sovushka/pages/mail.py` | [ALGO-mail-intake.md](ALGO-mail-intake.md) | ✅ |
...
| ops/vps-patch | общий с 0.24.16 для всех совместимых Windows-установок ограниченный канал быстрых обновлений: GUI проверяет feed на `les.ovc.me/updates/`, валидирует origin/allowlist/archive SHA и точный file SHA. Накопительный manifest принимает полный release base, target и каждый commit-снимок файла на доверенной ancestry, поэтому уже установленный промежуточный патч не блокирует следующий; произвольный локальный файл отклоняется. Helper и новый Tauri стартуют отдельными interactive Scheduled Tasks вне job-object старого runtime, выполняют backup→atomic replace→compile→restart→health и автоматический rollback; Legion — полигон приёмки, не особая ветка клиента | `tools/{vps_patch,vps_patch_apply}.py`; `proxy/services/update_service.py`; `proxy/routers/updates.py`; `sovushka/components/header.py`; `deploy/vps/` | [VPS_PATCH_CHANNEL.md](VPS_PATCH_CHANNEL.md) ✅ | ✅ |
| ops/versioning | единый `config/version.json`: пользовательский SemVer, отдельный монотонный номер сборки и внутренняя Tauri/NSIS-версия; `make version-sync` один раз обновляет все машинные поверхности, а `make verify` ловит drift; `/api/version` сохраняет совместимые старые поля и показывает repo↔runtime divergence. Patch-release после Mac/изолированного Windows gate автоматически ставит тот же EXE в production Legion, требует heavy-PDF RRF smoke, закрывает deploy SSH-сессию и отдельным новым соединением проверяет живые API/UI и desktop-процесс до публикации | `version_service`; `tools/sync_version_contract.py`; `tools/{patch_release.py,windows_patch_release.ps1,windows_production_deploy.ps1}`; `GET /api/version`; `make version-sync`; `make patch-release` | [RELEASE_LEDGER.md](RELEASE_LEDGER.md), [VERSIONING.md](VERSIONING.md), [SOFTWARE_VERSIONS.md](SOFTWARE_VERSIONS.md), [releases.md](releases.md) | ✅ |
...
| ops/public-showcase | curated public GitHub/Pages surface без приватных датасетов и runtime-секретов; README объясняет evidence-harness, единый dense+sparse native RRF, typed readers, Л.И.С.Т. и ФГИС ЦС; release staging исключает `.codex_tmp/**`, `tmp/**` и отдельный продукт `products/artel/**` вместе с его tools/fixtures/tests | `README.md`, `docs/index.md`, `docs/public/*`, `tools/publication_check.py`, `tools/build_release_artifacts.py` | [PUBLICATION_CHECKLIST.md](PUBLICATION_CHECKLIST.md), [public/overview.md](public/overview.md) | ✅ |
| ops/test | `make test`, `test-release`, `test-architecture` и collect-only `verify` используют одну каноническую LES-сюиту (2680 collected на 2026-07-17): 11 файлов feature-off Unified/Construction Harness и `test_artel*` исключены. 49 агрегатных повторов и семь always-green assertions удалены; архивный harness доступен только явно через `make test-legacy`. `test-mail` изолирует 61 offline/static проверку Е.Ж.И.К., `test-mail-release` добавляет Tauri и обязателен для patch-release. `smoke-basic` — dev L1; `smoke-basic-release`, `ship` и post-deploy делают P1 блокером. Live chat gate читает актуальный `versions.version_info`, проверяет тот же stamp на детерминированном и RAG final, а пустой clean-runtime без проверяемых ответов отражает как TOSKA WARN, не ложный ERR. Patch-release прогоняет канонический `test-release`, затем Windows live smoke установленного runtime. Изолированный smoke доказывает clean-install smeta baseline (≥40 000 норм, ≥1 500 ФСЭМ), запуск ФСНБ-job, API/UI/Qdrant и временный `dense + qdrant_sparse → RRF`; production smoke проверяет installed Е.Ж.И.К., тяжёлые PDF и native RRF до публикации | `Makefile`; `tests/test_test_profiles.py`; `test-mail[-release]`; `tools/{basic_function_smoke,smeta_release_baseline,patch_release}.py`; `tools/{windows_release_smoke,windows_patch_release,windows_production_deploy}.ps1` | [TEST_INVENTORY.md](TEST_INVENTORY.md) · [TEST_ARCHITECTURE_AUDIT_2026-07-14.md](TEST_ARCHITECTURE_AUDIT_2026-07-14.md) | ✅ |
| install | Tauri 2 — канонический Mac/Windows shell над NiceGUI; **production target — Legion/Windows**, Mac — dev/reference. Rust владеет окном/tray/health/lifecycle, Python остаётся backend sidecar. Финальный Tauri/NSIS EXE собирается на Windows. Свежая Windows-установка держит заменяемый код в латинском `%LOCALAPPDATA%\Programs\LES`, mutable state/venv — отдельно в `%LOCALAPPDATA%\LES`; существующая установка обновляется на месте и перед запуском новой сборки заменяет старые proxy/UI на канонических портах, не останавливая Qdrant и независимую загрузку ФСНБ. Канонический release-flow после Mac-полигона и изолированного Windows smoke автоматически разворачивает EXE в production Legion и проверяет четыре тяжёлых PDF; пользовательское обновление из GUI остаётся ручным и checksum-проверенным. Выпускной staging платформенный: Windows не получает macOS `bootstrap.sh`, Mac не получает Windows bootstrap. `windows-lite` мигрирует legacy с backup и требует полный локальный RAG-контур: закреплённые SHA-256-проверенные Python и `uv.exe` уже в payload. Отсутствующие Ollama/Docker, неработающий Docker/WSL, Qdrant и отсутствующие модели являются состоянием `setup_required`, а не падением приложения. Нативный Tauri wizard проверяет машину, ставит внешние программы через winget по явной кнопке, показывает ручные ссылки и остаётся доступным после запуска из трея как «Настройка и справка». Пользователь сам загружает и выбирает любую Ollama answer-модель; `qwen3.5:9b` только рекомендуется, а `bge-m3:latest` отдельно требуется контрактом embedding 1024. Bootstrap больше не выполняет скрытый `ollama pull`. Winget/official uv installer — только аварийный fallback для повреждённого bundle. Bootstrap пишет `bootstrap-status.json`; terminal `ready` публикуется только после живого `/api/health`, а Qdrant payload-индексы ставятся асинхронно и не блокируют API при первом старте. Здоровый сметный baseline сохраняется, повреждённый переносится в recovery и атомарно восстанавливается из проверенного payload, а невозможность восстановления ограничивает только smeta, не общий UI. Готовая canonical named dense+sparse коллекция без маленького sidecar-contract безопасно принимается при первом старте только если она пуста или каждый point имеет ожидаемый embedding fingerprint; неизвестную/mismatch коллекцию код не переписывает. Named Qdrant volume, единый Ollama URL для query/parse и checksum/load-probe проверенный `BAAI/bge-reranker-v2-m3` сохраняются. Живой выпускной gate отдельным процессом проверяет terminal `ready`, API/UI и native RRF. Legacy pywebview shell не входит в обязательный runtime | `desktop/tauri`, `desktop/tauri/web/{index.html,wizard.js}`, `desktop/tauri/src-tauri/windows-installer-hooks.nsh`, `tools/build_tauri_app.py`, `tools/build_{macos_app,windows_installer}.py`, `tools/windows_{release_smoke,production_deploy}.ps1`, `installers/windows/{state,start-light,stop-light}.ps1`, `installers/windows/app/bootstrap.ps1`, `tools/onboard_reranker.py`, `proxy/services/{update_service,smeta_chat_adapter_service}.py`, `proxy/routers/updates.py`, `backend/qdrant_adapter.py`, `backend/reranker.py` | [INSTALL_RUNBOOK.md](INSTALL_RUNBOOK.md) ✅, [PLATFORMS.md](PLATFORMS.md) ✅, [CODE_MAP.md](CODE_MAP.md) | ✅ |
