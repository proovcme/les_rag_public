...
| ops/vps-patch | Windows имеет два режима поверх одного lifecycle: hard install целиком заменяет дерево программы через verified installer payload, soft package атомарно меняет bounded allowlist файлов. `windows_update_engine.py` до остановки проверяет SHA/identity и persistent venv, затем stop→same-volume rename старого app в recovery→silent NSIS→state junctions→start→поэтапный identity/API/UI/Qdrant/index/process smoke с двумя стабильными пробами; провал удаляет новое дерево и возвращает старое одним rename. `%LOCALAPPDATA%\LES` никогда не входит в удаляемую область. Soft apply перед рестартом проверяет и backup-first восстанавливает bundled verified ФСНБ baseline; acceptance требует живого ГЭСН expand, а smeta readiness — одновременно trusted mechanical SQLite и активный полный dense+sparse/RRF alias. `windows_runtime.py` запускает exact console-free Python PID без PowerShell/cmd, имеет memory cap и до чтения блокирует `.env` больше 1 МБ; startup readiness отдельно фиксирует ранний exit процесса и alive-timeout, прикладывает bounded stderr tail с redaction секретов к status вместо общего 60-second сообщения. `windows_env_doctor.py` атомарно изолирует и восстанавливает повреждённый env без вывода значений. Installed runtime без `.git` не запускает Git при `/api/version`; допустимый dev-probe на Windows скрыт через `CREATE_NO_WINDOW`. Status JSON публикуется atomic replace с bounded retry, поэтому одновременное чтение UI не вызывает ложный rollback. Soft helper использует те же start/stop/smoke primitives. Apply не запускает tests/build/network sync и не использует WMI/CIM; stdout/stderr exact child PID сразу пишутся в файлы с timeout. PowerShell production/bootstrap entrypoints — только тонкие алиасы Python engine, standalone rollback закрыт. Настройки разделяют быстрое обновление и переустановку выпуска | `tools/{windows_runtime,windows_env_doctor,windows_update_engine,vps_patch_apply,vps_patch,windows_update_shell}.py`; `proxy/services/{update_service,version_service,rag_readiness_service}.py`; `proxy/routers/updates.py`; `sovushka/components/header.py`; `installers/windows/{state,start-light,stop-light}.ps1`; `make test-updater/prepare-windows-update` | [VPS_PATCH_CHANNEL.md](VPS_PATCH_CHANNEL.md) ✅ | ✅ |
