...
сначала повторяет обязательный tool-call той же моделью; резервная модель настраивается отдельно и
может быть отключена для чистого сравнительного прогона Gemma.
...
и собирает XLSX. С 0.25.10 каждый `bind` содержит полный technology/overlap evidence, а ресурсное
действие — явный `basis_ref`; это формальный гейт полноты, а не кодовый выбор нормы. Обязательных
resource/impact/dominant review и отдельного finish-вызова нет.

| Суб-модуль | Назначение | Точки входа | Док | Статус |
|---|---|---|---|---|
| smeta/core | model-first PDF/XLSX→ЛСР: единая публичная application-граница, lossless XLSX intake с provenance лист/строка, один общий model-tool loop с полным coverage, три native-инструмента, model-owned opt-in полного ресурсного состава и безопасная transport-нормализация служебных форм (`work_id`, одиночный `norm_code`) без изменения выбора модели. Нормативная SQLite открывается read-only/immutable; точный шифр из `submit_lsr_mapping` дочитывается как выбранная моделью identity и не отклоняется только из-за пропущенного transport-вызова `read_norms_batch`. Обязательны technology/overlap evidence для `bind`, `basis_ref` для ресурсных действий, один расчёт и формульный РИМ-XLSX. MLX tool-вызовы идут без prose-prefill; отсутствие проверяемых строк блокирует модельную placeholder-ЛСР. Ordinary/document orchestration находится в `smeta_chat_application_service`, а smeta transport/RAG/prompts/parsers — в `smeta_chat_adapter_service`; document workflow отдаёт heartbeat и границы каждого model turn, ограничивает локальный вызов 300 секундами, не повторяет timeout скрыто и cooperative-останавливается после текущего transport-вызова. Chat сохраняет request и общий response/history contract. Код проверяет только полноту ссылок/evidence, единицы и арифметику и не выбирает нормы. Legacy harness остаётся private implementation adapter до физической миграции | `proxy/smeta_core/{application,contracts,integrity,norm_browser,source_intake,document_workflow,calculator,resource_normalizer,lsr_renderer,workflow,unit_contract}.py`, `proxy/services/{smeta_chat_application_service,smeta_chat_adapter_service,prompt_registry_service,rim_lsr_trace_service,rim_trace_xlsx_service,smeta_user_message_service,fsem_machinist_service}.py`, `proxy/routers/{chat,chat_history}.py`, `sovushka/pages/chat.py`, `skills/smeta/{SKILL.md,references/gesn-storage.md,references/runtime-agent.md}` | [SMETA_MODULE_EXPLAINED.md](SMETA_MODULE_EXPLAINED.md) · [modules/smeta-core.md](modules/smeta-core.md) | 🟡 |
...
| ops/test | verify/test-release-critical/test-release/test-architecture/test разделены по смыслу. `smoke-basic` — dev L1; `smoke-basic-release`, `ship` и post-deploy делают P1 блокером. Patch-release обязан прогнать полный `test-release` без отдельного ARTEL, затем Windows live smoke установленного runtime. Изолированный smoke доказывает clean-install smeta baseline (≥40 000 норм, ≥1 500 ФСЭМ), запуск ФСНБ-job, API/UI/Qdrant и временный `dense + qdrant_sparse → RRF`; production smoke после автоматической установки требует `N/N` всех текущих тяжёлых PDF Legion-полигона (минимум 4), ненулевые фрагменты и native RRF, затем удаляет временный dataset. Несовместимая legacy-коллекция не усыновляется: production переключается на чистую named dense+sparse generation, сохраняя старую для явной миграции/аудита. Только после двух Windows gate сверяется SHA и разрешается публикация | `Makefile`; `test-release`; `tools/{basic_function_smoke,smeta_release_baseline,patch_release}.py`; `tools/{windows_release_smoke,windows_patch_release,windows_production_deploy}.ps1` | [TEST_INVENTORY.md](TEST_INVENTORY.md) · [TEST_ARCHITECTURE_AUDIT_2026-07-14.md](TEST_ARCHITECTURE_AUDIT_2026-07-14.md) | ✅ |
