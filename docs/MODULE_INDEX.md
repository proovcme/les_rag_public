...
|---|---|---|---|---|
| smeta/model-quality | Воспроизводимый live Qwen↔Gemma A/B на одном canonical XLSX/PDF→ЛСР workflow: одинаковые prompt, corpus, tools, seed, limits и начальное состояние; per-row calculated/partial/missing, tool calls/repeats, latency, unit/volume/provenance integrity, hierarchical-route trace и реальный checkpoint/resume. Профессиональная правильность считается только по явному `les.smeta.qrels.v1`, без self-judge и code-owned выбора нормы | `tools/smeta_model_quality_benchmark.py`; `tests/test_smeta_model_quality_benchmark.py` | [modules/smeta-core.md](modules/smeta-core.md) | ✅ |
