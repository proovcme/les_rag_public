...
| ops/versioning | единый `config/version.json`: пользовательский SemVer, отдельный монотонный номер сборки и внутренняя Tauri/NSIS-версия; `make version-sync` один раз обновляет все машинные поверхности, а `make verify` ловит drift; `/api/version` сохраняет совместимые старые поля и показывает repo↔runtime divergence | `version_service`; `tools/sync_version_contract.py`; `tools/patch_release.py`; `tools/windows_patch_release.ps1`; `GET /api/version`; `make version-sync`; `make patch-release` | [RELEASE_LEDGER.md](RELEASE_LEDGER.md), [VERSIONING.md](VERSIONING.md), [SOFTWARE_VERSIONS.md](SOFTWARE_VERSIONS.md), [releases.md](releases.md) | ✅ |
