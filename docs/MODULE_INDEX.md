...
| smeta/gesn | норма ГЭСН → ресурсы (расход×объём); canonical machine base `data/smeta_base/les_smeta_base.sqlite` (`norms/resources`), raw/unified parquet только source/staging; update pipeline `raw/cache → unified parquet → SQLite → SMETA_SERVICE cards`; source и runtime защищены conservative resource-identity dedupe, audit/manifest показывают схлопнутые дубли | `gesn_service`, `gesn_update_service`; `tools/gesn_update_from_fgis.py`, `tools/gesn_unify_base.py`, `tools/build_smeta_structured_base.py`, `tools/build_smeta_service_rag.py`; Make `smeta-base*`; `GET /api/lsr/gesn[/{code}/expand]`, `POST /api/service-sources/gesn_base/fgis-update`; GUI «Инструменты → ГЭСН → скачать/обновить»; MCP `les_gesn_*` | [ALGO-gesn.md](ALGO-gesn.md) | ✅ |
| smeta/fgis | цена ресурса по коду из «Сплит-формы» ФГИС ЦС; visible-книги управляются `pricebook_manifest.json`, duplicate/scratch parquet не показываются в обычном discovery и `SMETA_SERVICE` | `fgis_price_service`; `config/domain/pricebook_manifest.json`; `/api/prices/*`; MCP `les_price_lookup` | [ALGO-fgis-price.md](ALGO-fgis-price.md) | ✅ |
...
| Суб-модуль | Назначение | Точки входа | Док | Статус |
|---|---|---|---|---|
| rag/core | поток чата, retrieval/evidence/research-loop и честные статусы; corpus-derived notebook не подставляет доменные разделы; fail-closed contract и read-only Qwen sibling builder не позволяют смешивать embeddings | `retrieval_service`, `notebook_study_service`, `saferag_service`, `evidence_packet_service`, `runtime_dispatcher`; `routers/chat.py`; `tools/build_rag_contract_sibling.py` | [CODE_MAP.md](CODE_MAP.md) · [ALGO-rag-best-practices.md](ALGO-rag-best-practices.md) · [RAG_TEST_PROGRAM_AUDIT.md](RAG_TEST_PROGRAM_AUDIT.md) | ✅ |
...
| rag/document-explorer | no-AI просмотр и поиск по датасетам/документам: список датасетов, документы, открытие по `doc_id`, чанки, FTS/LIKE поиск по корпусу/датасету/документу; показывает total/indexed/pending/error/missing, чтобы свежий in-place датасет не выглядел пустым до парса; ручной `dataset_kind` (`project`/`norm`/...) хранится в профиле датасета, отдаётся для сортировки и фильтра списка; Совушка вкладка «Документы» открывается как **Л.И.С.Т.** датасета: сначала сворачиваемый реестр файлов по разделам/типам, затем Mermaid-карта, project roots, discipline summaries, PDF/тома из `project_pdf_extract`, фильтр по типу датасета и действия `Открыть в LES`/`Открыть системно`/`Спросить`; база для WebDAV | `document_explorer_service`, `routers/documents.py`, `sovushka/pages/documents.py`; endpoints `POST /api/documents/by-id/{doc_id}/open-native`, `PATCH /api/rag/datasets/{id}/profile/kind`; tests `test_document_explorer_service.py`, `test_static_assets.py`, `test_notebook_api.py` | [CODE_MAP.md](CODE_MAP.md) | ✅ |
...
| project/pdf-extract / Л.И.С.Т. | Единый source-map PDF-проекта без reindex: оркестрирует `drawing_manifest_v1`, `pd_rd_manifest_v1`, shared `project_pdf_table_manifest_v1` и дисциплинные readers в sidecar `storage/datasets/{dataset_id}/_les_pdf_extract`, отдаёт `project_pdf_extract_v1` с составом томов/ПЗ/ВОР/СО/таблицами/схемами как навигацией для модели; не evidence и не готовый ответ. Пользовательское имя слоя — **Л.И.С.Т. · Локальный индекс структуры томов**. Shared table layer извлекает ОВ `ХВС`, ВК водные балансы, экспликации помещений и compact semantic candidates. Reliability 0.24.0.344: роли/дисциплины распознаются по разделённым filename/cipher-кодам и whitelist; coverage разделяет attempted/successful и возвращает `ok/partial/failed/empty`; bounded warnings/refs/register имеют total+truncated; row/table refs приоритетны; `dataset_id` path-safe; generic `line/length` и одиночное `quantity` из руководств не считаются кабелем/ВОР | `project_pdf_extract_service`, `project_pdf_table_service`; `/api/rag/datasets/{dataset_id}/pdf-extract/{status,run,summary}`; `dataset_memory_service.project_pdf_extract`; `sovushka/pages/documents.py` panel; tests `test_project_pdf_extract_service.py`, `test_project_pdf_table_service.py`, `test_static_assets.py` | [ALGO-pdf-ingestion.md](ALGO-pdf-ingestion.md) · [ALGO-electrical-schematics.md](ALGO-electrical-schematics.md) | ✅ |
...
| ops/test | гейт (verify/test/smoke-basic); live smoke имеет отдельный конечный budget chat check, timeout остаётся failure/warn, а не silent hang | `Makefile`, `tools/basic_function_smoke.py` | [TEST_INVENTORY.md](TEST_INVENTORY.md) 🟡 | 🟡 |
