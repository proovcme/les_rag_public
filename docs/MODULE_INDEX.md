...
| core/tool-harness | controlled registry/executor для инструментов: карта датасета, indexed source search/read, PDF/Excel readers, bounded vision страницы/области исходного PDF и read-only filesystem; возвращает `les_tool_result_v1` с sources/missing/warnings/trace, без финальных предметных решений; общий чат умеет model-selected loop shortlist→JSON calls→executor→model final | `tool_harness_service`, `proxy/routers/tools.py`, `tools/les_tool_harness.py`, `routers/chat.py` | [ALGO-tool-harness.md](ALGO-tool-harness.md) · [CHAT_OPERATOR_CONTRACT.md](CHAT_OPERATOR_CONTRACT.md) | ✅ |
...
| ops/vps-patch | общий для всех совместимых Windows-установок ограниченный канал быстрых обновлений: GUI проверяет feed на `les.ovc.me/updates/`, валидирует origin/allowlist/base SHA/archive SHA, внешний helper делает backup→atomic replace→compile→restart→health и автоматический rollback; Legion — полигон приёмки, не особая ветка клиента | `tools/{vps_patch,vps_patch_apply}.py`; `proxy/services/update_service.py`; `proxy/routers/updates.py`; `sovushka/components/header.py`; `deploy/vps/` | [VPS_PATCH_CHANNEL.md](VPS_PATCH_CHANNEL.md) ✅ | ✅ |
