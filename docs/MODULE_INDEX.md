...
| rag/answer-render | видимый слой evidence в Совушке: conversation-first чат с progressive disclosure, копирование с источниками и artifact `Источники ответа`; P0 UI kit задаёт общие токены/AppShell/Header/controls/status/evidence/feedback для `/classic`, а `BLOCKED/MISSING` отделены от ответа модели и показывают действие оператора; перечень источников закрыт по умолчанию и при раскрытии остаётся в bounded scroll, основной клик открывает поддерживаемый файл во встроенном viewer, отдельная info-кнопка показывает ту же citation drawer со сниппетом, `source_ref`, оригиналом и viewer; технический trace свёрнут; не меняет retrieval/model answer | `sovushka/uikit/{tokens,states,components}.py`, `sovushka/answer_render.py`, `sovushka/pages/{chat,documents}.py`, `sovushka/components/header.py`, `sovushka_ng.py`; tests `test_sovushka_uikit.py`, `test_answer_render_v16.py`, `test_static_assets.py`, `test_sovushka_chat.py` | [modules/sovushka-uikit.md](modules/sovushka-uikit.md) · [modules/file-viewer.md](modules/file-viewer.md) | ✅ |
...
| rag/retrieval | production contract v2: exact readers отдельно; candidate retrieval только `dense + bm25_sparse → native RRF`, затем обязательный общий rerank и parent/context expansion. Native/Rerank failure даёт `status=blocked` без legacy dense/FTS fallback; явный или inferred scope не расширяется при отсутствии датасета. Weak retry сохраняет исходный embedding query и может только расширить пул. Trace аддитивно показывает `status/error_code/resolved_dataset_ids/scope_source`; SQLite FTS остаётся safety-каналом после успешного RRF | `retrieval_service`, `retrieval_quality_service`, `lexical_index_service`, `chat_evidence_application_service`, `backend/rag_config`, `backend/qdrant_adapter`, `backend/reranker` | [ADR-12-typed-retrieval.md](ADR-12-typed-retrieval.md) · [ALGO-rag-best-practices.md](ALGO-rag-best-practices.md) | ✅ |
...
| ops/deploy | dev→рантайм cp-деплой + stamp; internal updater для `codex/audit-rag` разделён на prepare-once и apply-fast. Gates/Mac build выполняются один раз на SHA; Windows installer и isolated smoke кэшируются отдельно; 54-МБ baseline передаётся только при новом checksum. Apply не запускает pytest/build, использует prepared identity/checksums, transactional rollback и persistence smoke без user data; публикация отсутствует | `tools/internal_update.py`, `tools/internal_dual_deploy.py` (Mac transaction), `tools/browser_layout_smoke.py`, `tools/windows_{prepare_update,apply_prepared_update,transactional_production_deploy,production_rollback}.ps1`, `tools/deploy_to_runtime.py`; `make prepare-audit-rag[-legion]`; `make deploy-audit-rag[-mac]` | [INTERNAL_DUAL_DEPLOY.md](INTERNAL_DUAL_DEPLOY.md) ✅, [SKILL.md](../SKILL.md), [RELEASE_LEDGER.md](RELEASE_LEDGER.md) | ✅ |
...
| ops/versioning | единый `config/version.json`: пользовательский SemVer, отдельный монотонный номер сборки и внутренняя Tauri/NSIS-версия; `make version-sync` обновляет машинные поверхности, а `make verify` ловит drift; `/api/version` сохраняет совместимые поля и показывает repo↔runtime divergence. Public patch-release и internal updater разделены: prepared bundle связан с exact commit/version/build и SHA-256, apply не пересобирает release metadata и сохраняет rollback обеих машин | `version_service`; `tools/sync_version_contract.py`; `tools/{patch_release,internal_update,internal_dual_deploy}.py`; Windows prepare/apply/transactional scripts; `GET /api/version`; `make version-sync`; `make patch-release`; `make prepare-audit-rag`; `make deploy-audit-rag` | [INTERNAL_DUAL_DEPLOY.md](INTERNAL_DUAL_DEPLOY.md), [RELEASE_LEDGER.md](RELEASE_LEDGER.md), [VERSIONING.md](VERSIONING.md), [SOFTWARE_VERSIONS.md](SOFTWARE_VERSIONS.md) | ✅ |
...
| ops/test | `make test`, `test-release`, `test-architecture` и collect-only `verify` используют одну каноническую LES-сюиту (2769 collected на 2026-07-28; 2760 passed / 9 skipped): 11 файлов feature-off Unified/Construction Harness и `test_artel*` исключены. Архивный harness доступен только явно через `make test-legacy`. `test-mail` изолирует offline/static проверки Е.Ж.И.К., `test-mail-release` добавляет Tauri и обязателен для выпуска. Полная suite/RAG/build выполняются в prepare ровно один раз на SHA; host apply/retry принимает только checksum-valid prepared bundle и не повторяет gates. Изолированный Windows smoke доказывает clean-install smeta baseline, API/UI/Qdrant и `dense + qdrant_sparse → RRF`; production smoke не индексирует проектные PDF и проверяет установленный ручной Е.Ж.И.К., совместимый RAG-контракт и desktop | `Makefile`; `.github/workflows/{verify,release}.yml`; `tests/test_test_profiles.py`; `test-mail[-release]`; `tools/{platform_release_gate,multiplatform_release,patch_release,internal_update,internal_dual_deploy}.py`; `tools/windows_{release_smoke,prepare_update,apply_prepared_update,transactional_production_deploy,production_rollback}.ps1` | [TEST_INVENTORY.md](TEST_INVENTORY.md) · [TEST_ARCHITECTURE_AUDIT_2026-07-14.md](TEST_ARCHITECTURE_AUDIT_2026-07-14.md) | ✅ |
