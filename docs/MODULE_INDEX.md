...
| smeta/object | мутное ТЗ «дай смету на …» → model-first декомпозиция: модель сама раскладывает объект, харнесс даёт `search_norm`/`add_position`; `search_norm` возвращает explainable shortlist/`candidate_selection_v1`, код проверяет нормы/единицы/объёмы и считает только прошедшие gates | `estimate_harness_service`, `estimate_math_service`, `nr_sp_service`, `evidence_contract`; готовые объектные составы удалены | [ALGO-object-estimate.md](ALGO-object-estimate.md) 🟡 | 🟡 |
