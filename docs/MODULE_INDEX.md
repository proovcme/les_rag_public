...
> **0.27.64 spec→VOR from chat attachment:** read XLSX + «сделай ВОР» routes to
> deterministic `spec_to_bor` (0 LLM) before `attachment_context`; works/qty only,
> no invented prices.

> **0.27.63 Windows RAG contract:** local `les_rag` index-contract adopted to
> v3 (same bge-m3/ollama vectors); `start-light` forces Ollama embed settings
> so Mac CoreML/Qwen `.env` cannot block native RRF.

> **0.27.62 LES+ODS coexist:** host ports split — LES `:11434`/`:6333`,
> ODS llama `:21134`, ODS Qdrant `:26333`; helper
> `scripts/windows/configure-ods-coexist.ps1`.

> **0.27.61 desktop port ownership:** `LES-START.ps1` verifies real Ollama
> (`/api/tags`) on :11434 and open LES Qdrant (`/collections`) on :6333;
> Docker ODS llama.cpp / auth Qdrant no longer silently hijack LES.

> **0.27.60 smeta stability:** Windows `windows-cuda.env` profile (turns=10,
> temp=0.2); exact-deny → broaden recovery; candidate binds do not seed route
> cache; unmapped rows finalize as open Excel without invented rates.

> **0.27.59 smeta batch soft-continue:** bounded schema-repair exhaustion
> (`MappingValidationExhausted`, including invalid provider mapping JSON) no longer aborts the whole document after
> accepted rows; stuck batches are skipped and a partial LSR is finalized.

> **0.27.58 desktop helpers:** `scripts/windows/LES-START.ps1` again bootstraps
> Ollama + native `tools/bin/qdrant.exe` (`config/qdrant.local.yaml`) +
> `start-light` on fixed 8050/8051; `LES-STOP.ps1` uses ownership-aware
> `stop-light` then stops local `qdrant.exe`. Thin start-light-only wrapper left
> API without :6333 (`health.status=error`).
...
| core/chat-evidence | application-граница общего model-first RAG после scope/route и typed tools: retrieval → context/evidence packet → model answer → validation → sources/trace/history. Кодовый professional visible-final запрещён; glossary/registry возвращают только evidence. Router передаёт три явных typed-контракта `EvidenceRequestContext`, `EvidenceRuntimeDeps`, `ResponseBoundary`; namespace capture запрещён | `proxy/services/chat_evidence_application_service.py`, `proxy/routers/chat.py`, `retrieval_service`, `evidence_packet_service`, `saferag_service` | [ALGO-evidence-packet.md](ALGO-evidence-packet.md) | 🟡 |
| core/tool-harness | controlled registry/executor для инструментов: карта датасета, indexed source search/read, PDF/Excel readers, bounded vision страницы/области исходного PDF и read-only filesystem; PDF vision разрешает видимое имя до registry/source_path и отдельно читает общий лист, штамп и легенду через Gemma; возвращает `les_tool_result_v1` с sources/missing/warnings/trace, без финальных предметных решений; общий чат умеет model-selected loop shortlist→JSON calls→executor→model final | `tool_harness_service`, `proxy/routers/tools.py`, `tools/les_tool_harness.py`, `routers/chat.py` | [ALGO-tool-harness.md](ALGO-tool-harness.md) · [CHAT_OPERATOR_CONTRACT.md](CHAT_OPERATOR_CONTRACT.md) | ✅ |
...
| Суб-модуль | Назначение | Точки входа | Док | Статус |
|---|---|---|---|---|
| smeta/core | model-first PDF/XLSX→ЛСР: общий `SmetaNormToolSession` исполняет catalog/search/read/submit; при потере `catalog_query` в transport выводит bounded query из model-authored `work_features` и stall после 3 одинаковых reject; показывает модели typed identity, scope, unit compatibility, operations/resource preview и matched query. Phase scheduler группирует все строки одной минимальной фазы в batch tool call; каждая принятая terminal-строка checkpoint'ится отдельно. Короткие меню и compact cards ограничивают frame, полная карточка читается перед bind. `reuse_norm_catalog_route` применяется только явным решением модели и переносит verified scope без нормы/applicability. `candidate_evaluations` сравнивает shortlist/opened; bind гидрирует выбранный shortlist-код в opened (не выбор нормы кодом); Python проверяет только форму/provenance. `LES_SMETA_NORM_RERANK` + `windows-reranker` для cross-encoder (CUDA torch `cu124`; cpu fallback если `RERANK_DEVICE=cuda` без CUDA). Rejected-table anti-stick (non-rejected search unlock; no re-offer in typed_bind_options). Ollama `qwen2.5-instruct` omits `think` (HTTP 400 retry strips thinking). Hard-reject `covered_by` без bind-provider; anti-stick rejected table root без нового search. `SmetaAgentRunner` даёт одинаковый skill движкам native, Qwen-Agent/Ollama и Google ADK/Gemini. Structured mapping ограничен 8 строками на JSON-вызов и после timeout не повторяет идентичный payload. Terminal mapping сразу показывается через SSE `smeta_row`. `unbound_evidence` сверяется только по реальной tool trajectory; post-budget evidence-repair окно выдаётся один раз, а повторный honest `unbound` без 2 поисков становится видимым `model_batch_candidate` вместо RuntimeError. Чат document LSR: `require_scoped_search=True`; auto-search после таблицы; `route_evidence_cache` только после bind (не table select); `norm_evidence` = search/read/broaden; force-mapping после opened cards со 2-го хода; global review при mapping RuntimeError сохраняет исходные решения (не валит ЛСР); exact-bind с reason «не применима/не соответствует» отвергается и не draft-promote; soft unbound-candidate только после search/opened card; family→root loop: после 2 возвратов одного паспорта без таблицы повторный вход блокируется (пока есть другие семейства), skill ведёт демонтаж→ГЭСНр/отделку→ГЭСН; collection shortlist при выключенном CE — norm-hit coverage+lexical (не голова 51–56); unbound после redirect только после table/search; candidate progress «Нужен поиск нормы»; identical retry→mapping только при evidence; Ollama tool XML 500 — seed+1 retry затем soft-degrade (не валит весь ВОР). Shortlist-меню не блокируется отсутствием cross-encoder. Локальный Ollama/Qwen: fast defaults `max_turns=12`/search·read=3/repair=2; при непустом `route_evidence_cache` — reuse-first; progress показывает `sec_per_row`. PDF «Таблица»→`read_*` для document LSR; без `attachment_id` — честный отказ, не harness с 0 строк. «Собери ЛСР/смету» без `read_*` fail-closed (кроме явной работы с объёмом → harness). После строк conflict-only validator строит connected groups; та же модель пересматривает только конфликтные группы, а остальные решения дословно входят в полную immutable global-review revision. Код нормы не исправляет: автoрасчёт `priced_draft`, пользовательский lock создаёт отдельную финальную ревизию. ETM adapter читает только явно заданные коды и возвращает provenance-bearing КАЦ quotes; материал по API не выбирает. Search/read/card/time budgets и expert metrics находятся в `professional_review.py`; `smeta_mapping_quality.py` готовит human annotation queue и считает только завершённую разметку. Google требует явное согласие и key, без fallback; default остаётся native до профессионального golden-гейта | `proxy/smeta_core/{application,contracts,integrity,norm_browser,source_intake,document_workflow,professional_review,calculator,resource_normalizer,lsr_renderer,workflow,unit_contract}.py`, `proxy/services/{smeta_agent_runner_service,smeta_chat_application_service,smeta_chat_adapter_service,etm_price_service,prompt_registry_service,rim_lsr_trace_service,rim_trace_xlsx_service,smeta_user_message_service,fsem_machinist_service}.py`, `proxy/routers/{chat,chat_history,prices,settings}.py`, `sovushka/{components/header.py,pages/chat.py}`, `tools/{smeta_agent_benchmark,smeta_document_local_run,smeta_mapping_quality}.py`, `skills/smeta/{SKILL.md,references/document-mapping-agent.md,references/gesn-storage.md}` | [SMETA_MODULE_EXPLAINED.md](SMETA_MODULE_EXPLAINED.md) · [modules/smeta-core.md](modules/smeta-core.md) | ✅ |
...
| smeta/gesn | норма ГЭСН → ресурсы (расход×объём); canonical typed SQLite, raw/unified только source/staging; повторный operator update при полном raw-кэше (≥300 отделов) не дергает ФГИС заново, пустые отделы кэшируются; builder проверяет floor и полное provenance до atomic replace, закрывает проверочные SQLite handles перед заменой на Windows и не повреждает предыдущую базу. Windows clean install получает immutable source/base/FSEM payload и default regional pricebook с SHA/schema/count gates, поэтому детализация ОТм воспроизводимо получает тарифы; существующий state не перезаписывается | `gesn_service`, `gesn_update_service`, `tools/{gesn_update_from_fgis,gesn_bulk_import,gesn_unify_base,build_smeta_structured_base,smeta_release_baseline}.py`; `config/domain/smeta_base_active.json`; Make `smeta-base*`; Windows bootstrap/release smoke | [ALGO-gesn.md](ALGO-gesn.md) · [modules/smeta-core.md](modules/smeta-core.md) | ✅ |
| smeta/fgis | цена ресурса по коду из «Сплит-формы» ФГИС ЦС; единый операторский updater получает публичный каталог, последнюю книгу каждой ценовой зоны и полный ГЭСН pipeline; уже скачанные `data/price_base/*_zone-*.parquet` и валидный checkpoint пропускаются без повторного download; pytest/temp checkpoint-пути вне price root игнорируются; raw GESN parquet пишется атомарно (`.tmp`+replace), битый cache уходит в `.corrupt.*`; UI показывает отдельный job-индикатор: запускается/скачивает/пропускает локальное/обрабатывает/ждёт/готово/ошибка, heartbeat, текущий регион или сборник, completed/total/remaining, байты, среднюю скорость и ETA; общий refresh не выдаётся за скачивание; visible-книги управляются `pricebook_manifest.json`, duplicate/scratch parquet не показываются в discovery; Windows status polling не вызывает `os.kill` и не завершает updater | `fgis_price_service`, `fgis_update_service`, `process_status`, `tools/{fgis_full_update,gesn_update_from_fgis,gesn_bulk_import,gesn_pdf_import}.py`; `/api/prices/*`, `/api/service-sources/fgis/update[/status]`; `sovushka/pages/instrumenty.py`; MCP `les_price_lookup` | [ALGO-fgis-price.md](ALGO-fgis-price.md) | ✅ |
...
| smeta/kac | конъюнктурный анализ цен (≥3 КП на материал); web-discovery принимает только model-selected exact product query, сильный артикул и три разных supplier domains, иначе цена остаётся missing; ЭТМ Product API даёт read-only quotes→`kac_map` (Goods browse + Price), не pricebook | `kac_service`, `kac_web_service`, `etm_price_service`; `/api/kac/*`, `/api/prices/etm/{status,browse,lookup-batch,kac-map}`; live `tools/etm_live_smoke.py` | [ALGO-kac.md](ALGO-kac.md) · [ALGO-etm-price.md](ALGO-etm-price.md) | ✅ |
...
| smeta/bor | спецификация Ф9 / PDF+XLSX → ВОР работ (0 LLM); Form-9 coalesce + RU qty + км→м; cable-noun boundaries в `_DECOMPOSE`/`work_verb` (не «кабельный»/«водогазопроводная»); чат read-вложение до attachment LLM; «ЛСР по ВОР» → document LSR | `spec_to_bor_service` (`rows_from_spec_document`, `_coalesce_form9_rows`, `_has_cable_noun`, `generate_spec_bor_from_rows`); `/api/bor/{id}/from-spec*`; `chat.py` channel `spec_to_bor` | [ALGO-spec-to-bor.md](ALGO-spec-to-bor.md) | ✅ |
...
| rag/routing | выбор контура: ProfileResolver → evidence application; model-owned research loop выбирает read-only tools, а любой обычный текстовый запрос никогда не получает professional visible-final от regex/SQL/Python; заметки и legacy deterministic cascade из чата исключены | `profile_resolver`, `chat_evidence_application_service`, `query_router`, `scope_service`, `answer_contract_service`, `workflow_plan_service`, `routers/chat.py` | [ALGO-routing.md](ALGO-routing.md) ✅ · [ALGO-workflow-plan.md](ALGO-workflow-plan.md) ✅ · [AUDIT_DETERMINISM.md](AUDIT_DETERMINISM.md) (история) | ✅ |
| rag/retrieval | production contract v3: deterministic hierarchy `navigation|evidence`; global evidence native RRF всегда сохраняется, navigation RRF только строит descendant filter, descendant evidence проходит второй native RRF, после чего evidence-only fusion идёт в общий rerank и parent/context expansion. Navigation nodes запрещены в citations. Exact readers отдельно. Публикация v3 требует sibling-build/readiness; runtime не мигрируется автоматически | `backend/rag_hierarchy.py`, `backend/rag_config.py`, `backend/qdrant_adapter.py`, `retrieval_service`, `tools/rag_rrf_readiness.py`; tests `test_rag_hierarchy.py`, `test_rag_config.py`, `test_rag_rrf_readiness.py` | [ADR-12-typed-retrieval.md](ADR-12-typed-retrieval.md) · [ALGO-rag-best-practices.md](ALGO-rag-best-practices.md) | ✅ |
...
| rag/service-datasets | Закреплённые сверху служебные датасеты `Нормативы и чек-листы`, `Прайсы`, `Сметные источники`: создаются идемпотентно при bootstrap реального RAG runtime как `dataset_scope=system`; отдельный временный `MetaDB` остаётся без продуктовых side effects. Датасеты доступны в поиске «Документов», принимают файлы через кнопку `Добавить файл` и существующий upload API. Прайсы и сметные источники автоматически входят только в smeta-turn, нормативы — в normcontrol-turn; в обычном чате служебный датасет выбирается явно. | `system_dataset_service`, `document_explorer_service`, `backend/qdrant_adapter.py`, `sovushka/pages/documents.py`, `config/service_sources.yaml` | [CODE_MAP.md](CODE_MAP.md) | ✅ |
...
| intake/mail / Е.Ж.И.К. | read-only полный сбор classic Outlook + multi-account IMAP; один ящик = отдельный P0 dataset; exact registry/cursors/snapshot, `.msg/.eml` message+attachment nodes и Windows interactive sidecar. Рабочий UI ведёт по цепочке `ящик → переписка → письмо → exact target_file в чат`; настройки называют тип локального сборщика без имени конкретного компьютера. Outlook cursor фиксирует завершённый backfill и не перечитывает историю; автоматического расписания нет, проход запускается кнопкой, ограничен 10 снимками/12 секундами и принудительно завершается watchdog через 15 секунд. Loopback intake атомарно сохраняет raw+manifest и освобождает Outlook до exact registry/RAG; последовательный spool возобновляется после рестарта, а parser стартует после его опустошения. Windows gate компилирует sidecar и проверяет cursor persistence; legacy `/push` сохранён | `mail_registry_service`, `mail_sync_service`, `mail_profile`; `proxy/routers/mail.py` (`_queue_outlook_spool_manifest`, `_drain_outlook_uploads`, `recover_outlook_spool`, `/collector/run`); `clients/outlook_mail_poller`; `tools/platform_release_gate.py`; `/api/mail/accounts*`, `/collector/import`, `/messages/{id}/open`; `sovushka/pages/mail.py` (`build_mail`, `build_mail_settings`) | [ALGO-mail-intake.md](ALGO-mail-intake.md) · [modules/sovushka-uikit.md](modules/sovushka-uikit.md) | ✅ |
...
| ops/windows-process | единый запрет видимых console windows: UI/runtime probes и фоновые Python jobs запускают Windows children с `CREATE_NO_WINDOW`; Чат/Студия переключаются внутри готовой `/classic` без повторного построения страницы; общий `lazy_tab_panels` создаёт только активный раздел, после чего нативный Quasar `keep_alive` удерживает уже открытый UI и сохраняет штатную анимацию; same-origin Чат↔Конфигурация получает короткий `@view-transition`; оба motion-пути соблюдают `prefers-reduced-motion`; soft allowlist разрешает exact общий process launcher и его проверенный helper | `tools/{les_runtime_control,vps_patch,vps_patch_apply}.py`; `proxy/services/runtime_dispatcher.py`; `sovushka/{lite_bridge.py,components/header.py,uikit/{components,tokens}.py}`; `sovushka_ng.py`; tests `test_{les_runtime_control,runtime_dispatcher,lite_bridge,sovushka_uikit,vps_patch,windows_application_update}.py` | [VPS_PATCH_CHANNEL.md](VPS_PATCH_CHANNEL.md) · [modules/sovushka-uikit.md](modules/sovushka-uikit.md) | ✅ |
...
| infra/api-integrations | единая карта proxy/MLX/Lite API, локальных сервисов, внешних провайдеров, каналов доступа и безопасных имён конфигурации; внешний разовый документ идёт через пользовательский `POST /api/chat/attachments` → `attachment_id` → идемпотентный `POST /api/chat` → артефакт, а административный `/api/rag/attach` остаётся Совушке/совместимости; PDF «В чат» — fast preserve bytes (без markdown convert) для VOR/LSR + optimistic UI strip; цены ресурсов доступны одиночно и пакетом | `proxy/app.py`, `proxy/routers/{chat,datasets,prices}.py`, `proxy/services/{chat_attachment,request_idempotency}_service.py`, `sovushka/pages/chat.py`, `proxy/routers/settings.py`, `sovushka/lite_bridge.py`, `mlx_host.py`, `env.example` | [LES_API_AND_EXTERNAL_INTEGRATIONS.md](LES_API_AND_EXTERNAL_INTEGRATIONS.md) | ✅ |
...
| ops/http-client-policy | внутренние loopback HTTP-вызовы не наследуют системный SOCKS/HTTP proxy; внешние, LAN и ZeroTier URL сохраняют обычный `httpx` contract. При `LES_LLM_PROVIDER=ollama` `/api/diag` и карта «Рабочие контуры» бьют Ollama `/api/tags` (не MLX `/api/health`) | `backend/http_client_policy.py`; `proxy/routers/diagnostics.py`; `sovushka/pages/diag.py`; callers в `sovushka/state.py`, `proxy/app.py`, metrics/reranker | [modules/http-client-policy.md](modules/http-client-policy.md) | ✅ |
...
| install | Tauri 2 — канонический Mac/Windows shell над NiceGUI; **production target — Legion/Windows**, Mac — dev/reference. Пользовательский Windows-контур как AnythingLLM: `LES-Setup.exe` / «Параметры → Приложения», upgrade поверх, optional wipe `%LOCALAPPDATA%\LES`; NSIS hooks + `les-setup-helpers.ps1` (`preflight-install` убивает все `les-desktop`/зомби `LES-removed|purge`, чинит ярлыки) останавливают LES best-effort (`exit 0`, без «ошибка 1»), сами тянут WebView2, пишут недостающие Ollama/Docker в `setup-deps-missing.txt`. Док: [WINDOWS_DESKTOP.md](WINDOWS_DESKTOP.md). Windows shell собирается как GUI subsystem, держит named single-instance + один lifecycle action, скрывает все Rust child-команды через `CREATE_NO_WINDOW`; proxy/UI/Lemonade запускаются прямыми Python PID без `cmd.exe`/`uv` wrappers. Updater повторно использует уже проверенный runtime при desktop handoff вместо второго bootstrap. Self-update запускает checksum-declared target helper/engine/runtime launcher из пакета, если они изменились, поэтому новый smoke действует на тот же выпуск, а не только на следующий. Acceptance-smoke вызывает фактический configured reranker с двумя фрагментами и принимает выпуск только после непустого ранжирования; HTTP 404 от чужого Ollama route больше не считается прогревом. Hosted CI на каждом PR/main запускает переносимый поведенческий/артефактный профиль и native Tauri build на `macos-14`/`windows-2022`. Approved production workflow на self-hosted Mac собирает и проверяет DMG, затем через Legion строит/устанавливает/проверяет NSIS и публикует обе платформы одной release-операцией. Mutable state остаётся вне replaceable app tree; Windows bundle получает проверенные Python/uv и immutable ФГИС/ФСНБ smeta baseline, но не пользовательский RAG. Отсутствующие Ollama/Docker/Qdrant/модели остаются `setup_required`; живой gate проверяет terminal ready, API/UI, native RRF, reranker, один desktop, direct Python process contract и отсутствие LES-owned `cmd.exe`. Пользовательский Windows UX как у AnythingLLM: `LES-Setup.exe` ставит/обновляет поверх; Uninstall из Параметров Windows останавливает LES и предлагает сохранить или снести `%LOCALAPPDATA%\LES`. | `.github/workflows/{verify,release}.yml`; `desktop/tauri`; `desktop/tauri/src-tauri/windows-installer-hooks.nsh`; `installers/windows/{start-light,state,app/LES.nsi}.ps1`; `tools/{platform_release_gate,multiplatform_release,patch_release,build_tauri_app}.py`; `tools/windows_{release_smoke,production_deploy,production_rollback}.ps1` | [WINDOWS_DESKTOP.md](WINDOWS_DESKTOP.md) ✅, [ALGO-windows-lifecycle.md](ALGO-windows-lifecycle.md), [INSTALL_RUNBOOK.md](INSTALL_RUNBOOK.md) ✅, [PLATFORMS.md](PLATFORMS.md) ✅ | ✅ |
