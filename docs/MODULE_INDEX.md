...
| rag/answer-render | видимый слой evidence в Совушке: conversation-first чат с progressive disclosure, копирование с источниками и artifact `Источники ответа`; P0 UI kit задаёт общие токены/AppShell/Header/controls/status/evidence/feedback для `/classic`, заметные кнопки «Чат»/«Конфигурация», громкий degraded/blocked статус и метки источников «Использован»/«Найден»/«Слабый». Документы ведут по цепочке dataset→files→sticky «Спросить в чате», почта показывает индекс/spool и передаёт exact письмо в чат; технический trace свёрнут; не меняет retrieval/model answer | `sovushka/uikit/{tokens,states,components}.py`, `sovushka/answer_render.py`, `sovushka/pages/{chat,documents,mail}.py`, `sovushka/components/header.py`, `sovushka_ng.py`; tests `test_sovushka_uikit.py`, `test_answer_render_v16.py`, `test_static_assets.py`, `test_sovushka_chat.py` | [modules/sovushka-uikit.md](modules/sovushka-uikit.md) · [modules/file-viewer.md](modules/file-viewer.md) | ✅ |
