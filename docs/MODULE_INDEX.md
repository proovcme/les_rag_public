...
| Суб-модуль | Назначение | Точки входа | Док | Статус |
|---|---|---|---|---|
| ops/windows-process | единый запрет видимых console windows: UI/runtime probes и фоновые Python jobs запускают Windows children с `CREATE_NO_WINDOW`; Чат/Студия переключаются внутри готовой `/classic` без повторного построения страницы; soft allowlist разрешает exact общий process launcher и его проверенный helper | `tools/{les_runtime_control,vps_patch,vps_patch_apply}.py`; `proxy/services/runtime_dispatcher.py`; `sovushka/{lite_bridge.py,components/header.py}`; tests `test_{les_runtime_control,runtime_dispatcher,lite_bridge,sovushka_uikit,vps_patch,windows_application_update}.py` | [VPS_PATCH_CHANNEL.md](VPS_PATCH_CHANNEL.md) · [modules/sovushka-uikit.md](modules/sovushka-uikit.md) | ✅ |
