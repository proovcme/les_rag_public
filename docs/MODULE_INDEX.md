...
| ops/test | гейты разделены по смыслу: verify=синтаксис/collection, test-architecture=текущая архитектура без feature-off harness, test=полная регрессия вместе с историей, test-rag-core=RAG integrity; Windows release smoke запускает установленный runtime, через его API создаёт и после прогона удаляет изолированный seed-датасет, ждёт terminal `ready`, проверяет API/UI/Qdrant и фактический `dense + qdrant_sparse → RRF → rerank`; patch-release дополнительно проверяет clean/pushed commit, изолированную установку EXE, SHA-256 и опубликованный asset | `Makefile`, `tools/basic_function_smoke.py`, `tools/windows_release_smoke.ps1`, `tools/patch_release.py`, `tools/windows_patch_release.ps1` | [TEST_INVENTORY.md](TEST_INVENTORY.md) · [TEST_ARCHITECTURE_AUDIT_2026-07-14.md](TEST_ARCHITECTURE_AUDIT_2026-07-14.md) | ✅ |
