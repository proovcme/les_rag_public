...
| smeta/rim-dialog | persistent диалоговая РИМ-сессия: XLSX/CSV intake, ВОР, один технический вопрос с вариантами, строгий model-selected маршрут `browse root → continue/ask/broaden/unbound → family → collection → section → table → search → read → submit`, durable resume после каждого tool result, read-only построчная progress-проекция checkpoint и authoritative resume-budget/evidence status. Typed FSNB-узлы содержат `node_id/parent_id/node_type/cipher/source_ref/edition`; код проверяет только смежность, реально показанное evidence и model self-conflict, не выбирая применимость. Успешные и отклонённые переходы получают `trace_id/outcome`, остаются в checkpoint и переходят в immutable `agent_audit`. Scope не наследуется между строками. Lifetime evidence остаётся в аудите, каждый resume получает bounded execution slice. `bind` разрешён schema только по открытым typed-карточкам с совместимой единицей, а внутренне противоречивый `exact` сохраняется как блокирующий conflict. Model-visible паспорта пяти семейств ФСНБ и model-query-driven reranked shortlist работают без скрытого query expansion. Phase-scoped skill/tool schemas не грузят Qwen всем workflow сразу. Project source identity наследуется из parent VOR. Далее immutable mapping/global review/two user locks, authored scenarios, canonical РИМ-расчёт, typed requirements, audit/XLSX и lazy NiceGUI workbench с восстановлением сессии и live-обновлением progress. RAG-card и реестр источников только навигация; расчёт допускает лишь полностью открытую карточку structured store. Закрытие КАЦ/коэффициента требует пересчёта трассы | `config/domain/smeta_normative_catalog.json`, `proxy/smeta_core/{norm_browser,rim_session,document_workflow}.py`, `proxy/services/{rim_knowledge_service,rim_agent_action_service,rim_agent_turn_service,rim_mapping_progress_service,rim_mapping_review_service,rim_mapping_xlsx_service,rim_scenario_service,rim_session_xlsx_service}.py`, `proxy/routers/rim.py`, `sovushka/pages/rim.py`; `/api/rim/sessions/*`, `GET /mapping/progress` | [modules/rim-dialog-session.md](modules/rim-dialog-session.md) | ✅ |
