...
| rag/answer-render | видимый слой evidence в Совушке: conversation-first чат с progressive disclosure, копирование с источниками и artifact `Источники ответа`; канонический Locia-like UI kit ограничивает новый UI семью общими примитивами, едиными tokens и WCAG-проверками. Desktop rail разделяет основные поверхности, рабочие разделы и подписанные служебные действия; все строки выровнены влево по фиксированной 20 px icon-column с единым gap 8 px. «Чат», «Студия», «Конфигурация» имеют равные размеры и одно активное состояние. Mobile-шапка открывает вторичные вкладки текстовым меню «Разделы». Общий `acronym_identity` показывает фирменный зелёный акроним и каноническую расшифровку, которую пользователь может скрыть. Верхняя строка чата содержит только компактные контекстные действия, composer явно обозначен как «Ваш запрос». Конфигурация показывает один паспорт готовности и вертикальные контуры; детали, бэкапы, словарь и общий журнал свёрнуты, постоянный terminal-footer удалён. «Датасеты» показывают компактный паспорт фактического корпуса и единый реестр наборов; поиск/фильтры и подписанные основные действия находятся в первом слое, индексатор и опасные операции — в disclosure/overflow. Общие controls/status/evidence/feedback показывают громкий degraded/blocked статус и метки источников «Использован»/«Найден»/«Слабый». Документы ведут по цепочке dataset→files→sticky «Спросить в чате», почта показывает индекс/spool и передаёт exact письмо в чат; технический trace свёрнут; не меняет retrieval/model answer | `sovushka/uikit/{tokens,states,components}.py`, `sovushka/styles.py`, `sovushka/answer_render.py`, `sovushka/pages/{chat,documents,mail,diag,samovar}.py`, `sovushka/components/header.py`, `sovushka_ng.py`; `skills/sovushka-ui/{SKILL.md,references/review-checklist.md}`; tests `test_sovushka_uikit.py`, `test_answer_render_v16.py`, `test_static_assets.py`, `test_sovushka_chat.py` | [modules/sovushka-uikit.md](modules/sovushka-uikit.md) · [modules/file-viewer.md](modules/file-viewer.md) | ✅ |
...
| ops/test | `make test`, `test-release`, `test-architecture` и collect-only `verify` используют одну каноническую LES-сюиту (2788 collected текущим `make verify` 2026-07-28): 11 файлов feature-off Unified/Construction Harness и `test_artel*` исключены. Архивный harness доступен только явно через `make test-legacy`. `test-mail` изолирует offline/static проверки Е.Ж.И.К., `test-mail-release` добавляет Tauri и обязателен для Windows-выпуска. Для Mac updater профильные behavior-тесты и `make verify` запускаются до commit; prepare/apply по кнопке никогда не повторяют pytest/build. Изолированный Windows smoke остаётся отдельным будущим этапом Legion | `Makefile`; `.github/workflows/{verify,release}.yml`; `tests/test_test_profiles.py`; `tests/test_mac_update.py`; `test-mail[-release]`; `tools/{platform_release_gate,multiplatform_release,patch_release,mac_update,mac_update_apply}.py` | [TEST_INVENTORY.md](TEST_INVENTORY.md) · [TEST_ARCHITECTURE_AUDIT_2026-07-14.md](TEST_ARCHITECTURE_AUDIT_2026-07-14.md) | ✅ |
