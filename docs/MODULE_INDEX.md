...
| core/runtime-config | GUI-first реестр runtime/env и фактических model/context-факторов: для preset/input/reserves/reasoning показывает человеческое `requested → effective · source`, restart и честный read-only/profile-clone путь; env сохраняет masked secrets, read-only bootstrap, `Danger` confirmation и recoverable atomic write. Advanced RAG policy хранится отдельно от env | `proxy/services/runtime_config_registry_service.py`, `proxy/services/rag_advanced_policy_service.py`, `proxy/routers/settings.py`, `proxy/routers/rag_advanced.py`, `sovushka/pages/diag.py` | [modules/sovushka-uikit.md](modules/sovushka-uikit.md) · [modules/chat-profiles.md](modules/chat-profiles.md) | ✅ |
