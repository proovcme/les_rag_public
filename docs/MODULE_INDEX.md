...
| smeta/rim-dialog | persistent диалоговая РИМ-сессия: XLSX/CSV intake, ВОР, строгий model-selected scoped batch `catalog→search→read→submit`, immutable mapping/global review/two user locks, authored scenarios, canonical РИМ-расчёт, typed requirements, audit/XLSX и lazy NiceGUI workbench. RAG-card только навигация; расчёт допускает лишь полностью открытую карточку structured store. Закрытие КАЦ/коэффициента требует пересчёта трассы | `proxy/smeta_core/{rim_session,document_workflow}.py`, `proxy/services/rim_{agent_action,agent_turn,mapping_review,mapping_xlsx,scenario,session_xlsx}_service.py`, `proxy/routers/rim.py`, `sovushka/pages/rim.py`; `/api/rim/sessions/*` | [modules/rim-dialog-session.md](modules/rim-dialog-session.md) | ✅ |
