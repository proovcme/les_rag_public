...
| core/context-governor | Единый budget-aware packer канонического inference: сначала generation/safety reserves и обязательные объекты, затем целые typed objects в каноническом порядке; omission возвращает IDs/cursor, required overflow блокирует model call. Legacy fitter стал compatibility adapter без mid-object slicing | `proxy/services/context_governor_service.py`, `proxy/services/llm_transport_profile_service.py` | [ALGO-context-memory.md](ALGO-context-memory.md) · [canonical design](superpowers/specs/2026-08-26-canonical-tool-context-memory-update-design.md) | ✅ |
