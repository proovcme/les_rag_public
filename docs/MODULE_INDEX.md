...
| intake/mail / Е.Ж.И.К. | read-only полный сбор classic Outlook + multi-account IMAP; один ящик = отдельный P0 dataset; exact registry/cursors/snapshot, `.msg/.eml` message+attachment nodes и Windows interactive sidecar. Письма читаются в рабочей вкладке «Почта», а подключения/синхронизация вынесены в отдельную admin-вкладку «Настройка почты». Outlook cursor фиксирует завершённый backfill, не перечитывает историю на каждом poll и по умолчанию запускается раз в 10 минут; Windows gate компилирует sidecar и проверяет cursor persistence; legacy `/push` сохранён | `mail_registry_service`, `mail_sync_service`, `mail_profile`; `clients/outlook_mail_poller`; `tools/platform_release_gate.py`; `/api/mail/accounts*`, `/collector/import`, `/messages/{id}/open`; `sovushka/pages/mail.py` (`build_mail`, `build_mail_settings`) | [ALGO-mail-intake.md](ALGO-mail-intake.md) | ✅ |
...
| ops/test | `make test`, `test-release`, `test-architecture` и collect-only `verify` используют одну каноническую LES-сюиту (2749 collected на 2026-07-28; 2740 passed / 9 skipped): 11 файлов feature-off Unified/Construction Harness и `test_artel*` исключены. Архивный harness доступен только явно через `make test-legacy`. `test-mail` изолирует 61 offline/static проверку Е.Ж.И.К., `test-mail-release` добавляет Tauri и обязателен для выпуска. Hosted `platform-gate` выполняет переносимые unit/integration, реальный smeta-baseline, проверки установщика текущей ОС и нативную Tauri-сборку на macOS/Windows; Windows дополнительно компилирует и self-test-ит реальный Outlook sidecar. Полная сюита остаётся локальным release-гейтом, а installed Legion smoke проверяет то, чего нет на hosted runner. `smoke-basic-release`, `ship` и post-deploy делают P1 блокером. Изолированный smoke доказывает clean-install smeta baseline (≥40 000 норм, ≥1 500 ФСЭМ), API/UI/Qdrant и временный `dense + qdrant_sparse → RRF`; production smoke проверяет installed Е.Ж.И.К., тяжёлые PDF и native RRF до публикации | `Makefile`; `.github/workflows/{verify,release}.yml`; `tests/test_test_profiles.py`; `test-mail[-release]`; `tools/{platform_release_gate,multiplatform_release,patch_release}.py`; `tools/{windows_release_smoke,windows_patch_release,windows_production_deploy}.ps1` | [TEST_INVENTORY.md](TEST_INVENTORY.md) · [TEST_ARCHITECTURE_AUDIT_2026-07-14.md](TEST_ARCHITECTURE_AUDIT_2026-07-14.md) | ✅ |
