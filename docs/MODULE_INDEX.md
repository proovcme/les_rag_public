...
| rag/notebook-study | NotebookLM-подобное чтение проекта: broad-запрос → адаптивный reading plan по блокноту → параллельный section retrieval → модельная сводка + artifact с источниками/пробелами; не deterministic final | `notebook_study_service`; `routers/chat.py`; payload `notebook_context`, `artifact` | [ALGO-notebook-study.md](ALGO-notebook-study.md) | ✅ |
