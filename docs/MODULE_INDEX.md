...
| rag/answer-render | видимый слой evidence в Совушке: conversation-first чат с progressive disclosure, source chips, citation drawer, копирование с источниками и artifact `Источники ответа`; композер по умолчанию однострочный, режимы и действия занимают один компактный уровень, постоянные развёрнутые подсказки скрыты ради окна ответов; технические статусы и вторичные действия скрыты из первого слоя; не меняет retrieval/model answer | `sovushka/answer_render.py`, `sovushka/pages/chat.py`, `sovushka/styles.py`; tests `test_answer_render_v16.py`, `test_notebook_study_chat_policy.py`, `test_sovushka_chat.py` | [CODE_MAP.md](CODE_MAP.md) | ✅ |
