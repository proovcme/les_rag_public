...
| Суб-модуль | Назначение | Точки входа | Док | Статус |
|---|---|---|---|---|
| smeta/core | model-first PDF/XLSX→ЛСР: общий `SmetaNormToolSession` исполняет `search_norms_batch`/`read_norms_batch`/`submit_lsr_mapping`, а переключаемый `SmetaAgentRunner` даёт одинаковый skill и LES tools движкам `native`, Qwen-Agent/Ollama и Google ADK/Gemini. Код не выбирает и не исправляет нормы, затем один раз считает immutable-ревизию и формирует РИМ-XLSX. Свежие прогоны без replay прошлого mapping: `LES_SMETA_DOCUMENT_SEED` + стабильный порядок кандидатов + bind/unbound rules + same-run self-check (`LES_SMETA_DOCUMENT_SELF_CHECK`) + local batch=5. Google требует явное согласие и key, без fallback; default остаётся native до профессионального гейта. Предыдущий результат 10/19 признан технически завершённым, но профессионально неприемлемым. Quick/full сравнение изолировано в `tools/smeta_agent_benchmark.py` | `proxy/smeta_core/{application,contracts,integrity,norm_browser,source_intake,document_workflow,calculator,resource_normalizer,lsr_renderer,workflow,unit_contract}.py`, `proxy/services/{smeta_agent_runner_service,smeta_chat_application_service,smeta_chat_adapter_service,prompt_registry_service,rim_lsr_trace_service,rim_trace_xlsx_service,smeta_user_message_service,fsem_machinist_service}.py`, `proxy/routers/{chat,chat_history,settings}.py`, `sovushka/{components/header.py,pages/chat.py}`, `tools/smeta_agent_benchmark.py`, `skills/smeta/{SKILL.md,references/document-mapping-agent.md,references/gesn-storage.md}` | [SMETA_MODULE_EXPLAINED.md](SMETA_MODULE_EXPLAINED.md) · [modules/smeta-core.md](modules/smeta-core.md) | ✅ |
...
| smeta/gesn | норма ГЭСН → ресурсы (расход×объём); canonical typed SQLite, raw/unified только source/staging; builder проверяет floor до atomic replace. Windows clean install получает immutable source/base/FSEM payload с SHA/integrity и не перезаписывает существующий state | `gesn_service`, `gesn_update_service`, `tools/{gesn_update_from_fgis,gesn_unify_base,build_smeta_structured_base,smeta_release_baseline}.py`; `config/domain/smeta_base_active.json`; Make `smeta-base*`; Windows bootstrap/release smoke | [ALGO-gesn.md](ALGO-gesn.md) · [modules/smeta-core.md](modules/smeta-core.md) | ✅ |
...
| rag/answer-render | видимый слой evidence в Совушке: conversation-first чат с progressive disclosure, source chips, citation drawer, копирование с источниками и artifact `Источники ответа`; композер по умолчанию однострочный, режимы и действия занимают один компактный уровень, а подсказки по активному режиму всплывают компактно над вводом; «Остановить диалог» отменяет только текущий streaming-task с честным статусом, SSE не прижимает читателя вниз после его ручной прокрутки; `session_id` персистится в browser storage и transcript подтягивается из `/api/chat/history` после закрытия web/app; SSE disconnect не cancel'ит runner (история/smeta дописываются); Lite chat тоже restore по localStorage session; технические статусы и вторичные действия скрыты из первого слоя; не меняет retrieval/model answer | `sovushka/answer_render.py`, `sovushka/{state,lite_chat}.py`, `sovushka/pages/{chat,history}.py`, `proxy/routers/chat.py`; tests `test_answer_render_v16.py`, `test_chat_stream_w51.py`, `test_sovushka_lite_chat.py`, `test_sovushka_chat.py` | [CODE_MAP.md](CODE_MAP.md) | ✅ |
...
| rag/pdf | PDF ingestion для RAG: обязательный быстрый page-text baseline на PyMuPDF, page-level nodes вместо markdown chunk flood, layout/table/OCR как enrichment; короткие технические обозначения получают sparse fallback только для иначе пустого фрагмента, а separator/symbol/custom-font noise без sparse-термов отбрасывается по чанку, не превращая весь PDF в `ERROR`; Ollama embedding использует bounded retry и деление проблемной партии, а dataset-job показывает очередь, текущий файл и человеческий этап. Timeout/один плохой фрагмент не должны превращать текстовый PDF в `ERROR`, а проектные PDF с weak smeta-словами не должны уходить в `TABLE_SMETA` | `backend/converter`, `backend/qdrant_adapter`, `backend/inference/bm25_sparse`, `backend/document_router`, `backend/pdf_layout`, `proxy/routers/datasets.py`; флаги `RAG_PDF_INDEX_FAST_TEXT_FIRST`, `RAG_PDF_FAST_TEXT_FALLBACK_ENABLED`, `RAG_PDF_PAGE_NODES_ENABLED`, `RAG_PDF_PAGE_NODE_MAX_CHARS`, `RAG_EMBED_RETRY_ATTEMPTS`, `LES_LAYOUT_PDF` | [ALGO-pdf-ingestion.md](ALGO-pdf-ingestion.md) · [ALGO-pdf-layout.md](ALGO-pdf-layout.md) | ✅ |
...
| ops/public-team | полный безопасный tracked source для командной сборки и PR-review без приватных датасетов и runtime-секретов; README объясняет evidence-harness, единый dense+sparse native RRF, typed readers, Л.И.С.Т. и ФГИС ЦС; `docs-check` проверяет канон, skills, ALGO-доки, ссылки и version drift, а release staging по-прежнему исключает runtime/private content | `README.md`, `docs/index.md`, `docs/public/*`, `tools/{publication_check,docs_contract_audit,build_release_artifacts}.py`, `tests/test_docs_contract_audit.py` | [PUBLICATION_CHECKLIST.md](PUBLICATION_CHECKLIST.md), [public/overview.md](public/overview.md) | ✅ |
| ops/test | verify/test-release-critical/test-release/test-architecture/test разделены по смыслу. `test-architecture` относится к LES и исключает legacy Unified + отдельный ARTEL; `test-mail` изолирует 61 offline/static проверку Е.Ж.И.К., `test-mail-release` добавляет Tauri и обязателен для patch-release. `smoke-basic` — dev L1; `smoke-basic-release`, `ship` и post-deploy делают P1 блокером. Patch-release обязан прогнать полный `test-release` без отдельного ARTEL, затем Windows live smoke установленного runtime. Изолированный smoke доказывает clean-install smeta baseline (≥40 000 норм, ≥1 500 ФСЭМ), запуск ФСНБ-job, API/UI/Qdrant и временный `dense + qdrant_sparse → RRF`; production smoke проверяет installed Е.Ж.И.К. одноразовым probe в interactive Scheduled Task пользователя Outlook, затем требует `N/N` всех текущих тяжёлых PDF Legion-полигона (минимум 4), ненулевые фрагменты и native RRF, удаляет только временный dataset и передаёт запуск Tauri в интерактивную сессию. Отдельная SSH-сессия подтверждает, что desktop/API/UI пережили release-процесс. Несовместимая legacy-коллекция не усыновляется. Только после Windows gates сверяется SHA и разрешается публикация | `Makefile`; `test-mail[-release]`; `tools/{basic_function_smoke,smeta_release_baseline,patch_release}.py`; `tools/{windows_release_smoke,windows_patch_release,windows_production_deploy}.ps1` | [TEST_INVENTORY.md](TEST_INVENTORY.md) · [TEST_ARCHITECTURE_AUDIT_2026-07-14.md](TEST_ARCHITECTURE_AUDIT_2026-07-14.md) | ✅ |
| install | Tauri 2 — канонический Mac/Windows shell над NiceGUI; **production target — Legion/Windows**, Mac — dev/reference. Rust владеет окном/tray/health/lifecycle, Python остаётся backend sidecar. Финальный Tauri/NSIS EXE собирается на Windows. Свежая Windows-установка держит заменяемый код в латинском `%LOCALAPPDATA%\Programs\LES`, mutable state/venv — отдельно в `%LOCALAPPDATA%\LES`; существующая установка обновляется на месте и перед запуском новой сборки заменяет старые proxy/UI на канонических портах, не останавливая Qdrant и независимую загрузку ФСНБ. Канонический release-flow после Mac-полигона и изолированного Windows smoke автоматически разворачивает EXE в production Legion и проверяет четыре тяжёлых PDF; пользовательское обновление из GUI остаётся ручным и checksum-проверенным. Выпускной staging платформенный: Windows не получает macOS `bootstrap.sh`, Mac не получает Windows bootstrap. `windows-lite` мигрирует legacy с backup и требует полный локальный RAG-контур: закреплённый SHA-256-проверенный `uv.exe` уже в payload; Ollama и Docker Desktop ставятся через winget либо показывается официальный адрес. Winget/official uv installer — только аварийный fallback для повреждённого bundle; Docker/Qdrant не деградируют молча. Bootstrap пишет `bootstrap-status.json`, Tauri запускает PowerShell без консольного окна и показывает точную причину/код/ссылку/журнал; terminal `ready` публикуется только после живого `/api/health`, а Qdrant payload-индексы ставятся асинхронно и не блокируют API при первом старте. Здоровый сметный baseline сохраняется, повреждённый переносится в recovery и атомарно восстанавливается из проверенного payload, а невозможность восстановления ограничивает только smeta, не общий UI. Готовая canonical named dense+sparse коллекция без маленького sidecar-contract безопасно принимается при первом старте только если она пуста или каждый point имеет ожидаемый embedding fingerprint; неизвестную/mismatch коллекцию код не переписывает. Named Qdrant volume, единый Ollama URL для query/parse `bge-m3`/1024 и checksum/load-probe проверенный `BAAI/bge-reranker-v2-m3` сохраняются. Живой выпускной gate отдельным процессом проверяет terminal `ready`, API/UI и native RRF, не зависая на stdout дочерних служб; PowerShell выводит диагностические сообщения в UTF-8. Legacy pywebview shell не входит в обязательный runtime | `desktop/tauri`, `desktop/tauri/src-tauri/windows-installer-hooks.nsh`, `tools/build_tauri_app.py`, `tools/build_{macos_app,windows_installer}.py`, `tools/windows_{release_smoke,production_deploy}.ps1`, `installers/windows/{state,start-light,stop-light}.ps1`, `installers/windows/app/bootstrap.ps1`, `tools/onboard_reranker.py`, `proxy/services/{update_service,smeta_chat_adapter_service}.py`, `proxy/routers/updates.py`, `backend/qdrant_adapter.py`, `backend/reranker.py` | [INSTALL_RUNBOOK.md](INSTALL_RUNBOOK.md) ✅, [PLATFORMS.md](PLATFORMS.md) ✅, [CODE_MAP.md](CODE_MAP.md) | ✅ |
