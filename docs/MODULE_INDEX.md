...
> (`MappingValidationExhausted`) no longer aborts the whole document after
...
| smeta/bor | спецификация Ф9 / tabular XLSX → ВОР работ (0 LLM); чат read-вложение перехватывается до attachment LLM | `spec_to_bor_service` (`rows_from_spec_xlsx`, `generate_spec_bor_from_rows`); `/api/bor/{id}/from-spec*`; `chat.py` channel `spec_to_bor` | [ALGO-spec-to-bor.md](ALGO-spec-to-bor.md) | ✅ |
