...
| rag/answer-render | видимый слой evidence в Совушке: conversation-first чат с progressive disclosure, копирование с источниками и artifact `Источники ответа`; UI kit задаёт тёплый зелёный Locia-like AppShell с left-aligned desktop rail: отдельно основные поверхности, «Рабочие разделы» и подписанные служебные действия/статус; mobile-шапка заменяет rail и открывает вторичные вкладки текстовым меню «Разделы». Общий `acronym_identity` показывает фирменный зелёный акроним и каноническую расшифровку, которую пользователь может скрыть в настройках. Верхняя строка чата содержит только компактные контекстные действия, composer явно обозначен как «Ваш запрос». Общие controls/status/evidence/feedback показывают громкий degraded/blocked статус и метки источников «Использован»/«Найден»/«Слабый». Документы ведут по цепочке dataset→files→sticky «Спросить в чате», почта показывает индекс/spool и передаёт exact письмо в чат; технический trace свёрнут; не меняет retrieval/model answer | `sovushka/uikit/{tokens,states,components}.py`, `sovushka/styles.py`, `sovushka/answer_render.py`, `sovushka/pages/{chat,documents,mail}.py`, `sovushka/components/header.py`, `sovushka_ng.py`; tests `test_sovushka_uikit.py`, `test_answer_render_v16.py`, `test_static_assets.py`, `test_sovushka_chat.py` | [modules/sovushka-uikit.md](modules/sovushka-uikit.md) · [modules/file-viewer.md](modules/file-viewer.md) | ✅ |
