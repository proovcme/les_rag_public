...
| core/chat-evidence | application-граница model-owned evidence-first RAG: явный user scope либо typed system datasets явной роли → model-authored `search_sources` через named dense+sparse native RRF → evidence packet → повторяемые model-selected search/read calls → governed final packet → неизменённый ответ/решения модели + sources/trace/history. Для estimator код не делает первичный query из команды пользователя: вложение видно первой модели как evidence, native tool schemas берутся из registry, все calls одного ответа сохраняются, а workbook adapter получает exact model-owned `decisions`. Grounded cache выключен; пустой retrieval не создаёт кодовый `NO_DATA`. Evidence/source-map входят раньше advisory memory и доступны в exact packet trace | `proxy/services/chat_evidence_application_service.py`, `proxy/services/{context_governor,typed_memory_projection,model_execution_preset,llm_transport_profile,model_research_tool}_service.py`, `proxy/routers/chat.py`, `retrieval_service`, `evidence_packet_service` | [ALGO-evidence-packet.md](ALGO-evidence-packet.md) · [ALGO-context-memory.md](ALGO-context-memory.md) · [design](superpowers/specs/2026-08-30-model-owned-evidence-first-rag-design.md) | ✅ |
