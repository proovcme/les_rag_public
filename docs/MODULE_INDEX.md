...
| smeta/gesn | норма ГЭСН → ресурсы (расход×объём); canonical machine base `data/smeta_base/les_smeta_base.sqlite` (`norms/resources`), raw/unified parquet только source/staging; update pipeline `raw/cache → unified parquet → SQLite → SMETA_SERVICE cards` | `gesn_service`, `gesn_update_service`; `tools/gesn_update_from_fgis.py`, `tools/gesn_unify_base.py`, `tools/build_smeta_structured_base.py`, `tools/build_smeta_service_rag.py`; Make `smeta-base*`; `GET /api/lsr/gesn[/{code}/expand]`, `POST /api/service-sources/gesn_base/fgis-update`; GUI «Инструменты → ГЭСН → скачать/обновить»; MCP `les_gesn_*` | [ALGO-gesn.md](ALGO-gesn.md) | ✅ |
...
| Суб-модуль | Назначение | Точки входа | Док | Статус |
|---|---|---|---|---|
| rag/core | поток чата, ретрив, C-RAG, source-map/latency trace, диспетч; с 0.24.0.287 валидатор/TOSKA в `/api/chat` маркирует качество, но не заменяет непустой модельный ответ generic fallback | `retrieval_service`, `saferag_service`, `runtime_dispatcher`/`runtime_admission`; `routers/chat.py` | [ARCHITECTURE_les_algorithm.md](ARCHITECTURE_les_algorithm.md) ✅ · [STORY_les_dispatcher.md](STORY_les_dispatcher.md) ✅ · [CODE_MAP.md](CODE_MAP.md) ✅ | ✅ |
...
| rag/notebook-study | NotebookLM-подобное чтение проекта: broad-запрос (`расскажи про проект`, `что это за датасет/проект`) → best-effort model reader-pass над картой датасета → адаптивный reading plan по блокноту → параллельный section retrieval + точечный `doc_filter`-проход по паспортным файлам из memory/inventory → модельная сводка + artifact с источниками/пробелами; не deterministic final | `notebook_study_service`; `dataset_memory_service`; `project_summary_service.is_project_inventory_query`; `routers/chat.py`; payload `notebook_context`, `notebook_context.targeted_files`, `project_inventory`, `artifact`, `retrieval_trace.dataset_reader_prepare` | [ALGO-notebook-study.md](ALGO-notebook-study.md) | ✅ |
...
| project/pdf-extract / Л.И.С.Т. | Единый source-map PDF-проекта без reindex: оркестрирует `drawing_manifest_v1`, `pd_rd_manifest_v1`, shared `project_pdf_table_manifest_v1` и дисциплинные readers в sidecar `storage/datasets/{dataset_id}/_les_pdf_extract`, отдаёт `project_pdf_extract_v1` с составом томов/ПЗ/ВОР/СО/таблицами/схемами как навигацией для модели; не evidence и не готовый ответ. Пользовательское имя слоя — **Л.И.С.Т. · Локальный индекс структуры томов**. Shared table layer извлекает ОВ `ХВС`/характеристики воздушных систем, ВК водные балансы и экспликации помещений `{номер, имя, площадь, категория}` на графических листах, а также compact `project_pdf_table_type_candidate_v1` для всех найденных PDF-таблиц: КР расчёты/арматура/грунты/сечения, ООС шум/выбросы/отходы, ПБ эвакуация/АУПТ/токопотребление/огнестойкость, ОВ теплопотери/воздухообмен, ИОС automation/слаботочка/спецификации и `SERVICE`/`NAV`/`NOISE`/`TEXT` группы | `project_pdf_extract_service`, `project_pdf_table_service`; `/api/rag/datasets/{dataset_id}/pdf-extract/{status,run,summary}`; `dataset_memory_service.project_pdf_extract`; `sovushka/pages/documents.py` panel; tests `test_project_pdf_table_service.py` | [ALGO-pdf-ingestion.md](ALGO-pdf-ingestion.md) · [ALGO-electrical-schematics.md](ALGO-electrical-schematics.md) | ✅ |
