...
| core/tool-harness | controlled registry/executor для инструментов: карта датасета, source search/read, PDF/Excel readers, bounded vision и read-only filesystem. В обычном grounded chat `ModelResearchToolService` исполняет `search_sources` тем же production native RRF и не позволяет модели расширить операторский dataset scope; цикл останавливает модель (`calls: []`) или технический deadline, без semantic call dedupe и кодовой оценки ответа | `proxy/services/{tool_contract_service,tool_registry_service,capability_broker_service,trusted_executor_service,tool_harness_service,model_research_tool_service}.py`, `proxy/routers/tools.py`, `tools/les_tool_harness.py`, `routers/chat.py` | [ALGO-tool-harness.md](ALGO-tool-harness.md) · [CHAT_OPERATOR_CONTRACT.md](CHAT_OPERATOR_CONTRACT.md) | ✅ |
...
| normcontrol/formal-v1 | внутренние формальные NK-01..04 (форматы ГОСТ, шифры, ведомость), переиспользуемые активным doc-review без параллельного API | `normcontrol_service`; `/api/doc-review` | [ALGO-normcontrol.md](ALGO-normcontrol.md) | ✅ |
