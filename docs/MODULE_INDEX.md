...
| Суб-модуль | Назначение | Точки входа | Док | Статус |
|---|---|---|---|---|
| smeta/core | model-first PDF/XLSX→ЛСР: общий `SmetaNormToolSession` исполняет catalog/search/read/submit; provenance gates для unbound/bind. Local Ollama/Qwen: soft-accept incomplete evidence → `precalculation_blockers`; fresh-run stability via `LES_SMETA_DOCUMENT_SEED` + stable candidate/RRF order + floating close-analog reject + hard demote `ГЭСНр`/`ГЭСНмр` без repair-intent в строке ВОР (unbound, без подмены нормы) + `mapping_fingerprint` + coverage gate (низкое покрытие → «сумма только по привязанным») + missing-rows pass (второй model-проход только по unbound, bound immutable); retry Ollama tool-XML HTTP 500 + mapping JSON retries (`MAPPING_RETRIES`, seed/think=false) + soft-unbound fallback при битом JSON (без выбора нормы); batch=1, turns=6, global review opt-in. Cloud: hard-reject + global review. Env: `LES_SMETA_DOCUMENT_SOFT_ACCEPT`/`SEED`/`MISSING_PASS`/`XML_PARSE_RETRIES`/`MAPPING_RETRIES`/`GLOBAL_REVIEW`/`MAX_TOOL_TURNS`/`BATCH_SIZE` | `proxy/smeta_core/{application,contracts,integrity,norm_browser,source_intake,document_workflow,professional_review,calculator,resource_normalizer,lsr_renderer,workflow,unit_contract}.py`, `proxy/services/{smeta_agent_runner_service,smeta_chat_application_service,smeta_chat_adapter_service,prompt_registry_service,rim_lsr_trace_service,rim_trace_xlsx_service,smeta_user_message_service,fsem_machinist_service}.py`, `proxy/routers/{chat,chat_history,settings}.py`, `sovushka/{components/header.py,pages/chat.py}`, `tools/{smeta_agent_benchmark,smeta_mapping_quality}.py`, `skills/smeta/{SKILL.md,references/document-mapping-agent.md,references/gesn-storage.md}`, `scripts/windows/LES-START.ps1` | [SMETA_MODULE_EXPLAINED.md](SMETA_MODULE_EXPLAINED.md) · [modules/smeta-core.md](modules/smeta-core.md) · [RELEASE_LEDGER.md](RELEASE_LEDGER.md) | ✅ |
...
| smeta/ontology | доменные понятия (ВОР/КАЦ/ЛСР/КС/КС-6а) | `smeta_ontology_service`; MCP `les_glossary` | [ALGO-smeta-ontology.md](ALGO-smeta-ontology.md) ✅ · [smeta_ontology.md](smeta_ontology.md) ✅ (генерится) | ✅ |
| smeta/ks-forms | заполненные КС-2/КС-3 из последней ЛСР (в т.ч. document-workflow `rim_trace`/`lsr` и compact `positions` sidecar); КС-2 XLSX — официальная шапка Госкомстат № 100 (как HTML-бланк); скачивание с `.xlsx` (не `.txt`); КС-6а из confirmed field_intake; blank бланки без source; чат `/кс-2`/`/кс-3`/`/кс-6а` и NL «собери КС-…»; код не выбирает нормы | `ks_forms_service` (`assembled_from_rim_trace`), `ks2_xlsx_render`, `ks_forms_chat_service`, `forms_service.generate(rows=)`, `les_action_service.save_smeta` (ks2/ks3), `smeta_chat_application_service` (KS sidecar после PDF/LSR), `command_service`, `routers/{forms,chat}.py`, `config/forms/{ks2,ks3,ks6a}.yaml`; MCP `les_form_generate`/`les_smeta_save` | [smeta_ontology.md](smeta_ontology.md) · [SKILL.md](../SKILL.md) · [RELEASE_LEDGER.md](RELEASE_LEDGER.md) | ✅ |
