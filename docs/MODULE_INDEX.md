...
| core/model-execution-presets | Capacity-bounded фабричные пресеты Qwen: неизвестная identity/capacity fail-closed к 9B, 35B требует наблюдаемой ёмкости; operator/profile могут только сужать лимиты. Диагностика показывает redacted requested/effective/source и не меняет route mode или backend | `proxy/services/model_execution_preset_service.py`, `proxy/services/llm_transport_profile_service.py`, `proxy/services/version_service.py` | [modules/chat-profiles.md](modules/chat-profiles.md) · [canonical design](superpowers/specs/2026-08-26-canonical-tool-context-memory-update-design.md) | ✅ |
