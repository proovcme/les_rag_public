...
| rag/retrieval | exact/FTS/dense/native hybrid/rerank с раздельными score scales; dense требует совместимый `les.rag.index-contract.v1`, retry сохраняет backend. В сметном shortlist cross-encoder является вторым сигналом: его порядок объединяется RRF с исходным typed+dense+sparse ranking, поэтому технически исправный, но слабый rerank не стирает сильную голову retrieval. После rerank общий format-neutral identifier guard поднимает уже найденный фрагмент с полным буквенно-цифровым обозначением с дефисами, не добавляя кандидатов или доменных весов. Debug endpoint является точной проекцией chunks | `retrieval_service`, `retrieval_quality_service`, `lexical_index_service`, `backend/rag_config`, `backend/qdrant_adapter`, `backend/reranker`, `proxy/smeta_core/norm_browser.py`, `routers/datasets.py` | [ADR-12-typed-retrieval.md](ADR-12-typed-retrieval.md) · [ALGO-rag-best-practices.md](ALGO-rag-best-practices.md) | ✅ |
...
| ops/versioning | единый `config/version.json`: пользовательский SemVer, отдельный монотонный номер сборки и внутренняя Tauri/NSIS-версия; `make version-sync` один раз обновляет все машинные поверхности, включая корень `package-lock.json`, а `make verify` ловит drift; `/api/version` сохраняет совместимые старые поля и показывает repo↔runtime divergence. Patch-release после Mac/изолированного Windows PDF+RRF gate автоматически ставит тот же EXE в production Legion без изменения пользовательского RAG, проверяет ручной Outlook-сборщик, закрывает deploy SSH-сессию и отдельным новым соединением проверяет живые API/UI и desktop-процесс до публикации | `version_service`; `tools/sync_version_contract.py`; `tools/{patch_release.py,windows_patch_release.ps1,windows_production_deploy.ps1}`; `GET /api/version`; `make version-sync`; `make patch-release` | [RELEASE_LEDGER.md](RELEASE_LEDGER.md), [VERSIONING.md](VERSIONING.md), [SOFTWARE_VERSIONS.md](SOFTWARE_VERSIONS.md), [releases.md](releases.md) | ✅ |
...
| ops/test | `make test`, `test-release`, `test-architecture` и collect-only `verify` используют одну каноническую LES-сюиту (2752 collected на 2026-07-28; 2743 passed / 9 skipped): 11 файлов feature-off Unified/Construction Harness и `test_artel*` исключены. Архивный harness доступен только явно через `make test-legacy`. `test-mail` изолирует 63 offline/static проверки Е.Ж.И.К., `test-mail-release` добавляет Tauri и обязателен для выпуска. Hosted `platform-gate` выполняет переносимые unit/integration, реальный smeta-baseline, проверки установщика текущей ОС и нативную Tauri-сборку на macOS/Windows; Windows дополнительно компилирует и self-test-ит реальный Outlook sidecar. Полная сюита остаётся локальным release-гейтом, а installed Legion smoke проверяет то, чего нет на hosted runner. `smoke-basic-release`, `ship` и post-deploy делают P1 блокером. Изолированный smoke доказывает clean-install smeta baseline (≥40 000 норм, ≥1 500 ФСЭМ), API/UI/Qdrant, контрольный PDF и временный `dense + qdrant_sparse → RRF`; production smoke не индексирует проектные PDF и проверяет установленный ручной Е.Ж.И.К., совместимый RAG-контракт и desktop до публикации | `Makefile`; `.github/workflows/{verify,release}.yml`; `tests/test_test_profiles.py`; `test-mail[-release]`; `tools/{platform_release_gate,multiplatform_release,patch_release}.py`; `tools/{windows_release_smoke,windows_patch_release,windows_production_deploy}.ps1` | [TEST_INVENTORY.md](TEST_INVENTORY.md) · [TEST_ARCHITECTURE_AUDIT_2026-07-14.md](TEST_ARCHITECTURE_AUDIT_2026-07-14.md) | ✅ |
| install | Tauri 2 — канонический Mac/Windows shell над NiceGUI; **production target — Legion/Windows**, Mac — dev/reference. Hosted CI на каждом PR/main запускает переносимый поведенческий/артефактный профиль и native Tauri build на `macos-14`/`windows-2022`. Approved production workflow на self-hosted Mac собирает и проверяет DMG, затем через Legion строит/устанавливает/проверяет NSIS и публикует обе платформы одной release-операцией. Mutable state остаётся вне replaceable app tree; Windows bundle получает проверенные Python/uv и immutable ФГИС/ФСНБ smeta baseline, но не пользовательский RAG. Отсутствующие Ollama/Docker/Qdrant/модели остаются `setup_required`; живой gate проверяет terminal ready, API/UI и native RRF. | `.github/workflows/{verify,release}.yml`; `desktop/tauri`; `tools/{platform_release_gate,multiplatform_release,patch_release,build_tauri_app}.py`; `tools/windows_{release_smoke,production_deploy}.ps1` | [INSTALL_RUNBOOK.md](INSTALL_RUNBOOK.md) ✅, [PLATFORMS.md](PLATFORMS.md) ✅, [CODE_MAP.md](CODE_MAP.md) | ✅ |
