...
| ops/vps-patch | Windows application updater v2 заменяет повседневный полный installer bounded ZIP-пакетом. Runtime allowlist дополнен только четырьмя exact lifecycle-скриптами; отдельный `scope=app` допускает ровно один `les-desktop.exe`. Windows shell-builder делает Cargo-only build без NSIS/baseline и attestation exact commit/version/build + base/target SHA; package builder отклоняет произвольный или чужой EXE. Helper работает через `pythonw.exe`, скрывает PowerShell/taskkill, до остановки проверяет внешний и внутренний manifest, archive/file SHA и точный состав ZIP; затем backup→stop→atomic replace→точечный py_compile→deploy stamp→restart. Success требует commit/version/build, API/UI/index contract, direct Python PID и 0 LES-owned `cmd.exe`; ошибка полностью откатывает существующие/новые файлы и stamp, user state не входит в transaction. Prepare запускает только короткий `make test-updater`; apply не запускает tests/build/baseline. Legion пока не затрагивается | `tools/{windows_update_shell,vps_patch,vps_patch_apply}.py`; `tools/windows_updater_smoke.ps1`; `proxy/services/update_service.py`; `proxy/routers/updates.py`; `sovushka/components/header.py`; `make test-updater/build-windows-update-shell/prepare-windows-update` | [VPS_PATCH_CHANNEL.md](VPS_PATCH_CHANNEL.md) ✅ | ✅ |
