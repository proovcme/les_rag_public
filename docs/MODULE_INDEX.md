...
| rag/retrieval | production contract v2: exact readers отдельно; candidate retrieval только `dense + bm25_sparse → native RRF`, затем обязательный общий rerank и parent/context expansion. Reranker определяется только runtime-политикой: legacy `reranker_enabled` принимается для совместимости, но клиентский override игнорируется. Native/Rerank failure даёт `status=blocked` без legacy dense/FTS fallback; явный или inferred scope не расширяется при отсутствии датасета. Weak retry сохраняет исходный embedding query и может только расширить пул. Trace аддитивно показывает `status/error_code/resolved_dataset_ids/scope_source` и mandatory reranker policy; SQLite FTS остаётся safety-каналом после успешного RRF | `retrieval_service`, `retrieval_quality_service`, `lexical_index_service`, `chat_evidence_application_service`, `backend/rag_config`, `backend/qdrant_adapter`, `backend/reranker` | [ADR-12-typed-retrieval.md](ADR-12-typed-retrieval.md) · [ALGO-rag-best-practices.md](ALGO-rag-best-practices.md) | ✅ |
