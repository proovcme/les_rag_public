...
| ops/vps-patch | Windows application updater v2 заменяет повседневный полный installer bounded ZIP-пакетом. Runtime allowlist содержит пять exact lifecycle-скриптов, включая общий bounded `runtime-process.ps1`; отдельный `scope=app` допускает ровно один `les-desktop.exe`. Windows shell-builder делает Cargo-only build без NSIS/baseline и attestation exact commit/version/build + base/target SHA; package builder отклоняет произвольный или чужой EXE. Helper работает через `pythonw.exe`, скрывает PowerShell/taskkill, до остановки проверяет внешний и внутренний manifest, archive/file SHA и точный состав ZIP; затем backup→stop→atomic replace→точечный py_compile→deploy stamp→restart. Bootstrap/deploy/rollback ждут exact PID с timeout и получают настоящий exit code; быстрый `netstat` исключает тяжёлый WMI/CIM port scan. Success требует commit/version/build, API/UI/index contract, direct Python PID и 0 LES-owned `cmd.exe`; ошибка полностью откатывает существующие/новые файлы и stamp, user state не входит в transaction. Prepare запускает только короткий `make test-updater`; apply не запускает tests/build/baseline. Последний полный bootstrap-installer принят на Legion; дальше обычные изменения идут application package | `installers/windows/runtime-process.ps1`; `tools/windows_bootstrap_updater_v2.ps1`; `tools/{windows_update_shell,vps_patch,vps_patch_apply}.py`; `tools/windows_updater_smoke.ps1`; `proxy/services/update_service.py`; `proxy/routers/updates.py`; `sovushka/components/header.py`; `make test-updater/build-windows-update-shell/prepare-windows-update` | [VPS_PATCH_CHANNEL.md](VPS_PATCH_CHANNEL.md) ✅ | ✅ |
...
| ops/test | Общая 2808-test suite сохранена только как явная диагностика и запрещена в updater-контуре до переработки тестовой архитектуры. `make test-updater` — отдельный hermetic Mac/Windows behavior-гейт: shell attestation, manifest/SHA, apply/backup/rollback, новый и существующий файл, повторная попытка, `les-desktop.exe`, user-state boundary и process hygiene; он не запускает NSIS/baseline/общую suite. Prepare вызывает этот короткий гейт один раз, apply не вызывает тесты. Нативный `windows_updater_smoke.ps1` ограничен 90 секундами и проверяет установленный identity/API/UI/index/process contract без RAG-запроса | `Makefile`; `tests/{test_vps_patch,test_windows_application_update,test_windows_update_shell,test_mac_update}.py`; `tools/windows_updater_smoke.ps1` | [TEST_INVENTORY.md](TEST_INVENTORY.md) · [VPS_PATCH_CHANNEL.md](VPS_PATCH_CHANNEL.md) | ✅ |
| install | Tauri 2 — канонический Mac/Windows shell над NiceGUI; **production target — Legion/Windows**, Mac — dev/reference. Windows shell собирается как GUI subsystem, держит named single-instance + один lifecycle action, скрывает все Rust child-команды через `CREATE_NO_WINDOW`; proxy/UI/Lemonade запускаются прямыми Python PID без `cmd.exe`/`uv` wrappers. Updater повторно использует уже проверенный runtime при desktop handoff вместо второго bootstrap. Hosted CI на каждом PR/main запускает переносимый поведенческий/артефактный профиль и native Tauri build на `macos-14`/`windows-2022`. Approved production workflow на self-hosted Mac собирает и проверяет DMG, затем через Legion строит/устанавливает/проверяет NSIS и публикует обе платформы одной release-операцией. Mutable state остаётся вне replaceable app tree; Windows bundle получает проверенные Python/uv и immutable ФГИС/ФСНБ smeta baseline, но не пользовательский RAG. Отсутствующие Ollama/Docker/Qdrant/модели остаются `setup_required`; живой gate проверяет terminal ready, API/UI, native RRF, один desktop, direct Python process contract и отсутствие LES-owned `cmd.exe`. | `.github/workflows/{verify,release}.yml`; `desktop/tauri`; `installers/windows/{start-light,state}.ps1`; `tools/{platform_release_gate,multiplatform_release,patch_release,build_tauri_app}.py`; `tools/windows_{release_smoke,production_deploy,production_rollback}.ps1` | [INSTALL_RUNBOOK.md](INSTALL_RUNBOOK.md) ✅, [PLATFORMS.md](PLATFORMS.md) ✅, [CODE_MAP.md](CODE_MAP.md) | ✅ |
