...
| rag/document-explorer | no-AI просмотр и поиск по датасетам/документам: список датасетов, документы, чанки, FTS/LIKE поиск по корпусу или одному файлу; база для UI-проводника и WebDAV | `document_explorer_service`, `routers/documents.py`; tests `test_document_explorer_service.py` | [CODE_MAP.md](CODE_MAP.md) | ✅ |
