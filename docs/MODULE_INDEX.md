...
| ops/test | verify/test-release-critical/test-release/test-architecture/test разделены по смыслу. Patch-release обязан прогнать полный `test-release` без отдельного ARTEL, затем Windows live smoke установленного runtime. Изолированный smoke доказывает clean-install smeta baseline (≥40 000 норм, ≥1 500 ФСЭМ), запуск ФСНБ-job, API/UI/Qdrant и временный `dense + qdrant_sparse → RRF`; production smoke после автоматической установки требует `4/4` тяжёлых PDF из Legion-полигона, ненулевые фрагменты и native RRF, затем удаляет временный dataset. Только после двух Windows gate сверяется SHA и разрешается публикация | `Makefile`; `test-release`; `tools/smeta_release_baseline.py`; `tools/patch_release.py`; `tools/{windows_release_smoke,windows_patch_release,windows_production_deploy}.ps1` | [TEST_INVENTORY.md](TEST_INVENTORY.md) · [TEST_ARCHITECTURE_AUDIT_2026-07-14.md](TEST_ARCHITECTURE_AUDIT_2026-07-14.md) | ✅ |
