...
| intake/mail / Е.Ж.И.К. | read-only полный сбор classic Outlook + multi-account IMAP; один ящик = отдельный P0 dataset; exact registry/cursors/snapshot, `.msg/.eml` message+attachment nodes и Windows interactive sidecar. Письма читаются в рабочей вкладке «Почта», а подключения/синхронизация вынесены в отдельную admin-вкладку «Настройка почты». Outlook cursor фиксирует завершённый backfill и не перечитывает историю на каждом poll; проход ограничен 10 снимками/12 секундами и запускается раз в 10 минут. Loopback intake подтверждает долговечный снимок до отдельной последовательной RAG-очереди, а parser стартует после её опустошения и не удерживает Outlook. Windows gate компилирует sidecar и проверяет cursor persistence; legacy `/push` сохранён | `mail_registry_service`, `mail_sync_service`, `mail_profile`; `proxy/routers/mail.py` (`_queue_outlook_upload`, `_drain_outlook_uploads`); `clients/outlook_mail_poller`; `tools/platform_release_gate.py`; `/api/mail/accounts*`, `/collector/import`, `/messages/{id}/open`; `sovushka/pages/mail.py` (`build_mail`, `build_mail_settings`) | [ALGO-mail-intake.md](ALGO-mail-intake.md) | ✅ |
