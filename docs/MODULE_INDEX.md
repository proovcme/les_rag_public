...
| core/tool-harness | controlled registry/executor для инструментов: карта датасета, indexed source search/read, PDF/Excel readers, bounded vision страницы/области исходного PDF и read-only filesystem; PDF vision разрешает видимое имя до registry/source_path и отдельно читает общий лист, штамп и легенду через Gemma; `TrustedExecutor` проверяет schema/scope/role/deadline/idempotency/approval/result budget и возвращает `les_tool_execution_v1` с вложенным `les_tool_result_v1`; общий чат умеет model-selected loop shortlist→JSON calls→executor→model final | `proxy/services/{tool_contract_service,tool_registry_service,capability_broker_service,trusted_executor_service,tool_harness_service}.py`, `proxy/routers/tools.py`, `tools/les_tool_harness.py`, `routers/chat.py` | [ALGO-tool-harness.md](ALGO-tool-harness.md) · [CHAT_OPERATOR_CONTRACT.md](CHAT_OPERATOR_CONTRACT.md) | ✅ |
...
| core/model-execution-presets | Capacity-bounded фабричные пресеты Qwen: неизвестная identity/capacity fail-closed к 9B, 35B требует наблюдаемой ёмкости; operator/profile могут только сужать лимиты. Redacted requested/effective/source виден в version API и GUI-first runtime registry; resolver не меняет route/backend | `proxy/services/model_execution_preset_service.py`, `proxy/services/llm_transport_profile_service.py`, `proxy/services/version_service.py`, `proxy/services/runtime_config_registry_service.py`, `sovushka/pages/diag.py` | [modules/chat-profiles.md](modules/chat-profiles.md) · [modules/sovushka-uikit.md](modules/sovushka-uikit.md) | ✅ |
| core/context-governor | Единый budget-aware packer inference: сначала generation/safety reserves и обязательные profile/request, затем целые typed objects в каноническом порядке; omission возвращает IDs/cursor, required overflow блокирует model call. Ordinary chat использует governor для tool-decision и answer; legacy fitter остаётся compatibility adapter без mid-object slicing | `proxy/services/context_governor_service.py`, `proxy/services/llm_transport_profile_service.py`, `proxy/services/chat_evidence_application_service.py` | [ALGO-context-memory.md](ALGO-context-memory.md) · [canonical design](superpowers/specs/2026-08-26-canonical-tool-context-memory-update-design.md) | ✅ |
