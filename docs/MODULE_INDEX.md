...
> **0.30.42 estimator chat packaging/UI hotfix:** «Собери ЛСР» пакует только
> ЛСР, «Собери ВОР» — только ВОР; имя файла `LSR_`/`VOR_` + дата, не
> `artifact.xlsx`. В пузыре всегда дата и время; карточки источников не
> наезжают и показывают шифр/наименование. Итоговая таблица строк в чате
> берётся из ответа модели (`draft_rows`), без возврата `document_workflow`.
> Точки: `proxy/services/{chat_evidence_application,workbook_tool}_service.py`,
> `proxy/routers/artifacts.py`, `sovushka/{pages/chat,answer_render,styles,uikit/tokens,state}.py`.
> Статус док↔код: ✅.
