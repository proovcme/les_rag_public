...
| install | сборка/инсталляторы Mac/Win/Linux; Windows-light выбирает свободные proxy/UI порты и передаёт их shell через state-файл | `tools/build_*`, `installers/`, `tools/les_shell.py` | [INSTALL_RUNBOOK.md](INSTALL_RUNBOOK.md) ✅, [PLATFORMS.md](PLATFORMS.md) ✅, [CODE_MAP.md](CODE_MAP.md) | 🟡 |
