...
| Суб-модуль | Назначение | Точки входа | Док | Статус |
|---|---|---|---|---|
| smeta/core | model-first PDF/XLSX→ЛСР: общий `SmetaNormToolSession` исполняет catalog/search/read/submit, показывает модели typed identity, scope, unit compatibility, operations/resource preview и matched query. `candidate_evaluations` сохраняет модельное сравнение открытых норм; Python проверяет только форму/provenance. `SmetaAgentRunner` даёт одинаковый skill движкам native, Qwen-Agent/Ollama и Google ADK/Gemini. Qwen работает по одной активной строке; native Ollama/Qwen также получает однострочные transport-пакеты, но обязательная global-review сохраняется. Принятые пакеты атомарно checkpoint'ятся по SHA исходника; retry продолжает только оставшиеся `work_id`. Structured mapping ограничен 8 строками на JSON-вызов и после timeout не повторяет идентичный payload. Terminal mapping сразу показывается через SSE `smeta_row`. Ordinary-text конец получает same-model terminal recovery; structured Ollama JSON имеет один bounded no-think retry и один schema-repair, без code-generated tool calls. `unbound_evidence` сверяется и нормализуется только по реальной tool trajectory; причины отказа и coverage код не дописывает. После строк conflict-only validator предъявляет противоречия, а та же модель создаёт immutable global-review revision по compact card-map с lazy reread спорных норм. Код нормы не исправляет: автoрасчёт `priced_draft`, пользовательский lock создаёт отдельную финальную ревизию. ETM adapter читает только явно заданные коды и возвращает provenance-bearing КАЦ quotes; материал по API не выбирает. Search/read/card/time budgets и expert metrics находятся в `professional_review.py`; `smeta_mapping_quality.py` готовит human annotation queue и считает только завершённую разметку. Google требует явное согласие и key, без fallback; default остаётся native до профессионального golden-гейта | `proxy/smeta_core/{application,contracts,integrity,norm_browser,source_intake,document_workflow,professional_review,calculator,resource_normalizer,lsr_renderer,workflow,unit_contract}.py`, `proxy/services/{smeta_agent_runner_service,smeta_chat_application_service,smeta_chat_adapter_service,etm_price_service,prompt_registry_service,rim_lsr_trace_service,rim_trace_xlsx_service,smeta_user_message_service,fsem_machinist_service}.py`, `proxy/routers/{chat,chat_history,prices,settings}.py`, `sovushka/{components/header.py,pages/chat.py}`, `tools/{smeta_agent_benchmark,smeta_document_local_run,smeta_mapping_quality}.py`, `skills/smeta/{SKILL.md,references/document-mapping-agent.md,references/gesn-storage.md}` | [SMETA_MODULE_EXPLAINED.md](SMETA_MODULE_EXPLAINED.md) · [modules/smeta-core.md](modules/smeta-core.md) | ✅ |
| smeta/rim-dialog | persistent диалоговая РИМ-сессия: XLSX/CSV intake, ВОР, один технический вопрос с вариантами, строгий model-selected scoped batch `catalog→search→read→submit`, durable resume после каждого tool result, read-only построчная progress-проекция checkpoint и authoritative resume-budget/evidence status. Lifetime evidence остаётся в аудите, каждый resume получает bounded execution slice. Повтор после search/read сводится к model-owned сериализации; catalog-stall до evidence очищает только active conversation до task prefix + compact latest-catalog working memory, сохраняя полный event audit. Новый model-authored `catalog_query` может уточнить shortlist того же семейства после отвергнутой первой книги. `bind` разрешён schema только по открытым typed-карточкам с совместимой единицей, а внутренне противоречивый `exact` сохраняется как блокирующий conflict. Успешная mapping revision содержит compact `agent_audit`, поэтому очистка checkpoint не удаляет историю инструментов. Model-visible паспорта пяти семейств ФСНБ, model-query-driven reranked shortlist до 6 сборников без скрытого query expansion, отдельные base/collection решения с причиной и явное batch-применение подтверждённого scope. Typed-проверка таблицы и phase-scoped реестр источников коэффициентов/цен/ФСЭМ/КАЦ/НР/СП. Transport timeout не расходует schema-repair; exact string booleans нормализуются только по tool schema. Project source identity наследуется из parent VOR; новая производная работа обязана ссылаться на известную строку родителя и не может заменить её нормативной ссылкой. Далее immutable mapping/global review/two user locks, authored scenarios, canonical РИМ-расчёт, typed requirements, audit/XLSX и lazy NiceGUI workbench с восстановлением сессии и live-обновлением progress раз в пять секунд. RAG-card и реестр источников только навигация; расчёт допускает лишь полностью открытую карточку structured store. Закрытие КАЦ/коэффициента требует пересчёта трассы | `config/domain/smeta_normative_catalog.json`, `proxy/smeta_core/{norm_browser,rim_session,document_workflow}.py`, `proxy/services/{rim_knowledge_service,rim_agent_action_service,rim_agent_turn_service,rim_mapping_progress_service,rim_mapping_review_service,rim_mapping_xlsx_service,rim_scenario_service,rim_session_xlsx_service}.py`, `proxy/routers/rim.py`, `sovushka/pages/rim.py`; `/api/rim/sessions/*`, `GET /mapping/progress` | [modules/rim-dialog-session.md](modules/rim-dialog-session.md) | ✅ |
...
| rag/retrieval | production contract v2: exact readers отдельно; candidate retrieval только `dense + bm25_sparse → native RRF`, затем обязательный общий rerank и parent/context expansion. Reranker определяется только runtime-политикой: legacy `reranker_enabled` принимается для совместимости, но клиентский override игнорируется. Wide RRF recall сохраняется, а дорогой cross-encoder получает bounded shortlist (`candidate_limit`; Windows default 16), после которого исходный RRF-хвост не теряется. Native/Rerank failure даёт `status=blocked` без legacy dense/FTS fallback; явный или inferred scope не расширяется при отсутствии датасета. Weak retry сохраняет исходный embedding query и может только расширить пул. Trace аддитивно показывает `status/error_code/resolved_dataset_ids/scope_source`, mandatory reranker policy и `pool_count/candidate_limit/input_count`; SQLite FTS остаётся safety-каналом после успешного RRF | `retrieval_service`, `retrieval_quality_service`, `lexical_index_service`, `chat_evidence_application_service`, `backend/rag_config`, `backend/qdrant_adapter`, `backend/reranker` | [ADR-12-typed-retrieval.md](ADR-12-typed-retrieval.md) · [ALGO-rag-best-practices.md](ALGO-rag-best-practices.md) | ✅ |
| rag/document-explorer | Л.И.С.Т. — read-only Document Explorer поверх пользовательского RAG. Рабочая вкладка «Документы» показывает выбор датасета/файла, фактически извлечённые фрагменты и одну явную кнопку открытия оригинала; технические паспорта, карта, PDF-контур и index-quality остаются backend/operator API, а не первым экраном просмотра. Qdrant/LES хранит проверяемые фрагменты и не генерирует ответ. Старые MetaDB без `lexical_chunks[_fts]` получают additive schema migration без reindex | `document_explorer_service`, `pdf_contour_service`, `lexical_index_service`, `backend/qdrant_adapter.py`, `routers/documents.py`, `sovushka/pages/documents.py`, `sovushka/styles.py`; endpoints `GET /api/documents/datasets/{id}/quality`, `GET /api/documents/by-id/{doc_id}/chunks`, `GET /api/rag/datasets/{id}/integrity`, `POST /api/documents/by-id/{doc_id}/open-native`; tests `test_document_explorer_service.py`, `test_pdf_contour_service.py`, `test_dataset_integrity.py`, `test_static_assets.py` | [CODE_MAP.md](CODE_MAP.md) · [modules/pdf-contour.md](modules/pdf-contour.md) | ✅ |
...
| ops/vps-patch | Windows имеет два ручных режима поверх одного lifecycle: hard install целиком заменяет дерево программы через verified installer payload, soft package атомарно меняет bounded allowlist файлов. `windows_update_engine.py` до остановки проверяет SHA/identity и persistent venv, затем stop→same-volume rename старого app в recovery→silent NSIS→state junctions→start→поэтапный identity/API/UI/Qdrant/index/process smoke с двумя стабильными пробами; провал удаляет новое дерево и возвращает старое одним rename. `%LOCALAPPDATA%\LES` никогда не входит в удаляемую область. `windows_runtime.py` запускает exact console-free Python PID без PowerShell/cmd, имеет memory cap и до чтения блокирует `.env` больше 1 МБ; `windows_env_doctor.py` атомарно изолирует и восстанавливает повреждённый env без вывода значений. Installed runtime без `.git` не запускает Git при `/api/version`; допустимый dev-probe на Windows скрыт через `CREATE_NO_WINDOW`. Status JSON публикуется atomic replace с bounded retry, поэтому одновременное чтение UI не вызывает ложный rollback. Soft helper использует те же start/stop/smoke primitives. Apply не запускает tests/build/baseline/network sync и не использует WMI/CIM; stdout/stderr exact child PID сразу пишутся в файлы с timeout. PowerShell production/bootstrap entrypoints — только тонкие алиасы Python engine, standalone rollback закрыт. Настройки разделяют быстрое обновление и переустановку выпуска | `tools/{windows_runtime,windows_env_doctor,windows_update_engine,vps_patch_apply,vps_patch,windows_update_shell}.py`; `proxy/services/{update_service,version_service}.py`; `proxy/routers/updates.py`; `sovushka/components/header.py`; `installers/windows/{state,start-light,stop-light}.ps1`; `make test-updater/prepare-windows-update` | [VPS_PATCH_CHANNEL.md](VPS_PATCH_CHANNEL.md) ✅ | ✅ |
