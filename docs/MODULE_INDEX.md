...
**✅ исправлено (437f1aa):** ALGO-smeta/object — два шаблона (wooden_house + monolith_office); ALGO-gesn — v2-слой базы; smeta_ontology.yaml «Прил.3»→«Прил.4», `smeta_ontology.md` регенерирован. В 0.24.0.12 явный smeta-режим получил детерминированное состояние параметров по истории текущей сессии, парсер понимает разговорные площадь/этажность, а неподдержанные варианты вроде свай/крыльца/плоской кровли не скрываются в расчёте, а выходят warning. В 0.24.0.13 smeta-режим использует `retrieval_trace` прошлых tool-расчётов для продолжений, показывает кандидаты ГЭСН для привязки и не светит внутренние refs ставок/yaml как доказательства. В 0.24.0.14 объектная смета при отсутствии точного шаблона ищет ближайший локальный аналог, помечает `rough_analog_object_assumed` и удерживает диалоговый сценарий каркасной дачи без ручных подсказок модели. В 0.24.0.16 спорные части объектной сметы получили `composition_candidates`: ГЭСН-кандидаты из локальной базы для детализации, без автоматического включения в сумму.
...
| rag/context-memory | паспорт чата + metadata/deep паспорт датасета (`_les_dataset_profile.json`) + quality/warmup benchmark, как навигационный фон, НЕ evidence; UI-кнопка «Паспорт области» | `context_memory_service`; `GET /api/chat/memory/{session_id}`; `GET/POST /api/rag/datasets/{id}/profile*`; `POST /api/rag/datasets/profiles/warmup`; `POST /api/rag/datasets/profiles/benchmark`; `routers/chat.py`; `sovushka/pages/chat.py` | [ALGO-context-memory.md](ALGO-context-memory.md) | ✅ |
