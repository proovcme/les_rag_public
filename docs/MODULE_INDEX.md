...
| ops/versioning | единый `config/version.json`: пользовательский SemVer, отдельный монотонный номер сборки и внутренняя Tauri/NSIS-версия; `/api/version` сохраняет совместимые старые поля и показывает repo↔runtime divergence | `version_service`; `tools/patch_release.py`; `tools/windows_patch_release.ps1`; `GET /api/version`; `make patch-release` | [RELEASE_LEDGER.md](RELEASE_LEDGER.md), [VERSIONING.md](VERSIONING.md), [SOFTWARE_VERSIONS.md](SOFTWARE_VERSIONS.md), [releases.md](releases.md) | ✅ |
...
| ops/test | гейты разделены по смыслу: verify=синтаксис/collection, test-architecture=текущая архитектура без feature-off harness, test=полная регрессия вместе с историей, test-rag-core=RAG integrity; Windows release smoke запускает установленный runtime, ждёт terminal `ready`, проверяет API/UI/Qdrant и фактический `dense + qdrant_sparse → RRF → rerank`; patch-release дополнительно проверяет clean/pushed commit, изолированную установку EXE, SHA-256 и опубликованный asset | `Makefile`, `tools/basic_function_smoke.py`, `tools/windows_release_smoke.ps1`, `tools/patch_release.py`, `tools/windows_patch_release.ps1` | [TEST_INVENTORY.md](TEST_INVENTORY.md) · [TEST_ARCHITECTURE_AUDIT_2026-07-14.md](TEST_ARCHITECTURE_AUDIT_2026-07-14.md) | ✅ |
