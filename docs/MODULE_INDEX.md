...
| core/agent-foundation | Canonical 0.29 foundation: structural gate, provider-neutral Registry, deterministic CapabilityBroker и TrustedExecutor реализованы; existing handlers не копируются, Broker не видит текст вопроса, Executor fail-closed проверяет scope/authorization/exact approval и не исполняет эффекты в shadow; ordinary-chat route, ContextGovernor, typed memory и presets выполняются следующими checkpoint-задачами; rollout `legacy | shadow | active` fail-closed | `tools/architecture_contract_gate.py`, `proxy/services/{tool_contract_service,tool_registry_service,capability_broker_service,trusted_executor_service,tool_harness_service}.py`, `proxy/routers/tools.py` | [ALGO-tool-harness.md](ALGO-tool-harness.md) · [current architecture](CURRENT_ARCHITECTURE.md) · [canonical design](superpowers/specs/2026-08-26-canonical-tool-context-memory-update-design.md) | 🟡 |
