...
| Суб-модуль | Назначение | Точки входа | Док | Статус |
|---|---|---|---|---|
| smeta/core | model-first PDF→первая денежная ЛСР: модель получает skill/prompt и typed RAG tools, сама выбирает нормы/аналоги/coverage; код проверяет показанный код, единицы и арифметику, затем сразу считает и экспортирует. Resource/component/dominant review выполняются только после первой ЛСР и не являются gate. Source→norm quantity однозначен; ФСЭМ/ФГИС/КАЦ дают деньги или построчный gap; машинный контракт отделён от человеческого ответа | `proxy/smeta_core/{contracts,integrity,norm_browser,source_intake,document_workflow,calculator,lsr_renderer,workflow}.py`, `proxy/services/{prompt_registry_service,rim_lsr_trace_service,rim_trace_xlsx_service,smeta_user_message_service,fsem_machinist_service}.py`, `skills/smeta/{SKILL.md,references/gesn-storage.md}`, `run_vor_pdf_workflow`; invariant-тесты `tests/test_smeta_core.py` | [modules/smeta-core.md](modules/smeta-core.md) | ✅ |
...
С contract v2 native RRF становится обязательным общим путём, а не датасетным флагом: все текущие indexed datasets мигрируют через полный re-embed в clean sibling, будущий parse записывает только named dense+sparse points, collection/embedding/chunk/vector schema закреплены manifest. `tools/rag_rrf_readiness.py` блокирует activation без полного migration report, единого fingerprint, обоих vector channels и покрытия каждого dataset; legacy vector-copy migration отключена.
