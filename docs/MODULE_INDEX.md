...
| core/agent-foundation | Canonical 0.29 foundation: structural architecture gate уже реализован; общий Registry/Broker/Executor, ContextGovernor, typed memory и Qwen 9B/35B presets выполняются следующими checkpoint-задачами; rollout `legacy | shadow | active` fail-closed | `tools/architecture_contract_gate.py`, `make architecture-gate`; остальные точки входа planned | [current architecture](CURRENT_ARCHITECTURE.md) · [canonical design](superpowers/specs/2026-08-26-canonical-tool-context-memory-update-design.md) | 🟡 |
