...
> **Статус:** актуально по коду на 2026-07-15.
...
```text
Браузер ── локально / ZeroTier ─────────> Совушка :8051
                                           └─ /lite-api/* ──> proxy :8050
https://les.ovc.me ──> статический лендинг + /updates/* (без доступа к runtime)
proxy :8050
...
| Совушка | `http://127.0.0.1:8051` | GUI и мост `/lite-api/*` в :8050 | Только локально / ZeroTier |
...
`https://les.ovc.me` больше не публикует Совушку и API. Корень отдаёт статический лендинг,
`/updates/*` — канал ограниченных обновлений с проверкой хешей, а прежние `/api/*`, `/lite-api/*`,
`/lite-runtime/*`, `/les*` и `/classic*` отвечают `404`. Живой ЛЕС доступен только локально или
через доверенный ZeroTier-контур. Контракт обновлений: [VPS_PATCH_CHANNEL.md](VPS_PATCH_CHANNEL.md).
