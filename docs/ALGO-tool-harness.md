...
Цикл заканчивается, когда модель возвращает `calls: []`, исчерпан общий
capability call budget либо наступил технический monotonic deadline. Call budget
действует на весь запрос, а не заново на каждый model response; он не выбирает
инструмент или поисковую формулировку за модель. Семантического дедупликатора
запросов, лимита «три попытки» и кодовой оценки достаточности нет. Код механически удаляет только точные
...
read-only tools. В малом restrictive shortlist `web_search` сохраняет приоритет
и не вытесняется алфавитно отсортированными filesystem capabilities из старого
user-profile snapshot. Это только доступность инструмента: вызывать его, формулировать
запросы, завершать исследование и писать финальный ответ продолжает модель.
