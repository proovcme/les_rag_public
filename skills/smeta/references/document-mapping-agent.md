...
## Стабильность решения на свежем прогоне

Каждый прогон независим: не опирайся на «прошлый» ответ и не меняй решение без нового
доказательства. Чтобы одинаковые исходные строки не плавали:

- при одном и том же наборе открытых карточек сохраняй тот же `bind`/`unbound`, если не
  появился явный unit/technology mismatch;
- между соседними шифрами одной таблицы (`…-01` / `…-02` / `…-05`) выбирай по явному критерию
  (крепление, размер, тип, измеритель) и запиши критерий в `reason`; без критерия не
  переключай соседний шифр;
- не делай `unbound` для close-analog, который уже открыт и применим, только из‑за более
  «строгой» формулировки поиска: для `unbound` нужны конкретные причины отказа по составу,
  измерителю или чужой технологии;
- не переключай `exact`↔`analog` и код нормы без изменения opened evidence.

После первичного mapping ЛЕС может запросить self-check в том же прогоне: подтверди `keep`
или точечно `change`. Меняй только явные ошибки соседнего шифра / слабый unbound; новый код
только из показанных кандидатов текущего прогона.
...
Если поиск вернул кандидатов — открой технологически близкие карточки и зафиксируй конкретные
причины отказа и проверку `covered_by`/декомпозиции в `unbound_evidence`. `has_more` не требует
листать страницы. После двух содержательных поисков и проверки близких карточек прими решение;
не повторяй перестановки тех же слов и не ищи бесконечно.
