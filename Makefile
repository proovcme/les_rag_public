...
WORKSPACE_TESTS := tests/test_chat_session_service.py tests/test_workspace_router.py tests/test_workspace_memory.py tests/test_workspace_memory_router.py tests/test_workspace_context.py tests/test_chat_workspace_ui.py
CURRENT_TESTS ?= $(sort $(UNIT_TESTS) $(INTEGRATION_TESTS) $(RAG_CORE_TESTS) $(FOCUS_TESTS) $(MEMORY_TESTS) $(SMETA_DOCUMENT_TESTS) $(MODEL_CONNECTION_TESTS) $(WORKSPACE_TESTS) tests/test_code_runtime_map.py tests/test_documentation_contract.py tests/test_test_profiles.py tests/test_software_versions.py)
