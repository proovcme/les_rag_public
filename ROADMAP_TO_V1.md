...
- ✅ Windows first-run переведён на нативный setup wizard: bundled Python/uv готовятся
  автоматически, внешние компоненты и пользовательские модели дают `setup_required`, а не
  закрывают приложение. Мастер остаётся доступным после запуска как справка из tray.
