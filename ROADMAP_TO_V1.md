...
## Текущая линия после 0.28.1

`0.28.1` закрепляет публичный Windows-контур: автономный офлайн runtime ЛЕС,
...
3. **0.29.0 — Agent Foundation.** Единый Tool Registry для внутренних tools и MCP,
...
4. **0.29.1 — controlled actions.** Typed compute, автоматические draft/append-only
...
2. **Память и управление контекстом.** Отдельные durable-факты, краткая рабочая память и история;
   видимые бюджеты, включение/исключение, сжатие и удаление без бесконтрольного заполнения prompt.
3. **Стабильность.** Crash recovery, диагностика внешних компонентов, нагрузочные и upgrade/rollback
   тесты, понятные деградации вместо скрытых fallback.
4. **macOS и Linux.** Довести desktop/server профили до того же явного lifecycle и документационного
   контракта, что и Windows, без платформенных defaults в предметном коде.
...
Это roadmap, а не обещание уже поставленных возможностей. Полный сметный workflow и
защищённый `proxy/smeta_core/**` не переписываем: в `0.28.3` наружу выходит только
тонкий tool-adapter к уже проверенному application-контуру. Специализированный chat-route,
снятый в 0.28.0, не возвращается. Любой основной workflow
...
runtime всегда берутся из `docs/RELEASE_LEDGER.md` и `GET /api/version`.
...
## Операционный пакет 2026-07-27

- ✅ Локальный document-сметчик получил повторяемый Qwen-профиль: `qwen3.5:9b`,
  `temperature=0`, фиксируемый seed, batch=5, стабильный порядок model-authored запросов и
  восстановление session/history после закрытия вкладки. Профессиональная ревизия остаётся
  отдельной immutable R2, а не code-side self-check.

- ✅ Windows first-run переведён на provider-neutral setup catalogue: bundled Python/uv и exact
  locked dependencies готовятся офлайн, внешние компоненты дают capability warnings и не
  закрывают приложение. Каталог остаётся доступным после запуска как справка из tray.

- ✅ Готовность RAG больше не объявляет деградацию при точном совпадении активной configured
  generation, dense+sparse, fingerprint, MetaDB и lexical-проекции только из-за отсутствующего
  старого advisory-marker.
- ✅ Сметная typed SQLite-база и необязательный векторный индекс карточек норм показаны раздельно.
- ✅ В «Документах» заведены закреплённые служебные датасеты: нормативы/чек-листы, прайсы,
  сметные источники; профильный чат получает их через module scope.
- ✅ Чат принимает явный выбор до 20 документов одного выбранного корпуса; retrieval остаётся
  внутри этих файлов, а модель сравнивает и формулирует вывод сама.
- ✅ Первый GUI-срез Л.И.С.Т. Студии создаёт append-only DOCX/XLSX draft-ревизии из шаблонов,
  показывает источники/пропуски и хранит manifest+SHA-256 без изменения оригиналов.
- ✅ Агентный документальный срез: `office_document_ir_v1` позволяет Л.Е.С. предлагать только
  ручные поля по выбранным exact/FTS-фрагментам с evidence/confidence/assumptions. Л.И.С.Т.
  показывает IR в GUI и выпускает draft только после применения и явного подтверждения человека.
- ✅ Явная настройка длины управляет реальным generation budget; стандарт сохраняет успешный
  baseline 8 192 токена, варианты не влияют на retrieval/route/tool choice.
- ✅ Добавлен read-only vision-tool страницы/области исходного PDF: модель по явному запросу
  «посмотри глазами» сама выбирает инструмент, код только рендерит пиксели и сохраняет page provenance.
- ✅ PDF-контур стал общим для RAG и GUI: `list.pdf_page_passport.v1` определяет тип страницы,
  качество text layer, OCR-needed, таблицы/графику/штамп и bbox evidence. OCR `## Стр. N`
  сохраняется как page node, а выбранный PDF сразу показывает тот же паспорт и PNG-превью в Л.И.С.Т.
...
## 0. Актуализация 2026-07-14

Старые номера runtime в последующих разделах — исторические отметки закрытия конкретных проверок,
а не текущая версия. Текущая траектория v1 включает корпусную навигацию, управляемые инструменты,
системный слой PDF/XLS, CAD/DWG-проекции и единый RRF-контур. Актуальная правда по версии и
развёртыванию — `docs/RELEASE_LEDGER.md`.

### Что уже фактически закрыто

```text
version/deploy stamp есть
выпускная ветка сведена в main; dev commit и runtime commit сверяются через /api/version
...
Основная публичная платформа v1 — **Windows/Tauri**. Answer provider, embeddings и Qdrant
подключаются независимо и принадлежат пользователю; Ollama/Docker — совместимые варианты, а не
часть ЛЕС. macOS и Linux развиваются как reference/server profiles без скрытых Windows defaults.
...
кабельную таблицу или ВОР. С 0.24.30 общий page-passport работает и в RAG, и в GUI:
показывает page/table locator, preview, `scan|damaged → requires_ocr` и координатные
фрагменты. Открытый остаток этапа — точные XLS/PDF reader API по диапазону/строке/
таблице, запуск подтверждённого OCR workflow и долговечный resumable sidecar на
больших архивах, а не новые эвристические категории.
...
Текущее состояние 0.24.43: поэтапный catalog→scoped search→typed read работает; поисковая карточка
показывает scope/unit/resource/matched-query, bind хранит явное модельное сравнение кандидатов, а
global review получает компактную карту с lazy reread. Подготовка human-only golden очереди готова;
каталог ограничен семействами/сборниками и не может повторно раздувать локальный контекст. Live Qwen
9B завершает одну строку, но остаётся медленным и профессионально нестабильным, поэтому default не
меняется. Следующий профессиональный гейт — экспертно разметить первые 20–30 строк и затем расширить набор.
