...
- **Целостность решения модели в сметах:** в режиме `estimate` код и Codex-аудитор не заменяют,
  не удаляют и не «улучшают» выбранные моделью нормы, аналоги, coverage или коэффициенты.
  Код исполняет инструменты, проверяет структурную ссылочную целостность/единицы/provenance и считает.
  Аудит выдаётся отдельно от модельной ЛСР. Новая исправленная ревизия создаётся только по явной
  просьбе пользователя изменить решения модели.
