...
- Hybrid retrieval: production default — named `dense` + `bm25_sparse` и Qdrant native `Fusion.RRF`; SQLite FTS остаётся третьим exact-word/file safety channel перед общим rerank/context expansion. Любая новая production collection создаётся только с `les.rag.index-contract.v2`; обычный parse без совместимого contract запрещён, а named-point без обоих vector channels не записывается. Старый `migrate_qdrant_native_hybrid.py`, копировавший существующие dense-вектора, выведен из эксплуатации: он мог переносить смешанное embedding-пространство. Полный перенос выполняется только переэмбеддингом через `tools/build_rag_contract_sibling.py`, затем обязательны `tools/rag_rrf_readiness.py`, FTS projection и live filtered RRF по каждому dataset. Текущий runtime остаётся lexical-only до завершения полной `les_rag_qwen3_06b_contract_v2`; fail-closed guard не обходить.
- **Полный clean rebuild (0.24.0.360):** bounded canary v1 остаётся только историческим доказательством Qwen RRF. Целевая общая коллекция — `les_rag_qwen3_06b_contract_v2`: полный CoreML re-embed всех indexed datasets через `tools/build_rag_contract_sibling.py`, migration report с source coverage `1.0`, затем `tools/rag_rrf_readiness.py --live-rrf` и FTS projection. Целевая сметная коллекция — отдельная `les_smeta_norm_cards_v3`, построенная с нуля тем же runtime CoreML encoder плюс BM25 sparse. Ни одна из них не становится production до полного gate; `native_v1`, `contract_v1` и `smeta_norm_cards_v2` не усыновлять и не дозаписывать как новый production.
