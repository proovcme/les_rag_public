...
Expected closeout baseline on 2026-05-31:
...
- Structured Rules: Google LangExtract schema extraction to SQLite `structured_rules` table with exact character offsets for absolute precision.
...
- Keep the `_parse_with_markitdown` fallback pipeline intact to guarantee clean mammot/pandas conversion if python dependencies are altered.
