...
- **Типовые формы:** `GET /api/forms`, `POST /api/forms/{id}/generate` → docx/xlsx/html. Есть: `aosr`, `spec_gost21110` (ГОСТ 21.110 ф.1), `vor`, `smeta_lsr` (ЛСР 421/пр), `ks2`/`ks3`/`ks6a`. Blank по умолчанию; заполненные КС: `source=last_lsr` (КС-2/КС-3 из последней ЛСР) или `source=field_journal` (КС-6а из confirmed журнала). Чат: `/кс-2` `/кс-3` `/кс-6а` или «собери КС-2». Дескрипторы — `config/forms/*.yaml`; сервис `ks_forms_service`.
