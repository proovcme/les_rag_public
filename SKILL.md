...
- **Полный clean rebuild (0.24.0.384):** рантайм использует стабильные aliases `les_rag` и `les_smeta_norm_cards`; номера поколений не протекают в конфигурацию потребителей. Физические поколения `les_rag_qwen3_06b_native_v2` и `les_smeta_norm_cards_v3` строятся с нуля для атомарного переключения и отката: общий корпус — полный CoreML re-embed всех indexed datasets, сметный — тот же runtime CoreML encoder плюс BM25 sparse. Перед сменой alias обязательны readiness, audited source/exclusion coverage, live RRF и FTS projection; старые поколения сохраняются до подтверждённого production switch и известного rollback.
