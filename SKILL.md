...
- **ЭТМ Product API → КАЦ:** креды только в `config/local/secrets.env` (gitignore; шаблон `secrets.env.example`). Пока ЛСР собирается, без рестарта proxy: `uv run python tools/etm_live_smoke.py`. После рестарта LES: Инструменты → «ЭТМ: настроен» и lookup кода. Не pricebook и не выбор ГЭСН. Канон: `docs/ALGO-etm-price.md`.
