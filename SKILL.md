...
- Public `https://les.ovc.me` — изолированный demo-контур: `/` оставляет минимальный
  лендинг, `/login` принимает ключ доступа, а после входа `/provider-setup` требует
  выбрать локальную MLX-модель (медленнее, без внешнего ключа) либо OpenRouter/OpenAI
  BYOK. Облачный ключ живёт только в process-memory vault текущей сессии до 12 часов
  и не меняет общий `.env`. Анонимный `/classic` редиректит на `/login`, а
  `POST /api/chat` возвращает `401`; публичный health endpoint не означает открытый чат/API.
