...
- Hybrid retrieval (W2.7): текущий рантайм — sibling-коллекция `les_rag_qwen3_06b_native_v1` с named vectors `dense` + `bm25_sparse`, `RAG_QDRANT_SCHEMA=named`, `RAG_HYBRID_BACKEND=qdrant_native`. Native Query API `Prefetch+Fusion.RRF` даёт dense+sparse shortlist, затем тот же путь добирает SQLite FTS exact-word/doc-filter safety merge и общий rerank/postprocess; FIRE/HVAC gate 16/16. Миграция без полного reindex: `uv run python tools/migrate_qdrant_native_hybrid.py --src les_rag_qwen3_06b --dst les_rag_qwen3_06b_native_v1 --create`; после первичного копирования `188068` точек удалены `108` orphan-точек PENDING-документа, активный runtime count = `187960` и `points_match_sqlite_chunks=true`. FTS-проекция: `uv run python tools/build_lexical_index.py --collection les_rag_qwen3_06b_native_v1 --db /Users/ovc/LES/data/les_meta_qwen.db --rebuild`. Откат: `RAG_COLLECTION_NAME=les_rag_qwen3_06b`, `RAG_QDRANT_SCHEMA=unnamed`, `RAG_HYBRID_BACKEND=lexical`, рестарт proxy. Старый sparse sidecar `{collection}_sparse` сохранён как legacy/rollback path.
- **Integrity sibling canary (0.24.0.352, НЕ production):** активная `les_rag_qwen3_06b_native_v1` смешивает 5 embedding fingerprints и не получает manifest задним числом. Отдельная `les_rag_qwen3_06b_contract_v1` построена read-only re-embed-ом: по 256 source points BAI/ПД ИЦ/FIRE → `397/297/278`, total `972`; audit `972/972` имеет один Qwen/Core ML fingerprint и полное token-budget/sanitation metadata, manifest fingerprint `33507479e43bd3ba8afc3b3bfeb86ad8033c7e1b3912d92a6a894b8ae288cb3c`. Продолжение идемпотентно: `LES_EMBED_PROFILE=qwen EMBED_BACKEND=coreml COREML_EMBED_FALLBACK=false uv run python tools/build_rag_contract_sibling.py --src les_rag_qwen3_06b_native_v1 --dst les_rag_qwen3_06b_contract_v1 --source-db /Users/ovc/LES/data/les_meta_qwen.db --contract-path /Users/ovc/LES/data/les_meta_qwen_contract_v1.db.index-contract.json --resume`. Перед switch теми же embedding env запустить `tools/rag_index_contract_audit.py --collection les_rag_qwen3_06b_contract_v1 --limit 1000`. Не переключать runtime на bounded canary: он не покрывает весь scope.
